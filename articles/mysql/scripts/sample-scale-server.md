...
這個範例 CLI 指令碼之後查詢 hello 度量縮放單一 Azure 資料庫的 MySQL server tooa 不同效能層級。
...
在這個範例指令碼中，變更反白顯示 hello 行 toocustomize hello 管理員使用者名稱和密碼。 取代您自己的訂用帳戶 id 與 hello az 監視器命令中使用的 hello 訂用帳戶 id。[!code-azurecli-interactive[main](../../../cli_scripts/mysql/scale-mysql-server/scale-mysql-server.sh?highlight=15-16 "Create and scale Azure Database for MySQL.")]
