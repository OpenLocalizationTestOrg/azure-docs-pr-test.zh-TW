...
---
# <a name="use-powershell-toocreate-a-single-azure-sql-database-and-configure-a-firewall-rule"></a>使用 PowerShell toocreate 單一 Azure SQL 資料庫並設定防火牆規則

此 PowerShell 指令碼範例會建立 Azure SQL Database 並設定伺服器層級防火牆規則。 一旦 hello 指令碼順利執行之後，您可以從所有 Azure 服務存取 SQL Database 的 hello 與 hello 設定 IP 位址。 
