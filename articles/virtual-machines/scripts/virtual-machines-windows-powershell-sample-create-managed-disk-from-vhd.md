...
請勿在短時間內從 VHD 檔案建立多個相同的受控磁碟。 toocreate 管理從 vhd 檔案的磁碟建立 hello vhd 檔案的 blob 快照集，則會使用受管理的 toocreate 磁碟。 可以建立只有一個 blob 快照集，使磁碟建立失敗，一分鐘內到期的 toothrottling。 tooavoid 這項調整流速，建立[hello vhd 檔案從受管理的快照集](virtual-machines-windows-powershell-sample-create-snapshot-from-vhd.md?toc=%2fpowershell%2fmodule%2ftoc.json)，然後使用 hello 管理快照集 toocreate 多個受管理的磁碟在短時間內。 
