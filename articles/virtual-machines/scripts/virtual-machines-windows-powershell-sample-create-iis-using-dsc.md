...
此指令碼會建立執行 Windows Server 2016 的 Azure 虛擬機器，然後使用 hello Azure 虛擬機器 DSC 延伸模組 tooinstall IIS。 執行 hello 指令碼之後, 您可以存取 hello 預設 IIS 網站上 hello 虛擬機器的 hello 公用 IP 位址。
...
| [Set-AzureRmVMExtension](/powershell/module/azurerm.compute/set-azurermvmextension) | 新增 VM 延伸模組 toohello 虛擬機器。 在此範例中，hello 自訂指令碼延伸模組是使用的 tooinstall IIS。 |
