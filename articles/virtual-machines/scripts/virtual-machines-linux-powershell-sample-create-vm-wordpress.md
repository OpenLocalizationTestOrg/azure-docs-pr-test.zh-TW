...
此指令碼會建立虛擬機器，並使用 hello Azure 虛擬機器自訂指令碼延伸 tooinstall WordPress。 執行 hello 指令碼之後, 您可以存取 hello WordPress 設定站台`http://<public IP of VM>/wordpress`。 
...
| [Set-AzureRmVMExtension](/powershell/module/azurerm.compute/set-azurermvmextension) | 新增 hello 自訂指令碼擴充 toohello 虛擬機器，這會叫用指令碼 tooinstall WordPress。 |
