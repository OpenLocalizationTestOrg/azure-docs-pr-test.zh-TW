...
在 hello Azure 傳統部署模型中，沒有 hello Azure 內部負載平衡器 （Azure 負載平衡器服務） 與 hello 雲端服務群組之間的相依性。 每個內部負載平衡器需要一個雲端服務群組。

Azure 資源管理員 中，您不需要 Azure 資源群組 toouse Azure 負載平衡器。 hello 環境是比較簡單且更有彈性。
...
1. 在 Windows 叢集組態中附加 hello 虛擬機器 (Vm) 的其他 Azure 虛擬硬碟 (VHD) tooeach。
2. 在兩個虛擬機器節點上執行 SIOS DataKeeper Cluster Edition。
3. 設定 SIOS DataKeeper 叢集版本，它會反映 hello hello 額外附加 VHD 的磁碟區從 hello 來源虛擬機器 toohello 額外附加 VHD 的磁碟區 hello 目標虛擬機器的內容。 SIOS DataKeeper 抽象化 hello 來源和目標本機磁碟區，並接著呈現 tooWindows Server 容錯移轉叢集與一個共用磁碟。
...
如需在高可用性案例中保護 SAP 元件的詳細資訊，請參閱[適用於 SAP NetWeaver 的 Azure 虛擬機器規劃和實作](planning-guide.md)。
...
Hello Azure 儲存體帳戶是潛在的單一失敗點，因為它是重要的 toohave 至少兩個 Azure 儲存體帳戶，分兩個以上的虛擬機器。 在理想的安裝程式中執行 SAP dialog 執行個體的每部虛擬機器的 hello 磁碟將部署於不同的儲存體帳戶。
...
hello ASCS/SCS 多重 SID 範本，在 hello tooset [ASCS/SCS 多重 SID 範本][sap-templates-3-tier-multisid-xscs-marketplace-image]，輸入 hello 下列參數的值：
...
hello 資料庫多 SID 範本，在 hello tooset[資料庫多 SID 範本][sap-templates-3-tier-multisid-db-marketplace-image]，輸入 hello 下列參數的值：
...
hello 應用程式伺服器多 SID 範本中，在 hello tooset[應用程式伺服器多 SID 範本][sap-templates-3-tier-multisid-apps-marketplace-image]，輸入 hello 下列參數的值：
...
兩個節點上安裝 SIOS DataKeeper 之後，您會需要 toostart hello 組態。 hello 組態 hello 目標是 toohave hello 其他 Vhd 附加 tooeach hello 虛擬機器之間的同步的資料複寫。
