...
在本文中，即將討論 hello 不同用法 hello 磁碟，及再討論 hello 不同類型的磁碟，您可以建立及使用。 本文也適用於 [Windows 虛擬機器](../windows/about-disks-and-vhds.md)。
...
每個 VM 都包含一個暫存磁碟。 hello 暫存磁碟的應用程式和處理序提供短期儲存，預期的 tooonly 存放區資料，例如分頁檔。 Hello 暫存磁碟上的資料可能會遺失期間[維護事件](../windows/manage-availability.md?toc=%2fazure%2fvirtual-machines%2fwindows%2ftoc.json#understand-vm-reboots---maintenance-vs-downtime)或當您[重新部署 VM](../windows/redeploy-to-new-node.md?toc=%2fazure%2fvirtual-machines%2flinux%2ftoc.json)。 Hello VM 標準重開機，應保存 hello hello 暫存磁碟機上的資料。

Linux 的虛擬機器上 hello 磁碟通常是**/開發/sdb** is 格式化及裝載太**/mnt**由 hello Azure Linux 代理程式。 hello hello 暫存磁碟的大小會根據 hello hello 虛擬機器大小而不同。 如需詳細資訊，請參閱 [Linux 虛擬機器的大小](../windows/sizes.md)。
...
* [將磁碟連接](add-disk.md?toc=%2fazure%2fvirtual-machines%2flinux%2ftoc.json)vm tooadd 額外的存放裝置。
