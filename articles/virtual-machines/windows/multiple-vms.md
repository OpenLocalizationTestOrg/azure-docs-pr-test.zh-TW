...
其中一個可建立許多類似 VM 的方法，就是使用 Azure Resource Manager 資源迴圈 建構。
...
虛擬機器擴展集是用來部署及管理一組相同 VM 的 Azure 計算資源。 由於所有 VM 的設定均相同，所以能輕鬆相應縮小和增加 VM 擴展集。 您只要變更集合中的 VM 數目即可。 您也可以根據工作負載的需求自動調整 VM 擴展集。
