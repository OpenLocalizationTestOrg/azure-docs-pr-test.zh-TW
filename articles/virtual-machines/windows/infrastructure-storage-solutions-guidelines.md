...
作業系統磁碟和資料磁碟具有 4TB 的大小上限。 您可以使用 Windows Server 2012 或更新版本中的「儲存空間」，匯集資料磁碟來提供大於 4TB 的邏輯磁碟區給 VM，以超越此限制。
...
除了讓您能夠建立大於 4TB 的磁碟，在許多情況下，針對資料磁碟使用等量，可透過允許多個 Blob 備份單一磁碟區的儲存體來增強效能。 從單一邏輯磁碟寫入和讀取資料所需的 I/O 會透過等量速度以平行方式繼續進行。
