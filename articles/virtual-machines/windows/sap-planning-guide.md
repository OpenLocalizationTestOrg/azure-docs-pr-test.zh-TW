...
---
# <a name="sap-netweaver-on-azure-windows-virtual-machines-vms--planning-and-implementation-guide"></a>Azure Windows 虛擬機器 (VM) 上的 SAP NetWeaver - 規劃和實作指南
...
平台提供 hello 紙張補充 hello SAP 安裝文件集和 SAP 附註 」 包含 hello 安裝和部署上的 SAP 軟體的主要資源。

## <a name="summary"></a>摘要
雲端運算廣泛使用的名詞日漸重要內 hello IT 產業、 從小型公司向上 toolarge 和一家跨國企業。

Microsoft Azure 為 hello 它提供了各式各樣的新的可能性的 microsoft 雲端服務平台。 現在的客戶可以 toorapidly 佈建和取消佈建應用程式以 hello 雲端服務讓它們不受限制的 tootechnical 或預算限制。 而不是投入時間和預算硬體基礎結構中，公司可以專注於 hello 應用程式、 商務程序和它的優點的客戶與使用者。
...
hello 紙張本身將焦點放在兩個主要層面：

* hello 第一個部分將說明在 Azure 上的 SAP NetWeaver 架構應用程式的兩個支援的部署模式。 也將說明考慮到 SAP 部署之一般處理 Azure 的方式。
* hello 第二個部分會詳細說明實作 hello 兩個不同的案例 hello 第一個部分中所述。
...
* DBMS 層和應用程式層，例如 SAP ERP 開發系統、 SAP BW 測試系統、 SAP CRM 生產系統等的 SAP 系統： hello 組合.. 在 Azure 中不支援的部署 toodivide 這些內部部署與 Azure 之間的兩個層級。 這表示 SAP 系統可以在內部部署或在 Azure 部署。 不過，您可以部署至 Azure 或內部部署 SAP 版圖的 hello 不同系統。 例如，您可以部署 hello SAP CRM 開發和測試系統在 Azure 中的，但是 hello SAP CRM 生產系統在內部。
* 僅限雲端部署： 部署其中 hello Azure 訂用帳戶未連線透過網站的站台或 ExpressRoute 連線 toohello 內部部署網路基礎結構。 在一般 Azure 文件中，這類部署也會描述為「僅限雲端」的部署。 採用這種方法部署的虛擬機器可透過存取 hello 網際網路和公用 IP 位址和/或公用 DNS 命名指派的 toohello Vm 在 Azure 中。 Microsoft windows hello 在內部部署 Active Directory (AD) 和 DNS 未延伸 tooAzure 在這些類型的部署。 因此 hello Vm 不屬於 hello 內部部署 Active Directory。 此規則也適用於使用 OpenLDAP + Kerberos 的 Linux 實作。
...
* 跨單位： 說明的案例，當 Vm 是已部署的 tooan Azure 訂用帳戶具有站台間，多站台或 ExpressRoute hello 在內部部署資料中心與 Azure 之間的連線能力。 在一般 Azure 文件中，這類部署也會描述為跨單位案例。 hello 原因 hello 連線是 tooextend 內部網域中，內部部署 Active Directory / OpenLDAP 和內部部署 DNS 至 Azure。 hello 內部橫向是擴充的 toohello hello 訂用帳戶的 Azure 資產。 透過這樣延伸，hello Vm 可以是 hello 在內部部署網域的一部分。 Hello 在內部部署網域的網域使用者可以存取 hello 伺服器，並且可以執行服務那些 Vm （例如 DBMS 服務）。 但無法在內部部署的 VM 和 Azure 部署的 VM 之間進行通訊和名稱解析。 這是我們預期大部分 SAP 資產 toobe 部署中的 hello 案例。  如需詳細資訊，請參閱[這篇文章][vpn-gateway-cross-premises-options]和[這個主題][vpn-gateway-site-to-site-create]。

> [!NOTE]
> SAP 生產系統支援跨單位部署 SAP 系統，其中執行 SAP 系統的 Azure 虛擬機器是內部部署網域的成員。 支援跨單位組態，以便將部分或完整的 SAP 架構部署到 Azure。 即使在 Azure 中執行 hello 完整的 SAP 環境，則需要具有那些 Vm 屬於內部部署網域和廣告 / OpenLDAP。 Hello 文件的先前版本中我們剛才討論過混合式 IT 案例，其中 hello 詞彙 '混合' 已進行 root 破解 hello 事實會在內部部署與 Azure 之間的跨單位連線。 此外，該 hello Azure 中的 Vm 屬於 hello hello 事實內部部署 Active Directory / OpenLDAP。
>
>

有些 Microsoft 文件在描述跨單位案例時稍有不同，特別是針對 DBMS HA 組態。 在 hello 的 hello SAP 案例相關的文件、 hello 跨內部部署案例只分布 toohaving 站對站或私人 (ExpressRoute) 連線和 hello 事實 hello SAP 版圖分散在內部部署和 Azure。  
...
順序 toosuccessfully 中部署 SAP 系統到 Azure IaaS 或 IaaS 一般情況下，很重要的 toounderstand hello 明顯差異的傳統外包或主機服務提供者的供應項目 hello 和 IaaS 供應項目。 Hello 傳統主機服務提供者或外包會調整基礎結構 （網路、 儲存體和伺服器類型） toohello 工作負載而客戶希望 toohost，改為 hello 客戶責任 toochoose hello 適當工作負載在 IaaS 部署。
...
請記住，hello hello 上方的連結中所列的限制是上限。 它並不表示 hello 限制任何 hello 資源，例如可以在所有情況下提供 IOPS。 雖然 hello 例外狀況會是 hello 選擇 VM 類型的 CPU 和記憶體資源。 針對 SAP 所支援的 hello VM 類型，hello CPU 和記憶體資源都已保留，因此可用在任何時間點 hello VM 內耗用的時間。

hello 像其他 IaaS 平台的 Microsoft Azure 平台是多租用戶的平台。 這表示儲存體、網路和其他資源會在租用戶之間共用。 智慧型節點和配額邏輯是使用的 tooprevent 一個租用戶大幅影響 hello 效能的另一個租用戶 （雜訊像素）。 雖然 Azure 嘗試 tookeep 頻寬的小型、 高度分享的平台差異中的邏輯通常會比很多客戶資源/頻寬可用性 toointroduce 較大的變化會使用的 tooin 其內部部署。 如此一來，您可能會遇到分鐘 toominute 從不同層級方面 toonetworking 或儲存體 I/O （hello 磁碟區，以及延遲） 的頻寬。 在 Azure 上的 SAP 系統，可能經歷較大的變化，比在內部部署系統中的 hello 機率需要 toobe 列入考量。

最後一個步驟是 tooevaluate 可用性需求。 這可能是，該 hello 基礎 Azure 基礎結構必須 tooget 更新，且必須執行重新啟動 Vm toobe hello 主機。 在這些情況下，也會關閉並重新啟動正在這些主機上執行的 VM。 hello 計時的這類的維護期間特定區域的非基本營業時間內完成，但較寬 hello 潛在窗口的幾個小時的期間將會重新啟動電腦。 有各種技術 hello 可能是 Azure 平台內設定 toomitigate 部分或所有 hello 這類更新的影響。 未來的增強功能的 hello Azure 平台、 DBMS 和 SAP 應用程式所設計的這類的重新啟動 toominimize hello 影響。

為了 toosuccessfully 部署到 Azure 的 SAP 系統、 hello 在內部部署 SAP 系統的作業系統，資料庫和 SAP 應用程式必須出現在 hello SAP 的 Azure 支援矩陣，在 hello 資源 hello Azure 基礎結構可以提供，而且這可以使用可用性 Sla Microsoft Azure 提供的 hello。 當識別出這些系統，您會需要 toodecide hello 下列兩個部署案例的其中一個上。
...
* hello SAP 系統 （和 VM(s)) 代表在 Azure 只需要公用網際網路連線的使用者權限，而且不需要連接 tooother Vm 在 Azure 中的獨立案例。
* SAPGUI 和瀏覽器安裝並直接在 hello VM 上執行。
* 快速 VM toohello 原始狀態重設和新的部署該原始狀態再需要。
* Hello 案例示範和訓練案例的多個 Vm，Active Directory 中實現 /，就需要為每一組 Vm OpenLDAP 和 （或） DNS 服務。
...
此案例是具有許多可能之部署模式的跨單位案例。 它可以描述為做為 Azure 上執行 hello SAP 版圖在內部的某些部分和 hello SAP 環境的其他部分。 Hello 事實 hello SAP 元件的一部分會在 Azure 上執行的所有層面都應該都是透明的使用者。 因此 hello SAP 傳輸更正系統 (STMS)、 RFC 通訊、 列印、 安全性 （例如 SSO)、 等會一起順暢運作的 hello SAP 系統在 Azure 上執行。 但 hello 跨內部單位案例也說明其中 hello 完整的 SAP 環境在 Azure 中執行與 hello 客戶的網域和 DNS 延伸至 Azure 的案例。

> [!NOTE]
> 這是支援執行生產性 SAP 系統 hello 部署案例。
>
>
...
>
> * 以不同的部署方法執行不同的 SAP 應用程式層。 例如 hello DBMS 層在內部，但 hello SAP 應用程式層執行 Vm 部署為 Azure Vm，反之亦然。
> * SAP 層的部分元件在 Azure 中，部分元件在內部部署。 例如 分割 hello SAP 應用程式層之間的內部部署和 Azure Vm 執行的個體。
...
hello Azure 平台可減少 hello 需要預先採購技術和基礎結構。 它可簡化維護和操作應用程式藉由提供隨運算和儲存體 toohost 調整和管理 web 應用程式和連接的應用程式。 自動化設計高可用性的平台而動態調整 toomatch 使用需求與 hello 選項隨用隨付計價模型的基礎結構的管理。
...
就操作觀點而言，Azure 虛擬機器服務提供類似的 hello 體驗做為內部部署的虛擬機器。 不過，它有 hello 更大的優點，您不需要 tooprocure，管理並管理 hello 基礎結構。 開發人員和管理員具有完全控制權的 hello 這些虛擬機器內的作業系統映像。 系統管理員可以從遠端登入這些虛擬機器 tooperform 維護和疑難排解工作，以及軟體部署工作。 在考慮 toodeployment hello 唯一限制是 hello 大小和 Azure Vm 的功能。 您可能無法如內部部署般，對這些項目進行細微的設定。 您可以選擇包含下列項目組合的 VM 類型︰
...
hello 大小和各種不同的虛擬機器大小的限制所提供的資料表中可以看到[這篇文章][virtual-machines-sizes]

如您所見，虛擬機器有不同的系列。 自 2015 年 12 月中，您可以分辨 hello 遵循系列的 Vm:

* A0-A7 VM 類型︰並未全部通過 SAP 認證。 這是 Azure IaaS 引進的第一個 VM 系列。
* A8-A11 VM 類型：高效能運算執行個體。 在計算主機上的執行效能高於其他 A 系列 VM。
* D 系列 VM 類型︰執行效能高於 A0-A7。 不是所有的 hello VM 類型經過與 SAP 驗證。
* DS 系列 VM 類型： 與 D 系列使用相同的主機，但會無法 tooconnect tooAzure 高階儲存體 (請參閱第章[Azure 高階儲存體][ planning-guide-3.3.2]的這份文件)。 同樣地，此 VM 類型並未全部通過 SAP 認證。
* G 系列 VM 類型︰高記憶體 VM 類型。
* GS 系列 VM 類型： 像是 G 系列，但包括 hello 選項 toouse Azure 高階儲存體 (請參閱第章[Azure 高階儲存體][ planning-guide-3.3.2]的這份文件)。 使用 GS 系列 Vm 做為資料庫伺服器時，強制 toouse 高階儲存體的資料庫資料和交易記錄檔
...
請注意不完全不同的 VM 系列可能會提供每一種 hello Azure 區域 （適用於 Azure 區域，請參閱下一章）。 另請注意，並非所有 VM 或 VM 系列都通過 SAP 認證。
...
* [部署及管理虛擬機器使用的 Azure 資源管理員範本和 hello Azure CLI][virtual-machines-linux-cli-deploy-templates]
* [使用 Azure Resource Manager 和 PowerShell 管理虛擬機器][virtual-machines-deploy-rmtemplates-powershell]
* <https://azure.microsoft.com/documentation/templates/>

另一個有趣的功能是 hello 能力 toocreate 映像，從虛擬機器，可讓您 tooprepare 特定儲存機制是無法 tooquickly 部署符合需求的虛擬機器執行個體。
...
升級網域代表協助的 toodetermine VM 中 SAP 系統，可包含多個 Vm 中執行的 SAP 執行個體，更新的方式的邏輯單元。 進行升級時，Microsoft Azure 會開始逐一更新這些 「 升級 」 網域一個 hello 程序中。 藉由在部署時將 VM 散佈到不同的升級網域，即可防止您的 SAP 系統可能有一部分停機。 中排序 tooforce Azure toodeploy hello Vm 的 SAP 系統分散到不同的 「 升級 」 網域，您需要 tooset 特定的屬性在部署期間的每個 VM。 類似 tooFault 定義域、 Azure 縮放單位會分成多個升級網域。 順序 toodirect hello Azure 網狀架構控制器 toodeploy 一組的 Vm，透過不同的 「 升級 」 網域，您需要 tooassign Azure 可用性設定組 toohello Vm 在部署階段。 如需有關「Azure 可用性設定組」的詳細資訊，請參閱下一章 [Azure 可用性設定][planning-guide-3.2.3]。

#### <a name="18810088-f9be-4c97-958a-27996255c665"></a>Azure 可用性設定組
一個 Azure 可用性設定組內的 azure 虛擬機器將會透過不同的容錯和升級網域散發的 hello Azure 網狀架構控制器。 hello 目的 hello 分散至不同的容錯和升級網域是的 tooprevent 從 SAP 系統的所有 Vm 都關機 hello 案例的基礎結構維護或一個 「 故障 」 網域內的失敗。 根據預設，VM 不是可用性設定組的一部分。 hello 參與 VM 的可用性設定組被定義在部署期間或在稍後重新設定和重新部署 VM。

toounderstand hello 概念 Azure 可用性設定組和 hello 方式可用性設定組與 tooFault 」 和 「 升級 」 網域，請閱讀[這篇文章][virtual-machines-manage-availability]
...
Microsoft Azure 虛擬機器使用不同的儲存體類型。 在 Azure 虛擬機器服務上實作 SAP 時，重要 toounderstand hello 差異存放區這兩種主要類型：
...
上述陳述式會套用 toohello 經過驗證和 SAP 的 VM 類型。 絕佳的 IOPS 及輸送量與 hello VM 系列限定運用的一些 DBMS 功能。 請參閱 hello[的 DBMS 部署指南][ dbms-guide]如需詳細資訊。

Microsoft Azure 儲存體提供持續性儲存體和 hello 一般層級的保護和備援在 SAN 存放裝置上看到。 Azure 儲存體為基礎的磁碟是虛擬硬碟 (Vhd) 位於 hello Azure 儲存體服務。 hello 本機作業系統磁碟 (Windows c:\, Linux / (/ 開發/sda1)) 會儲存在 hello Azure 儲存體，和其他磁碟區/磁碟掛接的 toohello VM 儲存在這裡太。

可能的 tooupload 現有的 VHD 從內部部署或建立從在 Azure 中的空的並附加這些 toodeployed Vm。 這些 VHD 會當做 Azure 磁碟。
...
* 這會分散到不同資料中心內的 hello 三個影像的區域備援儲存體 hello 相同 Azure 區域。
* 預設的備援層級是以非同步方式將 hello 內容複寫到另一個 3 的映像到另一個 Azure 區域裝載於 hello hello 資料的地理備援地理政治相同的區域。

另請參閱 hello 資料表頂端方面 toohello 不同備援選項中的這篇文章： <https://azure.microsoft.com/pricing/details/storage/>

在存放裝置可以在這裡找到相關事宜 tooAzure 的詳細資訊：
...
Azure 的標準 BLOB 儲存體 Azure IaaS 發行時可用的儲存體的 hello 型別。 每個單一 VHD 會強制執行 IOPS 配額。 發生延遲不是在相同類別，如 SAN/NAS 裝置通常會部署為高階的 SAP 系統裝載在內部部署的 hello。 不過，標準儲存體 Azure 證實足以應付數以百計的 hello 同時部署在 Azure 中 SAP 系統。

Azure 的標準儲存體計費根據 hello 實際儲存的資料，hello 磁碟區的儲存體交易、 傳出資料傳輸與選擇的備援性選項。 可以在 hello 大 1 TB 的大小，建立多 Vhd，但只要維持空白會產生任何費用。 接著，您就會填入一個 100 gb VHD，如果您將支付儲存 100 GB，不能用於建立 vhd 與 hello 名義大小 hello。
...
基於這個目的，許多變更導入的 hello 兩個最重要的：

* Hello Azure 儲存體節點中的 SSD 磁碟的使用方式
* 新的讀取快取，並受到 hello Azure 計算節點的本機 SSD

中相反 tooStandard 儲存體功能並未變更相依於 hello 大小 hello 磁碟 （或 VHD），高階儲存體目前有 3 hello hello 常見問題集區段之前的這篇文章結尾處會顯示這些不同的磁碟類別： <https://azure.microsoft.com/pricing/details/storage/>

您會看到 IOPS/VHD 和磁碟輸送量/VHD 會相依於 hello 的 hello 磁碟的大小類別目錄

這類的 Vhd，但這類的 VHD，獨立於 hello hello hello VHD 中儲存的資料數量的 hello 大小類別目錄中儲存的 hello 實際的資料磁碟區的無法在 hello 案例中的進階儲存體的成本為基礎。

您也可以建立不會直接對應到顯示 hello 大小類別目錄的高階儲存體的 Vhd。 這可能是 hello 的情況下，尤其是將 Vhd 從標準儲存體複製到高階儲存體。 在此情況下對應 toohello 下一個最大高階儲存體磁碟選項會執行。

請注意，只有特定 VM 系列可受益於 hello Azure 高階儲存體。 自 2015 年 12 月中，這些是 hello DS-與 GS 系列。 hello DS 系列是基本上 hello 相同作為 DS 系列有 hello 能力 toomount 高階儲存體的 hello 例外狀況的 D 系列 Vm 此外 tooVHDs 裝載在 Azure 標準儲存體上。 G 系列比較 tooGS 數列相同的動作無效。

如果您正在簽出 hello DS 系列 Vm 中的 hello 一部分[本文][ virtual-machines-sizes]您也會發現有資料磁碟區限制 tooPremium 儲存 Vhd hello hello VM 層級的資料粒度。 不同的 DS 系列或 GS 系列 Vm 也有不同的限制相關事宜 toohello 數目可掛接的 Vhd。 這些限制記載於 hello 發行項，以及先前所述。 但在本質上這表示，如果您如裝載 32 x P30 磁碟/Vhd tooa 單一 DS14 VM 您無法取得 32 x hello P30 磁碟的最大的輸送量。 改為 hello hello 文件中所述的 VM 層級上的最大輸送量會限制資料輸送量。
...
在 Azure 中部署服務或 VM 時，VHD 和 VM 映像的部署必須組織成不同單位，稱為 Azure 儲存體帳戶。 當規劃 Azure 部署時，您需要 toocarefully 考慮 hello Azure 的限制。 在 hello 一邊沒有有限的儲存體帳戶的每個 Azure 訂用帳戶。 Hello 雖然每個 Azure 儲存體帳戶可以保存大量的 VHD 檔案，但沒有固定的限制的總每個儲存體帳戶的 IOPS。 在部署數以百計的 SAP Vm 且 DBMS 系統會建立大量 IO 呼叫時，它會建議 toodistribute 高 IOPS 的 DBMS Vm 之間的多個 Azure 儲存體帳戶。 必須小心不要 tooexceed hello 目前限制 Azure 儲存體帳戶的每個訂閱。 這個概念，因為存放裝置 hello 資料庫部署 SAP 系統不可或缺的一部分，hello 已經被參考的更詳細地討論[的 DBMS 部署指南][dbms-guide]。

如需有關「Azure 儲存體帳戶」的詳細資訊，請參閱[這篇文章][storage-scalability-targets]。 讀取這份文件，您會發現有 hello 限制 Azure 標準儲存體帳戶和 Premium 儲存體帳戶之間的差異。 主要差異是 hello 可以儲存在儲存體帳戶內的資料量。 在標準儲存體 hello 磁碟區是大於高階儲存體的範圍。 Hello 上其他側邊 hello 標準儲存體帳戶是嚴重限制了的 IOPS （請參閱資料行 ' 總要求率 '），而則 hello Azure 高階儲存體帳戶有這樣的限制。 討論的 SAP 系統，特別是 hello DBMS 伺服器的 hello 部署時，我們將討論詳細資料，以及這些差異的結果。

儲存體帳戶，在您目前 hello 目的，是組織和分類不同 Vhd 的 hello 可能性 toocreate 不同容器。 這些容器是通常使用的 tooe.g。 分隔不同的 Vm 的 Vhd。 在單一 Azure 儲存體帳戶下只使用一個容器或使用多個容器對效能不會有太大影響。
...
Microsoft Azure 會提供網路基礎結構可讓所有的案例，我們想要的 hello 對應 toorealize SAP 軟體。 hello 功能包括：
...
針對跨單位的案例我們依賴 hello 事實，hello 在內部部署 AD/OpenLDAP DNS 已經過擴充，透過 VPN 或私人連線 tooAzure。 某些這裡所述的案例中，可能需要 toohave 安裝在 Azure AD/OpenLDAP 複本。
...
建置 Azure 虛擬網路中，您可以定義 hello 的 hello Azure DHCP 功能所配置的私人 IP 位址的位址範圍。 在跨單位案例中，定義 hello IP 位址範圍將仍然會配置使用 DHCP azure。 不過，網域名稱解析都是在內部部署 （假設 hello Vm 會在內部部署網域的一部分），因此可以解決超出不同 Azure 雲端服務的位址。
...
hello hello 虛擬網路卡的 MAC 位址之後可能會變更例如重新調整大小和 hello Windows 或 Linux 客體作業系統將會挑選 hello 新的網路卡，並會自動在此情況下使用 DHCP tooassign hello IP 和 DNS 位址。
...
您可以為一個 Azure 虛擬機器定義多個虛擬網路介面卡 (vNIC)。 與 hello 能力 toohave vNICs 多個您可以啟動 tooset 出網路流量分隔，例如用戶端流量路由傳送透過一個 vNIC 後, 端流量路由傳送到第二個 vNIC。 依存於 VM 有一些不同的限制的 hello 類型目前 toohello vNICs 數目。 如需確切詳細資料、功能和限制，請參閱下列文章︰
...
在排序 toocreate 站對站連線 （在內部部署資料中心 tooAzure 資料中心），您將需要 tooeither 取得和設定 VPN 裝置，或使用路由及遠端存取服務 (RRAS) 導入的 Windows server 的軟體元件2012。
...
點對站 VPN 需要有它自己的 VPN 每個用戶端機器 tooconnect 至 Azure。 我們會查看 hello SAP 案例，並不實用點對站連線能力。 因此，沒有進一步的參考也會指定 toopoint 對站 VPN 連線能力。
...
Microsoft Azure ExpressRoute 可讓 hello 建立私人連線 Azure 資料中心與任一 hello 客戶的內部部署基礎結構之間部署或共置環境中。 ExpressRoute 是由各種 MPLS (封包交換式) VPN 提供者或其他網路服務提供者所提供。 ExpressRoute 連線不會經過 hello 公用網際網路。 ExpressRoute 連線 hello 網際網路提供較高的安全性、 透過多個並聯電路、 速度和低延遲比一般的連線。
...
* 部署虛擬機器後就無法再虛擬網路指派 toohello VM 的可能 toochange hello

### <a name="quotas-in-azure-virtual-machine-services"></a>Azure 虛擬機器服務中的配額
我們需要 toobe 清除有關 hello 事實，hello 儲存，並在 hello Azure 基礎結構中執行各種服務的 Vm 之間會共用網路基礎結構。 如同 hello 客戶自己的資料中心，過度佈建一些 hello 基礎結構資源採用位置 tooa 程度。 hello Microsoft Azure 平台會使用磁碟、 CPU、 網路和其他配額 toolimit hello 資源耗用量和 toopreserve 一致和確定的效能。  hello 不同 VM 類型 （A5、 A6 等） 會有不同的磁碟、 CPU、 RAM 和網路的 hello 數目的配額。

> [!NOTE]
> SAP 所支援的 hello VM 類型的 CPU 和記憶體資源上會預先配置 hello 主機節點。 這表示 hello VM 部署之後，hello hello 主機上的資源將會使用 hello VM 類型所定義。
>
>

您必須考量時，規劃和調整 SAP Azure 解決方案上 hello 每種虛擬機器大小的配額。  說明 hello VM 配額[這裡][virtual-machines-sizes]。

所述的 hello 配額代表 hello 理論上的最大值。  每個 VHD 的 IOPS 的 hello 限制可能達到較小 Io (8kb)，但可能可能無法達到較大 Io (1mb)。  hello IOPS 限制為單一 vhd 的 hello 資料粒度上強制執行。
...
針對新部署的 SAP 系統，您應該已經完成調整工作應該決定 hello hello 系統的 SAPS 需求。
...
**步驟 3**: hello 與 hello SAPS hello 不同 VM 類型的 Azure 的 DBMS 伺服器可以提供比較 hello SAPS 需求。 hello 不同 Azure VM 類型的 saps 相關 hello 資訊記載於 SAP Note [1928533]。 hello 焦點應該 hello DBMS VM 上第一次，因為 hello 資料庫層是 hello 層在不會向外延展 hello 的大部分部署中的 SAP NetWeaver 系統。 相反地，hello SAP 應用程式層可以向外延展。如果支援任何 hello SAP Azure VM 類型可以傳送 hello 所需的 SAPS，無法在 Azure 上執行計劃的 hello SAP 系統的 hello 工作負載。 您必須 toodeploy hello 系統在內部部署或 hello 系統所需 toochange hello 工作負載磁碟區。

**步驟 4**︰如[這裡][virtual-machines-sizes]所述，不論您使用的是「標準儲存體」還是「進階儲存體」，Azure 都會針對每個 VHD 強制執行 IOPS 配額。 視 VM 類型 hello hello 可掛接的 Vhd 數目而有所不同。 如此一來，您可以計算與每個 hello 不同 VM 類型可達到 IOPS 數目上限。 相依於 hello 資料庫檔案配置，您可以等量分割 Vhd toobecome 一個磁碟區 hello 客體作業系統中。 不過，如果 hello 目前 IOPS 磁碟區已部署的 SAP 系統超過 hello 最大 VM 類型的 Azure，而且如果沒有更多的記憶體使用沒有機會 toocompensate hello 計算限制，hello 的 hello SAP 系統的工作負載可能會影響嚴重。 在這種情況下，您可以按其中則不應部署在 Azure 上的 hello 系統的點。

**步驟 5**： 特別是在 SAP 系統部署在內部 2 層組態中，hello 機會 hello 系統，可能需要 toobe 3 層組態中設定。 在此步驟中，您會需要 toocheck 中是否有元件 hello SAP 應用程式層的無法擴充，以及這不會放入 hello CPU 和記憶體資源 hello 不同 Azure VM 類型提供項目。 如果確實有這類元件，hello SAP 系統和其工作負載無法部署到 Azure。 但如果您可以向外延展 hello 到多個 Azure Vm，hello 系統的元件可以部署到 Azure 的 SAP 應用程式。
...
hello Azure 入口網站是一個三個介面 toomanage Azure VM 部署。 hello 基本管理工作，例如從映像，來部署 Vm 可以透過 hello Azure 入口網站。 此外，hello 建立儲存體帳戶、 虛擬網路和其他 Azure 元件也會是工作 hello Azure 入口網站可妥善處理。 不過，像是將 Vhd 從上傳內部 tooAzure 或 Azure 內複製 VHD 所需要協力廠商工具或透過 PowerShell 或 CLI 管理工作。
...
除了重新啟動和關閉虛擬機器，您也附加、 卸離和建立 hello 虛擬機器執行個體，來準備映像 toocapture hello 執行個體的資料磁碟並設定 hello hello 虛擬機器執行個體的大小。
...
也可以在中找到 tooinstall，如何更新和設定 hello Azure PowerShell cmdlet 的詳細的步驟[hello 部署指南的這一章][deployment-guide-4.1]。

客戶經驗到目前為止已 PowerShell (PS) 的確是 hello 功能更強大的工具 toodeploy Vm 和自訂 toocreate hello 部署的 Vm 中的步驟。 Hello 客戶在 Azure 中執行 SAP 執行個體的所有使用 PS cmdlet toosupplement 管理工作它們在 hello Azure 入口網站中，或甚至使用 PS 指令程式以獨佔方式 toomanage Azure 中的部署。 因為 hello Azure 專用指令程式共用 hello 相同的命名慣例為 hello 超過 2000年與 Windows 相關的指令程式，就會以簡單的工作 Windows 系統管理員 tooleverage 這些指令程式。
...
部署的 hello Azure Monitoring Extension for SAP (請參閱第章[適用於 SAP 的 Azure 監視解決方案][ planning-guide-9.1]本文件中) 僅能透過 PowerShell 或 CLI。 因此，它是強制性 toosetup 並設定 PowerShell 或 CLI 時部署或管理在 Azure 中的 SAP NetWeaver 系統。  

隨著 Azure 提供更多的功能，toobe 加入需要 hello 指令程式更新將新增 PS 指令程式。 因此它使意義 toocheck hello 下載 Azure 站台至少 hello 月一次<https://azure.microsoft.com/downloads/> hello cmdlet 的新版本。 hello 新版本會直接安裝而覆蓋 hello 較舊的版本。
...
也請參閱第章[適用於 Linux Vm 的 Azure CLI] [ deployment-guide-4.5.2]在 hello[部署指南 》] [ planning-guide]上 toouse Azure CLI toodeploy hello Azure 的方式適用於 SAP 的監視功能延伸模組。

## <a name="different-ways-toodeploy-vms-for-sap-in-azure"></a>不同的方式 toodeploy Azure 中 sap 的 Vm
在本章中，您將學習 hello 不同的方式 toodeploy Azure 中的 VM。 本章涵蓋其他準備程序，以及在 Azure 中處理 VHD 和 VM 的方式。

### <a name="deployment-of-vms-for-sap"></a>為 SAP 部署 VM
Microsoft Azure 提供的多種 toodeploy Vm 和相關聯的磁碟。 因此是非常重要的 toounderstand hello 差異因為 hello Vm 的準備工作可能會根據部署的 hello 方法不同。 一般情況下，我們將看看下列案例的 hello:

#### <a name="4d175f1b-7353-4137-9d2f-817683c26e53"></a>將 VM 從內部部署 tooAzure 使用非一般化磁碟
您計劃 toomove 特定 SAP 系統從內部部署 tooAzure。 這可藉由來上傳 VHD，含 hello OS hello、 hello SAP 二進位檔和 DBMS 二進位檔，再加上 hello Vhd hello 資料和記錄檔的 hello DBMS tooAzure。 相較之下太[案例 #2 下方][planning-guide-5.1.2]、 保留 hello 主機名稱、 SAP SID 和 SAP 使用者帳戶在 hello Azure VM，為 hello 在內部部署環境中設定。 因此，將一般化 hello 映像不需要。 請參閱章節[將 VM 從內部部署 tooAzure 使用非一般化磁碟準備][ planning-guide-5.2.1]在內部部署準備步驟及非一般化 Vm 或 Vhd 上的傳此文件tooAzure。 請參閱第章[案例 3： 將 VM 從內部部署使用非一般化 Azure VHD 和 SAP] [ deployment-guide-3.4]在 hello[部署指南 》] [ deployment-guide]如部署在 Azure 中的這類映像的詳細步驟。

#### <a name="e18f7839-c0e2-4385-b1e6-4538453a285c"></a>使用客戶特定的映像部署 VM
因為 OS 或 DBMS 版本 toospecific 修補程式需求，提供 hello hello Azure Marketplace 中的映像可能不適合您的需求。 因此，您可能需要 toocreate 使用您自己 'private' 的 OS/DBMS VM 映像之後數次可部署的 VM。 tooprepare 這類的 'private' 映像進行複製，hello 下列項目有 toobe 考量：
...
> ![Windows][Logo_Windows] Windows
>
> Windows 設定 （例如 Windows SID 和主機名稱） 必須是上摘錄/一般化 hello hello 內部 VM 透過 hello sysprep 命令。
>
>
> ![Linux][Logo_Linux] Linux
>
> 請依照中的文章所述的 hello 步驟[SUSE] [ virtual-machines-linux-create-upload-vhd-suse]或[Red Hat] [ virtual-machines-linux-redhat-create-upload-vhd] tooprepare VHD toobe 上傳 tooAzure。
>
>

- - -
如果您已在內部部署 VM （特別是 2 層系統） 中安裝 SAP 內容，hello 部署的 hello Azure VM 透過 hello 執行個體重新命名程序支援 hello SAP 軟體佈建之後，您可以調整 hello SAP 系統設定管理員 (SAP 附註[1619720])。 請參閱章節[準備使用部署 VM 客戶特定映像為 SAP] [ planning-guide-5.2.2]和[上傳 VHD 從內部部署 tooAzure] [ planning-guide-5.3.2]在內部部署準備步驟及將一般化 VM tooAzure 上的傳此文件。 請參閱第章[案例 2： 使用部署 VM 的自訂映像為 SAP] [ deployment-guide-3.3]在 hello[部署指南 》] [ deployment-guide]詳細步驟部署在 Azure 中的這類映像。

#### <a name="deploying-a-vm-out-of-hello-azure-marketplace"></a>部署 hello Azure Marketplace 中的 VM
您想要 toouse Microsoft 或第 3 個合作對象提供的 VM 映像 hello Azure Marketplace toodeploy 從您的 VM。 部署在 Azure VM 之後，請依照 hello 相同的指導方針和工具 tooinstall hello SAP 軟體和 （或) 在 VM 內的 DBMS 像您一樣在內部部署環境中。 如需詳細的部署說明，請參閱章節[案例 1： 部署 hello 適用於 SAP 的 Azure Marketplace 中的 VM] [ deployment-guide-3.2]在 hello[部署指南 》] [deployment-guide].
...
* 掛接的 toohello VM 且應該固定 VHD 格式以及 Azure toohello VM 需要 toobe 中再次掛接的 Vhd。 hello 的 hello OS 磁碟相同的大小限制適用於以及 toodata 磁碟。 VHD 可以有 1TB 的大小上限。 當您上傳與 PowerShell 指令程式或 CLI hello VHD 動態 Vhd 將會轉換的 toostatic Vhd
* 新增可使用具有系統管理員權限可供 Microsoft 支援服務或指派做為服務和應用程式中的 toorun 環境直到 VM 部署的 hello 與更適當的使用者的本機帳戶。
* 使用僅限雲端的部署案例的 hello 案例 (請參閱第章[-僅限雲端的虛擬機器部署到 Azure 無相依性 hello 內部部署客戶網路][ planning-guide-2.1]這個文件） 搭配使用此部署方法，在 Azure 中部署 hello Azure 磁碟之後，帳戶可能無法運作的網域。 這特別適用於使用的 toorun 如同 hello DBMS 或 SAP 應用程式的服務帳戶。 因此您需要 tooreplace 這類 VM 本機帳戶的網域帳戶，並刪除 hello VM 中的 hello 在內部部署網域帳戶。 保留在內部部署網域使用者 hello VM 映像中不是問題章節中所述 hello VM 部署在 hello 跨內部部署案例時[跨單位-部署的單一或多個 SAP Vm 至 Azure 的 hello 需求完全整合到 hello 與內部網路][ planning-guide-2.2]本文件中。
...
> 確定不會使用 /mnt/resource，並透過 uuid 掛接所有磁碟。 Hello OS 磁碟確定 hello 開機載入器項目也會反映 hello uuid 型掛接。
>
>

- - -
#### <a name="57f32b1c-0cba-4e57-ab6e-c39fe22b6ec3"></a>準備使用客戶特定的映像為 SAP 部署 VM
含有一般化 OS 的 VHD 檔案也會儲存在 Azure 儲存體帳戶的容器中。 您可以藉由參考做為來源 VHD 在您的部署範本檔案中的 hello VHD 章節中所述部署新的 VM，從這類映像 VHD[案例 2： 使用部署 VM 的自訂映像為 SAP] [ deployment-guide-3.3]的 hello[部署指南 》][deployment-guide]。
...
* 其他軟體可以安裝必要 toorun hello Vm 成功跨內部部署案例中的，只要此軟體可與 hello 重新命名的 hello VM。
...
> ![Windows][Logo_Windows] Windows
>
> hello 最後一個步驟是 toolog tooa VM 以系統管理員帳戶中。 以「系統管理員」身分開啟 Windows 命令視窗。 移 too...\windows\system32\sysprep and execute sysprep.exe。
...
> 如果您想 tooperform hello 程序已在 Azure 中執行 vm 時，請依照下列中所述的 hello 步驟[本文][virtual-machines-windows-capture-image]。
>
> ![Linux][Logo_Linux] Linux
>
> [如何為資源管理員範本 Linux 虛擬機器 toouse toocapture][virtual-machines-linux-capture-image]
>
>
...
第三個替代會 toouse 協力廠商的各種 GUI 導向工具。 不過，請確定這些工具支援 Azure 分頁 Blob。 針對我們的目的我們需要 toouse Azure 分頁 Blob 儲存 (hello 差異如下所示： <https://msdn.microsoft.com/library/windowsazure/ee691964.aspx>)。 也提供 Azure 的 hello 工具可以非常有效率地壓縮 hello Vm 和 Vhd 所需要 toobe 上傳。 這是很重要，因為此壓縮效率可縮短 hello 上傳時間 （hello 上傳連結 toohello 網際網路 hello 在內部部署設備和 hello Azure 部署區域從目標還是而有所不同）。 它是一項公平假設，上載 VM 或 VHD 從歐洲地區 toohello 美國基礎 Azure 資料中心將會花費超過上傳 hello 相同的 Vm/Vhd toohello 歐洲 Azure 資料中心。
...
* 登入 tooyour 訂用帳戶與*登入 AzureRmAccount*
* 設定的內容，您的訂用帳戶 hello*組 AzureRmContext*及參數訂用帳戶 Id 或訂用帳戶名稱-請參閱<https://msdn.microsoft.com/library/mt619263.aspx>
* 上傳 hello 與 VHD*新增 AzureRmVhd* tooan Azure 儲存體帳戶-請參閱<https://msdn.microsoft.com/library/mt603554.aspx>
* 新的 VM 組態 toohello VHD 組 hello OS 磁碟與*組 AzureRmVMOSDisk* -請參閱<https://msdn.microsoft.com/library/mt603746.aspx>
* 從 hello VM 組態，以建立新的 VM*新增 AzureRmVM* -請參閱<https://msdn.microsoft.com/library/mt603754.aspx>
* 新增資料磁碟 tooa 與新的 VM*新增 AzureRmVMDataDisk* -請參閱<https://msdn.microsoft.com/library/mt603673.aspx>

**Azure CLI**

* 使用交換器 tooAzure Resource Manager 模式*azure 組態模式 arm*
* 登入 tooyour 訂用帳戶與*azure 登入*
* 使用 *azure account set `<subscription name or id`>* 來選取您的訂用帳戶
* 上傳 hello 與 VHD *azure 儲存體 blob 上傳*-請參閱[使用 hello 與 Azure 儲存體的 Azure CLI][storage-azure-cli]
* 建立新的 VM 指定 hello 當做作業系統磁碟上傳 VHD *azure vm 建立*和-d 參數
* 新增資料磁碟 tooa 與新的 VM *vm 磁碟附加-新增*

**範本**

* 上傳 VHD 使用 Powershell 或 Azure CLI hello
* Hello VM 部署使用 JSON 範本中所示，參考 hello VHD[此範例 JSON 範本](https://raw.githubusercontent.com/Azure/azure-quickstart-templates/master/201-vm-from-specialized-vhd/azuredeploy.json)。

#### <a name="deployment-of-a-vm-image"></a>VM 映像的部署
現有的 VM 或 VHD hello 從內部網路中的順序 toouse 它做為 Azure VM 映像這類 VM 或 VHD 是否需要 toomeet hello 需求章節中所列的 tooupload[準備使用部署 VM 客戶特定映像為 SAP][ planning-guide-5.2.2]的這份文件。

* 使用*sysprep* Windows 上或*waagent-取消佈建*上 Linux toogeneralize VM-請參閱[toocapture hello 資源管理員部署中的 Windows 虛擬機器建立的模型] [ virtual-machines-windows-capture-image-prepare-the-vm-for-image-capture]或[如何 toocapture Linux 虛擬機器 toouse 做為資源管理員範本][virtual-machines-linux-capture-image-capture]
* 登入 tooyour 訂用帳戶與*登入 AzureRmAccount*
* 設定的內容，您的訂用帳戶 hello*組 AzureRmContext*及參數訂用帳戶 Id 或訂用帳戶名稱-請參閱<https://msdn.microsoft.com/library/mt619263.aspx>
* 上傳 hello 與 VHD*新增 AzureRmVhd* tooan Azure 儲存體帳戶-請參閱<https://msdn.microsoft.com/library/mt603554.aspx>
* 新的 VM 組態 toohello VHD 組 hello OS 磁碟與*組 AzureRmVMOSDisk SourceImageUri-CreateOption fromImage* -請參閱<https://msdn.microsoft.com/library/mt603746.aspx>
* 從 hello VM 組態，以建立新的 VM*新增 AzureRmVM* -請參閱<https://msdn.microsoft.com/library/mt603754.aspx>

**Azure CLI**

* 使用*sysprep* Windows 上或*waagent-取消佈建*上 Linux toogeneralize VM-請參閱[toocapture hello 資源管理員部署中的 Windows 虛擬機器建立的模型] [ virtual-machines-windows-capture-image-prepare-the-vm-for-image-capture]或[如何 toocapture Resource Manager 範本為 Linux 虛擬機器 toouse] [ virtual-machines-linux-capture-image-capture] for Linux
* 使用交換器 tooAzure Resource Manager 模式*azure 組態模式 arm*
* 登入 tooyour 訂用帳戶與*azure 登入*
* 使用 *azure account set `<subscription name or id`>* 來選取您的訂用帳戶
* 上傳 hello 與 VHD *azure 儲存體 blob 上傳*-請參閱[使用 hello 與 Azure 儲存體的 Azure CLI][storage-azure-cli]
* 建立新的 VM 指定 hello 當做作業系統磁碟上傳 VHD *azure vm 建立*和參數-Q

**範本**

* 使用*sysprep* Windows 上或*waagent-取消佈建*上 Linux toogeneralize VM-請參閱[toocapture hello 資源管理員部署中的 Windows 虛擬機器建立的模型] [ virtual-machines-windows-capture-image-prepare-the-vm-for-image-capture]或[如何 toocapture Resource Manager 範本為 Linux 虛擬機器 toouse] [ virtual-machines-linux-capture-image-capture] for Linux
* 上傳 VHD 使用 Powershell 或 Azure CLI hello
* Hello VM 部署使用 JSON 範本中所示，參考 hello 映像 VHD[此範例 JSON 範本](https://raw.githubusercontent.com/Azure/azure-quickstart-templates/master/101-vm-from-user-image/azuredeploy.json)。

#### <a name="downloading-vhds-tooon-premises"></a>下載 Vhd tooon 內部
Azure 基礎結構即服務不是單向的僅能 tooupload Vhd 和 SAP 系統。 您可以移動 SAP 系統從 Azure 送回 hello 內部世界。

Hello 下載的 hello 期間 hello Vhd 無法啟動。 即使是下載掛接的 tooVMs 的 Vhd，hello VM 需要 toobe 關機。 如果您只想 toodownload hello 資料庫內容的則應該使用新的系統 tooset 內部，而且如果可接受的 hello hello 階段期間下載及 hello hello hello 系統在 Azure 中的新系統的安裝程式可以依然可運作您無法執行壓縮的資料庫備份到 VHD 來避免長的停機時間，並且只下載 VHD，而不必同時下載 hello OS 基本 VM。

#### <a name="powershell"></a>Powershell
一旦 hello SAP 系統停止而且 hello VM 關閉時，您可以使用在 hello hello PowerShell 指令程式儲存 AzureRmVhd 內部目標 toodownload hello VHD 磁碟回到 toohello 內部世界。 為了讓 toodo，您需要的 hello VHD，您可以在 hello ' 儲存體區段 中找到的 hello URL 的 hello Azure 入口網站 （需要 toonavigate toohello 儲存體帳戶和 hello 儲存體 hello VHD 已建立的容器），而且您必須的 tooknow hello VHD 應該儲存複製到。
...
如需詳細的 hello 儲存 AzureRmVhd cmdlet 的詳細資訊，請參閱以下<https://msdn.microsoft.com/library/mt622705.aspx>。

#### <a name="cli"></a>CLI
一旦 hello SAP 系統停止而且 hello VM 關閉時，您可以使用 hello Azure CLI 命令 azure 儲存體 blob 下載 hello 內部目標 toodownload hello VHD 磁碟回到 toohello 內部世界。 為了讓 toodo，您需要 hello 名稱及 hello 容器的 hello VHD，您可以在 hello ' 儲存體區段 中找到的 hello Azure 入口網站 （需要 toonavigate toohello 儲存體帳戶和 hello 儲存體 hello VHD 已建立的容器），而且您必須 tooknow 位置hello VHD 應該複製到。
...
SAP 系統或甚至專用的 DBMS 伺服器支援 SAP 應用程式層會可能包含數個 Vhd，其中包含可能是 OS hello 與 hello 二進位檔或 hello 資料和記錄檔案中的 hello SAP 資料庫。 Hello Azure 的複製 Vhd 功能都 hello 儲存 Vhd toodisk 已同步處理機制可快照多個 Vhd 同步。 因此，hello hello 複製或狀態儲存 Vhd，即使它們都已掛接 hello 針對相同的 VM 會有不同。 這表示，在 hello 具體的案例中具有不同的資料和記錄中包含 hello 不同 Vhd、 hello hello 結尾中的資料庫會是不一致。

**結論： 順序 toocopy] 或 [儲存 Vhd 的一部分的 SAP 系統設定您需要 toostop hello SAP 系統，而且也需要向 hello tooshut 部署 VM。只有然後您可以複製或下載 Vhd tooeither hello 組建立在 Azure 或內部部署的 hello SAP 系統的複本。**

資料磁碟會儲存為 Azure 儲存體帳戶中的 VHD 檔案，而且可以是直接附加 tooa 虛擬機器或作為映像。 在此情況下，hello VHD 都是複製的 tooanother 位置之前 beeing 附加 toohello 虛擬機器。 hello Azure 中的 hello VHD 檔案的完整名稱必須是在 Azure 中獨一無二的。 已經稍早所述，hello 名稱是種類型的三部分名稱所示：

    http(s)://<storage account name>.blob.core.windows.net/<container name>/<vhd name>

##### <a name="powershell"></a>Powershell
中所示，您可以使用 Azure PowerShell cmdlet toocopy VHD[本文][storage-powershell-guide-full-copy-vhd]。

##### <a name="cli"></a>CLI
中所示，您可以使用 Azure CLI toocopy VHD[這篇文章][storage-azure-cli-copy-blobs]
...
* 建立 hello 來源儲存體帳戶的儲存體帳戶內容*新增 AzureStorageContext* -請參閱<https://msdn.microsoft.com/library/dn806380.aspx>
* 建立 hello 目標儲存體帳戶的儲存體帳戶內容*新增 AzureStorageContext* -請參閱<https://msdn.microsoft.com/library/dn806380.aspx>
* 開始使用 hello 複製
...
在理想情況下 hello 處理 VM 的 hello 結構和相關聯的 hello Vhd 應該非常簡單。 在內部部署安裝中，客戶開發了許多方法來結構化伺服器安裝。

* 有一個基底 VHD 包含 hello OS 和所有 hello 二進位檔的 hello DBMS 和/或 SAP。 自 2015 年 3 月，此 VHD 可以是總大小，而不是受限制的舊版限制 too1TB too127GB。
* 一個或多個 Vhd 包含 hello SAP 資料庫 hello DBMS 記錄檔，以及 hello hello DBMS 暫存儲存區記錄檔 （如果 hello DBMS 支援）。 如果 hello 資料庫記錄 IOPS 需求很高，您需要 toostripe 順序 tooreach hello IOPS 磁碟區所需的多個 Vhd。
* Vhd 包含 hello SAP 資料庫的一個或兩個資料庫檔案和 hello DBMS 暫存資料檔以及 （如果 hello DBMS 支援這） 數目。
...
- - -
hello 使用 hello DBMS 資料檔案和 hello 型別，這些 Vhd 位於 Azure 儲存體的 Vhd 數目應該取決 hello IOPS 需求和所需的 hello 延遲。 如需確切配額的說明，請參閱[這篇文章][virtual-machines-sizes]

SAP 部署透過 hello 過去 2 年內的經驗過關部分課程其中可以摘要如下：

* IOPS 流量 toodifferent 資料檔案不一定相同因為現有的客戶系統可能會以不同的方式調整資料檔案代表其 SAP 資料庫的 hello。 因此事實上 toobe 更多個 Vhd tooplace hello 資料檔案超出所規劃 Lun 使用 RAID 組態。 有個情況下，特別是在 Azure 標準儲存體 IOPS 速率要叫用 hello hello DBMS 交易記錄檔針對單一 vhd 的配額。 建議在 hello 這類案例使用 Premium 儲存體或或者彙總多個標準儲存體 Vhd 的軟體 RAID。
...
記住該 hello VHD 包含作業系統、 hello 和建議，因為 hello SAP 和 hello 資料庫 (基本 VM) 的二進位檔，已不再限制的 too127GB。 它現在最多可以有 too1TB 的大小。 這應該是足夠空間 tookeep 所有 hello 包括例如 SAP 批次工作記錄所需的檔案。

如需其他建議和詳細資訊，特別是關於 DBMS Vm，請參閱 hello[的 DBMS 部署指南][dbms-guide]

#### <a name="disk-handling"></a>磁碟處理
在大部分情況下您需要 hello VM toocreate 順序 toodeploy hello SAP 資料庫中的其他磁碟。 我們已討論 hello 考量章節中的 Vhd 數目上[SAP 部署的 VM/VHD 結構][ planning-guide-5.5.1]的這份文件。 hello Azure 入口網站可讓 tooattach 和部署基本 VM 之後，中斷連接磁碟。 hello 磁碟可以附加/卸離 hello VM 已啟動，以及停止時執行。 附加磁碟時, hello Azure 入口網站提供 tooattach 空的磁碟或現有的磁碟不在此時間點是附加 tooanother VM。

**請注意**: Vhd 在任何給定時間只能附加的 tooone VM。

![連接/中斷連接 Azure 標準儲存體的磁碟][planning-guide-figure-1400]

您是否想 toocreate 新的空白 VHD （這會建立在相同的儲存體帳戶為 hello 基本 VM 處於的 hello） 或您是否要 tooselect，先前上傳也應該將現有的 VHD 附加 toohello VM 現在，您會需要 toodecide。
...
> ![Windows][Logo_Windows] Windows
>
> [Tooattach 資料 hello Azure 入口網站中的磁碟][virtual-machines-windows-attach-disk-portal]
>
> 如果已連接磁碟，您需要在 toolog hello VM tooopen hello Windows 磁碟管理員。 如果依照章節中的建議未啟用自動掛接[設定自動掛接，如附加磁碟][planning-guide-5.5.3]，hello 新附加的磁碟區需要 toobe 採取上線和初始化。
>
> ![Linux][Logo_Linux] Linux
>
> 如果已連接磁碟，您需要 hello VM toolog 中的並初始化 hello 磁碟中所述[這篇文章][virtual-machines-linux-how-to-attach-disk-how-to-initialize-a-new-data-disk-in-linux]
>
>
...
>
> 應該以系統管理員身分開啟 hello Windows 命令列視窗。
>
> 如果已連接磁碟，您需要在 toolog hello VM tooopen hello Windows 磁碟管理員。 如果依照章節中的建議未啟用自動掛接[設定自動掛接，如附加磁碟][planning-guide-5.5.3]，hello 新連接的磁碟區 > 需要 toobe 採取上線和初始化。
>
> ![Linux][Logo_Linux] Linux
>
> 中所述，您會需要新附加空磁碟 tooinitialize[本文][virtual-machines-linux-how-to-attach-disk-how-to-initialize-a-new-data-disk-in-linux]。
...
> * 您可以在 hello 的 hello 精靈的最後一個步驟，需要 toogive hello 規則的名稱，然後按 [完成] 儲存 hello 規則
>
> hello 規則會立即生效。
>
...
在此案例中 (請參閱第章[僅限雲端][ planning-guide-2.1]的這份文件)，我們實作典型的訓練/示範系統案例，其中 hello 整個訓練/示範案例所包含之單一 VM 內。 我們假設 hello 部署透過 VM 映像範本完成。 我們也假設其中多個以 hello 擁有 hello 的 Vm 部署這些示範/訓練 Vm 需要 toobe 相同名稱。
...
#### <a name="setup-network-for-communication-between-hello-different-vms"></a>安裝網路之間的通訊 hello 不同的 Vm
...
您執行 SAP 環境，並希望 toodivide hello 部署之間高階 DBMS 伺服器的裸機、 應用程式層級和較小的 2 層在內部部署虛擬化環境設定 SAP 系統和 Azure IaaS。 hello 基本假設是一個 SAP 環境內的 SAP 系統需要 toocommunicate 與彼此、 與許多其他 hello 家公司的何種部署形式獨立部署的軟體元件。 也應該有任何差異 hello 部署形式 hello 終端使用者使用 SAP GUI 或其他介面連接。 我們有時，可以只符合這些條件 hello 內部部署 Active Directory/OpenLDAP 和 DNS 服務延伸 toohello 透過站台-到-站台/多站台連線能力或私人連線，例如 Azure ExpressRoute 的 Azure 系統。
...
hello 如上所示的案例描述一種案例其中 hello 內部部署 AD/OpenLDAP 和 DNS 延伸 tooAzure。 在 hello 內部端，每個 Azure 訂用帳戶保留特定 IP 位址範圍。 hello IP 位址範圍會指派 hello Azure 端上的 tooan Azure 虛擬網路。
...
> * 只要遵循的 hello 公用 Linux 指南[SUSE](https://www.suse.com/documentation/sles-12/book_sle_deployment/data/sec_y2_hw_print.html)或[Red Hat](https://access.redhat.com/documentation/en-US/Red_Hat_Enterprise_Linux/6/html/Deployment_Guide/sec-Printer_Configuration.html)如何 tooadd 印表機。
>
>
...
>
> 共用您的本機印表機。
> Hello Azure VM 中開啟 hello Windows 檔案總管 並輸入 hello 共用名稱中的 hello 印表機。
...
在 站對站連線的跨內部部署案例，在內部部署與 Azure 之間的 hello 延遲仍然很嚴重。 如果我們依照傳輸物件經由開發和測試系統 tooproduction hello 順序，或考慮套用傳輸或支援封裝 toohello 不同系統，您會發現，取決於 hello 中央傳輸 hello 位置hello 系統目錄中，會發生高延遲讀取或寫入 hello 中央傳輸目錄中的資料。 hello 的情況是類似 tooSAP 橫向設定其中 hello 不同系統分布於相距遙遠 hello 資料中心使用不同資料中心。

在排序 toowork 解決這種延遲，而且有 hello 系統更快讀取或寫入 tooor 從 hello 傳輸目錄，您可以設定兩個 STMS 傳輸網域 （一個用於內部部署。 而另一個為 Azure 和連結 hello 傳輸網域中的 hello 系統 請參閱此文件，其中說明 hello SAP TMS 中此概念背後的 hello 原則： <http://help.sap.com/saphelp_me60/helpdata/en/c4/6045377b52253de10000009b38f889/content.htm?frameset=/en/57/38dd924eb711d182bf0000e829fbfe/frameset.htm>。
...
系統之間的內部部署和 Azure 中的 RFC 流量必須 toowork。 toosetup 連接呼叫交易 SM59 來源系統中，您需要 toodefine hello 目標系統 RFC 連線。 hello 組態是類似的 toohello 標準設定 RFC 連線。
...
自 2014 年 2 月 hello ' Azure VM 代理程式 」，可讓 VM 插入 Windows Vm 預設會在建立 VM hello Azure 入口網站中的 hello 內的特定 Azure VM 延伸模組的處理。 發生 SUSE 或 Red Hat Linux hello VM 代理程式已經是 Azure Marketplace 映像的一部分。 萬一其中一個會將從 Linux VM 上傳內部 tooAzure hello VM 代理程式有 toobe 手動安裝。

hello Azure hello 監控解決方案的基本建置組塊，如 SAP 看起來像這樣：

![Microsoft Azure 擴充功能元件][planning-guide-figure-2400]

Hello 區塊上圖所示，hello 的 SAP 監控解決方案的一個部分會裝載於 hello Azure VM 映像和 Azure 延伸模組庫即由 Azure Operations 管理的全球複寫儲存機制。 它負責 hello hello hello Azure 實作 SAP toowork hello Azure Monitoring Extension for SAP 的 Azure 作業 toopublish 新版本上使用聯合 SAP/MS 小組。 此 Azure Monitoring Extension for SAP 會使用 hello Microsoft Azure 診斷 (WAD) 擴充功能或 Linux Azure 診斷 （年輕人） tooget hello 所需的資訊。
...
Azure 虛擬機器上目前沒有任何單一 VM SLA 可用。 tooget 了解如何在單一 VM 的 hello 可用性看起來就像是只可以建立 hello 產品的 hello 不同可用的 Azure Sla: <https://azure.microsoft.com/support/legal/sla/>。
...
Microsoft Azure 儲存體帳戶中的 hello 資料會一直複寫的 tooensure 耐久性與高可用性，會議 hello Azure 儲存體 SLA，即使在 hello 字體的暫時性硬體失敗

由於 Azure 儲存體而導致 3 映像的 hello 資料依預設，就不需要 RAID5 或 RAID1 跨多個 Azure 磁碟。
...
如果您決定不 toouse 功能類似 Windows Server 容錯移轉叢集 (WSFC) 或 Linux 的對等 (hello 後者一個尚不支援在 Azure SAP 軟體的結合)、 Azure VM 重新啟動是利用的 tooprotect SAP 系統，以防計劃和hello Azure 的實體伺服器基礎結構及整體基礎的 Azure 平台的非計劃性的停機。
...
每個 SAP 執行個體都會在自己的 Azure 儲存體帳戶中 - 當一個 Azure 儲存體可能無法使用時，只會導致一個 VM 及其 SAP AS 執行個體無法使用。 不過請注意，一個 Azure 訂用帳戶中的「Azure 儲存體帳戶」數目有限。 tooensure 自動啟動 (A) SCS 執行個體之後 hello VM 重新開機，請確定 tooset Autostart 參數 (A) SCS 執行個體中的啟動章節所述的設定檔[使用自動啟動 SAP 執行個體][planning-guide-11.5]。
...
tooachieve 完整 SAP 系統高可用性，我們需要 tooprotect 所有關鍵 SAP 系統的元件，例如備援的 SAP 應用程式伺服器，而等唯一元件 （例如單一失敗點） SAP (A) SCS 執行個體和 DBMS。
...
Azure 縮放單位內的 Azure 可用性設定組可使用不限數目的容錯和升級網域。 這表示將放入一個可用性設定組中，多個 VM 最後會送入 hello 相同錯誤或升級網域中的 hello 事實很快的 Vm 數目

在其專用的 Vm 中部署一些 SAP 應用程式伺服器執行個體，假設我們有 5 個升級網域，hello 下列圖片會形成 hello 結束。 hello 未來可能會變更 hello 實際的最大數容錯網域和更新網域內的可用性設定組：
...
* 執行一個 SAP 系統之 (A)SCS 執行個體的所有 VM 都會在一個可用性設定組中。 這些 Vm 的內部是設定 Windows Server 容錯移轉叢集 (WSFC) tooprotect (A) SCS 執行個體。
...
SAP HA 在 Linux on Azure 的 hello 架構基本上是的 hello 與 Windows，做為相同上面所述。 不過截至 2016 年 1 月為止，有兩個限制︰

* Azure 上的 Linux 目前只支援 SAP ASE 16，而不提供任何 ASE 複寫功能。
* Azure 上的 Linux 尚未支援 SAP (A)SCS HA 解決方案

因此 SAP Linux Azure 系統無法達成的 2016 年 1 月 hello 做為 SAP Azure 系統的同一個可用性，因為遺漏 HA hello (A) SCS 執行個體和 hello 單一執行個體 SAP ASE 資料庫。

### <a name="4e165b58-74ca-474f-a7f4-5e695a93204f"></a>對 SAP 執行個體使用自動啟動
SAP 提供 hello OS hello VM 中的 hello 開頭之後立即 hello 功能 toostart SAP 執行個體。 hello 確切步驟已記載於 SAP 知識庫文章[1909114] -如何使用 Autostart 參數來自動 toostart SAP 執行個體。 不過，SAP 不建議您使用 toouse hello 設定，再因為 hello 順序的執行個體重新啟動時，沒有控制項假設影響多個 VM 收到或每個 VM 執行的多個執行個體。 假設 Azure 典型案例一個 SAP 應用程式伺服器執行個體的單一 VM，最後會開始重新啟動 VM，hello 案例中，hello Autostart 不是真的很重要，而且可以藉由加入這個參數啟用：
...
相依於 hello SAP 組態選擇 （2 層或 3 層） 有可能是需要 toobackup。 hello 內容 hello VM 本身加上 toohave hello 資料庫的備份。 hello DBMS 相關的備份是以資料庫方法預期的 toobe。 Hello 不同資料庫的詳細的說明位於[DBMS 指南][dbms-guide]。 在 hello 另一方面，hello SAP 資料可以備份採用離線方式 （包括 hello 資料庫內容） 這一節中所述或線上 hello 下一節中所述。

hello 離線備份基本上需要關機的 hello VM 透過 hello Azure 入口網站，並 hello 基底 VM 磁碟的複本及所有附加 Vhd toohello VM。 這會保留 hello VM 時間映像中的點和其相關聯的磁碟。 建議 toocopy hello 「 備份 」 到不同的 Azure 儲存體帳戶。 因此 hello 章節所述的程序[Azure 儲存體帳戶之間複製磁碟][ planning-guide-5.4.2]本文件將會套用。
除了 hello 關機使用 hello 其中一個 Azure 入口網站也可以執行它透過 Powershell 或 CLI 如下所述： <https://azure.microsoft.com/documentation/articles/virtual-machines-deploy-rmtemplates-powershell/>

還原該狀態會組成刪除 hello、 基本 VM 與 hello 原始磁碟的 hello 基底 VM 及掛接的 Vhd 複製後的 hello 儲存 Vhd toohello 原始儲存體帳戶，然後重新部署 hello 系統。
本文示範如何 tooscript 此程序會在 Powershell: <http://www.westerndevs.com/azure-snapshots/>

請先確定 tooinstall 新的 SAP 授權 restoing 上面所述的 VM 備份會建立新的硬體金鑰。

### <a name="online-backup-of-an-sap-system"></a>SAP 系統的線上備份
在 hello 中所述以 DBMS 專用方法執行備份的 hello DBMS [DBMS 指南][dbms-guide]。

Hello SAP 系統內的其他 Vm 可以備份使用 Azure 虛擬機器備份功能。 Azure 虛擬機器備份引進早期 2015年中，並同時是標準方法 toobackup 完成 VM 在 Azure 中。 Azure 備份儲存在 Azure 中的 hello 備份，並允許虛擬機器還原一次。
...
> ![Windows][Logo_Windows] Windows
>
> 理論上執行的資料庫可以備份以一致的方式以及如果 hello DBMS 系統支援 hello Windows VSS 的 Vm (磁碟區陰影複製服務<https://msdn.microsoft.com/library/windows/desktop/bb968832 (vs.85> ) 一樣例如 SQL Server。
> 不過請注意，您無法根據 Azure VM 備份還原時間點來還原資料庫。 因此，建議使用 DBMS 功能，而不是依賴 Azure VM 備份的資料庫的 tooperform 備份
>
> 熟悉 Azure 虛擬機器備份 tooget 請從這裡開始： <https://azure.microsoft.com/documentation/articles/backup-azure-vms/>。
>
> 其他可能性是 toouse 組合 Microsoft Data Protection Manager 安裝在 Azure VM，Azure Backup 備份/還原資料庫。 如需詳細資訊，請參閱：<https://azure.microsoft.com/documentation/articles/backup-azure-dpm-introduction/>。  
>
> ![Linux][Logo_Linux] Linux
>
> 沒有任何對等的 tooWindows VSS 在 Linux 中。 因此，您只能進行檔案一致備份，而無法進行應用程式一致備份。 hello SAP DBMS 備份應該使用 DBMS 功能。 hello 檔案系統，其中包括 hello SAP 相關可以儲存資料，例如使用 tar 檔案解壓縮述此處： <http://help.sap.com/saphelp_nw70ehp2/helpdata/en/d3/c0da3ccbb04d35b186041ba6ac301f/content.htm>
>
>
...
* 備份 hello VM 所在的 hello SAP 系統，並與其 hello 全域目錄 hello 不同執行個體的所有 hello 設定檔就很重要，應該使用 Windows 備份執行或例如 tar 檔案解壓縮在 Linux 上。 由於 Windows Server 2008 (R2) 和 Windows Server 2012 (R2)，使其更容易 toobackup 之間有差異使用較新的 hello Windows Server 版本，我們建議 toorun 做為 Windows 客體作業系統的 Windows Server 2012 (R2)。
