...
當您建立雲端服務時，Azure 將其指派 tooa cloudapp.net 子網域。 例如，如果您的雲端服務名稱為"contoso"，您的使用者將會無法 tooaccess http://contoso.cloudapp.net 之類的 URL 上的應用程式。 Azure 也會指派虛擬 IP 位址。
...
> [!NOTE]
> 加快腳步！ 使用 hello Azure[引導式逐步解說](http://support.microsoft.com/kb/2990804)。 在彈指之間完成自訂網域名稱的關聯，以及與 Azure 雲端服務或 Azure 網站之間的通訊 (SSL) 保護。
> 
> 
...
1. 使用其中一個這些方法 toofind hello **。.cloudapp.net**指派 tooyour 雲端服務的網域名稱。
   ...
   * 登入 toohello [Azure 傳統入口網站]，選取您的雲端服務，選取**儀表板**，然後尋找 hello**網站 URL** hello 中的項目**快速概覽** > 一節。
...
1. 使用下列方法 tooget hello 的雲端服務的 IP 位址的 hello 之一。
   ...
   * 登入 toohello [Azure 傳統入口網站]，選取您的雲端服務，選取**儀表板**，然後尋找 hello**公用虛擬 IP (VIP) 位址**hello中的項目**快速概覽**> 一節。
   ...
   * 安裝和設定[Azure Powershell](/powershell/azure/overview)，，然後使用 hello 下列命令：
     ...
     如果您有多個與您的雲端服務相關聯的端點，您會收到多行包含 hello IP 位址，但所有應該會顯示 hello 相同位址。
     
     儲存 hello IP 位址，您將需要建立 A 記錄時。
2. 登入 tooyour DNS 註冊機構的網站，並移至 toohello 管理 DNS 的頁面。 查詢的連結或 hello 網站的區域標示為**網域名稱**， **DNS**，或**名稱伺服器管理**。
3. 現在找出可選取或輸入 A 記錄的地方。 您可能必須從下拉式清單，輸入或瀏覽 tooan 進階設定 頁面上，tooselect hello 記錄。
4. 選取或輸入 hello 網域或將使用此 A 記錄的子網域。 例如，選取**www**如果您想 toocreate 別名**www.customdomain.com**。如果您想要所有子網域 toocreate 萬用字元項目，請輸入 '__*__'。 這將會涵蓋所有子網域，例如 **mail.customdomain.com**、**login.customdomain.com** 和 **www.customdomain.com**。
...
這個範例將示範如何建立 hello 根網域的 A 記錄。 如果您想 toocreate 萬用字元項目 toocover 所有子網域，您會輸入 '__*__' 為 hello 子網域。
