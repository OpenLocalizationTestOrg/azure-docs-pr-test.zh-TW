...
更新 0.5 會對應 toohello 軟體版本**10.0.10290.0**。

> [!NOTE]
> 更新是干擾性作業，會重新啟動您的裝置。 如果正在進行中的 I/O，hello 裝置會產生停機時間。 如需如何 tooapply hello 更新的詳細指示，請移過[安裝更新 0.5](storsimple-virtual-array-install-update-05.md)。


## <a name="whats-new-in-hello-update-05"></a>新功能更新 0.5 hello
Update 0.5 主要是錯誤修正組建。 hello 主要增強功能和 bug 修正如下：

- **備份復原改善**-此版本中已改善 hello 備份復原的修正程式。 在 hello 舊版本中，備份淘汰僅適用於某些例外狀況。 此版本會重試所有 hello 備份例外狀況，並讓 hello 更有彈性的備份。

- **更新儲存體使用量監視**-從開始 30 年 6 月 2017年，hello 儲存體使用量監視 StorSimple 虛擬裝置系列將已淘汰。 這適用於 toohello 監控圖表的所有執行更新，則只需 0.4 或更低的 hello 虛擬陣列。 此更新包含針對您 toocontinue hello 使用的存放裝置使用量監視 hello 在 Azure 入口網站所需的 hello 變更。 安裝此 2017 年 6 月 30 之前, 的重大更新 toocontinue 使用 hello 監視功能。


## <a name="issues-fixed-in-hello-update-05"></a>Hello 0.5 更新中修正的問題

hello 下表提供此版本中修正問題的摘要。

| 否。 | 功能 | 問題 |
| --- | --- | --- |
| 1 |備份復原功能| 在 hello 舊版本中，備份淘汰僅適用於某些例外狀況。 此版本包含修正 toomake 備份更有彈性，重試一次所有 hello 備份例外狀況。|
| 2 |監視| hello 儲存體使用量的監視 StorSimple 虛擬裝置系列會遭取代啟動 2017 年 6 月 30。 這個動作會影響 hello 監控圖表上 StorSimple 虛擬陣列上執行的 hello StorSimple 裝置管理員服務 （1200年模型）。 此版本中有更新，可允許 hello 使用者 toocontinue hello 使用 hello 2017 年 6 月 30，超出的虛擬陣列上的儲存體使用量監視。|
| 3 |檔案伺服器| 在 hello 舊版本中，使用者不小心將複製加密的檔案 toohello 虛擬陣列。 這個版本包含的修正程式，不會讓加密的檔案 toovirtual 陣列的複製。 如果您的裝置有更新的現有加密的檔案先前 toohello，備份將繼續 toofail，直到從 hello 系統刪除所有加密的 hello 檔案。 |


## <a name="known-issues-in-hello-update-05"></a>Hello 更新 0.5 的已知的問題
