...
本教學課程描述 hello 步驟，您應該遵循 tooinstall 並執行 Windows Server 2012 R2 和連線的 tooa StorSimple 實體裝置的主機上使用 hello 多重路徑 I/O (MPIO) 功能。 這篇文章中的 hello 指導方針適用於 tooStorSimple 8000 系列實體裝置。 StorSimple 雲端設備目前不支援 MPIO。

建置在 Windows Server toohelp hello 多重路徑 I/O (MPIO) 功能的支援的 Microsoft 建置高可用性、 容錯的 SAN 組態。 MPIO 使用備援實體路徑元件，配接器、 纜線和交換器 — toocreate hello 伺服器與 hello 存放裝置之間的邏輯路徑。 如果沒有元件故障，造成邏輯路徑 toofail，則多重路徑邏輯會使用替代路徑 i/o，讓應用程式仍然可以存取其資料。 此外您的組態，根據 MPIO 也可以改善效能所重新 hello 負載平衡跨所有這些路徑。 如需詳細資訊，請參閱 [MPIO 概觀](https://technet.microsoft.com/library/cc725907.aspx "MPIO 概觀 and features")。
...
請遵循這些步驟 tooconfigure MPIO StorSimple 裝置上：
...
3. 在 [hello**新增角色及功能**精靈] 中，執行下列步驟的 hello:
   
   1. 在 hello**開始之前**頁面上，按一下**下一步**。
   2. 在 hello**選取安裝類型**頁面上，接受預設設定 hello**角色型或功能型**安裝。 按一下 [下一步] 。
   
       ![新增角色及功能精靈 2](./media/storsimple-configure-mpio-windows-server/IC740999.png)
   3. 在 hello**選取目的地伺服器**頁面上，選擇**從 hello 伺服器集區選取伺服器**。 應該會自動探索主機伺服器。 按一下 [下一步] 。
   4. 在 hello**選取伺服器角色**頁面上，按一下**下一步**。
   5. 在 hello**選取功能**頁面上，選取**多重路徑 I/O**，然後按一下**下一步**。
   
       ![新增角色及功能精靈 5](./media/storsimple-configure-mpio-windows-server/IC741000.png)
   6. 在 hello**確認安裝選項**頁面上，確認 hello 選項，然後選取**必要時自動重新啟動 hello 目的地伺服器**，如下所示。 按一下 [Install] 。
   
       ![新增角色及功能精靈 8](./media/storsimple-configure-mpio-windows-server/IC741001.png)
   7. Hello 安裝完成時，會通知您。 按一下**關閉**tooclose hello 精靈。
...
MPIO 必須設定的 tooidentify StorSimple 磁碟區。 tooconfigure MPIO toorecognize StorSimple 磁碟區，執行下列步驟的 hello。
...
6. 在 hello**新增 MPIO 支援**對話方塊的 **裝置硬體識別碼**，輸入您裝置的序號。 tooget hello 裝置的序列數字，存取您的 StorSimple 裝置管理員服務。 瀏覽過**裝置 > 儀表板**。 hello 裝置序號會顯示在右 hello**快速概覽**hello 裝置儀表板的窗格。
    </br>
...
3. 在 hello**探索目標入口網站**對話方塊方塊中，執行下列步驟的 hello:
   
   1. 輸入您的 StorSimple 裝置的資料連接埠 hello hello IP 位址 （例如，輸入 DATA 0）。
...
11. 在 hello**進階設定**對話方塊：
    ...
    1. 在 hello**本機介面卡**下拉式清單中，選取 Microsoft iSCSI 啟動器。
    2. 在 hello**啟動器 IP**下拉式清單中，選取 hello IP 位址對應 toohello 主機。 在此情況下，您所連接 hello 裝置 tooa hello 主機上的單一網路介面上的兩個網路介面。 因此，這個介面是 hello 相同的 hello 提供第一個工作階段。
...
12. 開啟**電腦管理**瀏覽過**伺服器管理員 > 儀表板 > 電腦管理**。 在 hello 左窗格中，按一下 **存放裝置 > 磁碟管理**。 hello hello StorSimple 裝置的可見 toothis 主機上建立的磁碟區之下**磁碟管理**為新磁碟。
...
15. 在 [hello StorSimple 型號 # # #**多重路徑磁碟裝置屬性**對話方塊方塊中，按一下 hello **MPIO** ] 索引標籤。
...
hello 下列程序描述如何使用兩個網路介面主機兩個網路介面的 StorSimple 裝置時連接的 tooa 的 tooadd 工作階段。 這只會為您提供 4 個工作階段。 使用這個相同的程序與 StorSimple 裝置與兩個網路介面連接的 tooa 主機具有四個網路介面。 您將需要 tooconfigure 8，而不是 hello 4 此處所述的工作階段。
...
5. 在 hello**連接 tooTarget**對話方塊：
   ...
   1. 保持 hello 預設選取的目標設定**將此連線新增**toohello 我的最愛目標清單。 這使得每次重新啟動此電腦會自動嘗試 toorestart hello 連接 hello 裝置。
   2. 選取 hello**啟用多重路徑**核取方塊。
   3. 按一下 [進階] 。
6. 在 hello**進階設定**對話方塊：
   
   1. 在 hello**本機介面卡**下拉式清單中，選取**Microsoft iSCSI 啟動器**。
   2. 在 hello**啟動器 IP**下拉式清單中，選取 hello hello 主機 IP 位址。
   3. 在 hello**目標入口網站 IP**下拉式清單中，選取 hello hello 的裝置上啟用的 hello 資料介面 IP 位址。
   4. 按一下**確定**tooreturn toohello iSCSI 啟動器屬性 對話方塊。
...
深入了解[使用您的 StorSimple 裝置組態 hello StorSimple 裝置管理員服務 toomodify](storsimple-8000-modify-device-config.md)。
