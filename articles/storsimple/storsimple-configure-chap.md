...
本教學課程說明如何 tooconfigure CHAP 您 StorSimple 裝置。 此文件中詳述的 hello 程序適用於 tooStorSimple 8000 系列以及 StorSimple 1200 裝置。

CHAP 代表 Challenge Handshake 驗證通訊協定。 它是伺服器 toovalidate hello 身分識別的遠端用戶端所使用的驗證配置。 hello 驗證根據共用的密碼或密碼。 CHAP 可以是單向 (單向) 或相互 (雙向)。 單向 CHAP 時，hello 目標驗證啟動器。 相互或反向 CHAP 時，在 hello 相反地，必須先 hello 目標驗證 hello 啟動器，再 hello 啟動器驗證目標 hello。 不需要目標驗證也能實作啟動器驗證。 不過，必須同時實作啟動器驗證，才能實作目標驗證。 
...
1. 在 Azure 傳統入口網站，在 [hello hello**裝置**頁面上，按一下 hello**設定**] 索引標籤。
   
    ![CHAP 啟動器](./media/storsimple-configure-chap/IC740943.png)
2. 向下捲動這個頁面上，然後在 hello **CHAP 啟動器**> 一節：
   ...
   3. 確認 hello 的密碼。
3. 按一下 [儲存] 。 將顯示確認訊息。 按一下**確定**toosave hello 變更。
...
5. 在 [hello**目標**hello] 索引標籤**iSCSI 啟動器屬性**hello 裝置狀態應顯示視窗中，為**已連接**。 如果您使用 StorSimple 1200 裝置，則每個磁碟區將可掛接為 iSCSI 目標，如下所示。 步驟 3-4，因此，需要 toobe 重複每個磁碟區。
   
    ![做為個別目標掛接的磁碟區](./media/storsimple-configure-chap/chap4.png)
   
   > [!IMPORTANT]
   > 如果您變更 hello iSCSI 名稱，則會使用 hello 新名稱，新的 iSCSI 工作階段。 您必須先登出再登入，現有的工作階段才會使用新的設定。
   > 
   > 
...
在雙向驗證 hello 目標驗證啟動器 hello，然後 hello 啟動器驗證目標 hello。 這需要 hello 使用者 tooconfigure hello CHAP 啟動器設定，以及 hello 反向 CHAP 設定 hello 裝置和 iSCSI 啟動器軟體 hello 主機上。 hello 下列程序說明 hello 步驟 tooconfigure 相互驗證 hello 裝置上和 hello Windows 主機上。
...
3. 在 hello **CHAP 啟動器**> 一節：
   ...
   2. 提供裝置的 [密碼]  。
   3. 確認 hello 的密碼。
4. 按一下 [儲存] 。 將顯示確認訊息。 按一下**確定**toosave hello 變更。
...
4. 在 hello **iSCSI 啟動器相互 CHAP 密碼**對話方塊：
   ...
   1. 型別 hello**反向 CHAP 密碼**hello Azure 傳統入口網站中設定。
   2. 按一下 [確定] 。
...
在 hello**連接 tooTarget**對話方塊會呈現，請選取 hello**新增我的最愛目標連線 toohello 清單**核取方塊。 這可確保，每次 hello 電腦重新啟動，就會試著 toorestore hello 連接 toohello iSCSI 我的最愛目標。
