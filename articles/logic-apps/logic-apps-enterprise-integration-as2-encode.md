...
---
# <a name="encode-as2-messages-for-azure-logic-apps-with-hello-enterprise-integration-pack"></a>Azure 邏輯應用程式的編碼 AS2 訊息，以 hello 企業版整合套件

tooestablish 安全性和可靠性時傳輸訊息，使用 hello 編碼的 AS2 訊息連接器。 此連接器提供數位簽章、 加密及通知透過訊息處理通知 (MDN)，這也會 toosupport 使用於不可否認性。
...
* 已經定義並與 Azure 訂用帳戶相關聯的[整合帳戶](logic-apps-enterprise-integration-create-integration-account.md)。 您必須為整合帳戶 toouse hello 編碼的 AS2 訊息連接器。
...
2. 沒有觸發程序，hello 編碼的 AS2 訊息連接器，所以您必須新增啟動邏輯應用程式，像是要求觸發程序的觸發程序。 在 hello 邏輯應用程式的設計工具，加入觸發程序，然後再加入動作 tooyour 邏輯應用程式。

3.  Hello 搜尋方塊中，輸入您的篩選器"AS2"。 選取 [AS2 - 編碼 AS2 訊息]。
