...
1. Hello，下列命令安裝所有封裝。 其中包含 Azure IoT 裝置 SDK、BME280 感應器程式庫和接線 Pi 程式庫。
   ...
   ```
   > [!NOTE] 
   此安裝程序，根據您的網路連線，可能需要幾分鐘的時間 toofinish。
...
1. 執行下列命令的 hello 開啟 hello 設定檔：
   ...
   此檔案中有兩個項目可供您設定。 hello 第一次是`interval`，這兩個訊息，傳送 toocloud 之間定義 hello 時間間隔 （以毫秒為單位）。 hello 第二個`simulatedData`，這是 toouse 是否模擬感應器資料的布林值。
...
藉由執行下列命令的 hello 執行 hello 範例應用程式：
