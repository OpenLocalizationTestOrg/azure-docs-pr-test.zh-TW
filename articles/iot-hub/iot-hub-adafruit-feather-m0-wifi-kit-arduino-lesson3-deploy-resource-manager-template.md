...
如果您有任何問題，尋找解決方案上 hello[疑難排解 Adafruit 羽毛 M0 WiFi Arduino 面板頁面](iot-hub-adafruit-feather-m0-wifi-kit-arduino-troubleshooting.md)。
...
* hello`app.ino`檔案在 hello`app`子資料夾位於 hello 金鑰來源檔案。 這個原始程式檔包含 hello 程式碼 toosend 它所傳送的訊息 20 倍 tooyour IoT 中樞與閃爍 hello LED 為每個訊息。
* hello`config.json`包含必要的組態設定。
...
## <a name="summary"></a>摘要
您已建立您的 Azure 函式應用程式 tooprocess IoT 中樞訊息，與 Azure 儲存體帳戶的 toostore 這些訊息。 您現在可以部署和執行 hello 範例 toosend 裝置到雲端訊息 Arduino 面板上。

## <a name="next-steps"></a>後續步驟
[執行範例應用程式 toosend Arduino 面板上的裝置到雲端訊息][send-device-to-cloud-messages]
