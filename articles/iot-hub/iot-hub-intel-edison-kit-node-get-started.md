...
---
# <a name="connect-intel-edison-tooazure-iot-hub-nodejs"></a>連接 Intel Edison tooAzure IoT 中樞 (Node.js)
...
3. 然後瀏覽下列命令 tooinstall toohello 儲存機制資料夾 toorun hello 所有封裝時，可能需要 serval 分鐘 toocomplete。
...
### <a name="configure-and-run-hello-sample-application"></a>設定及執行 hello 範例應用程式
...
1. 執行下列命令的 hello 開啟 hello 設定檔：
   ...
   此檔案中有兩個巨集可供您設定。 hello 第一次是`INTERVAL`，而後者可定義兩個訊息，傳送 toocloud hello 時間間隔。 hello 第二個`simulatedData`，這是 toouse 是否模擬感應器資料的布林值。
