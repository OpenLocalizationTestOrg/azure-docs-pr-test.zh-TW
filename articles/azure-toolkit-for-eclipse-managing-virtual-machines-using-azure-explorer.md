...
---
# <a name="manage-virtual-machines-by-using-hello-azure-explorer-for-eclipse"></a>使用適用於 Eclipse hello Azure 總管來管理虛擬機器

hello Azure 總管 中，這是 hello Azure Toolkit for Eclipse 的一部分，提供 Java 開發人員容易使用的解決方案來管理從其 Azure 帳戶中的虛擬機器 hello Eclipse 整合式的開發環境 (IDE) 內。
...
4. 在 hello**選取虛擬機器映像**視窗中，輸入下列資訊的 hello:
   ...
   * **發行者**： 指定建立 hello 映像，您將使用 toocreate 您的虛擬機器的 hello 發行者 (例如， *Microsoft*)。
...
8. 在 hello**建立新的儲存體帳戶**視窗中，輸入下列資訊的 hello:

   * **資源群組**： 指定您的虛擬機器的 hello 資源群組。 選取其中一個 hello 下列選項：
      * **建立新**： 指定您想 toocreate 新的資源群組。
      * **使用現有**： 指定您想 tooselect 已經與您的 Azure 帳戶相關聯的資源群組。

      ![hello 建立新的儲存體帳戶對話方塊][CR05]

   * **儲存體帳戶**： 指定 hello 儲存體帳戶 toouse 來儲存您的虛擬機器。 您可以使用現有的儲存體帳戶或建立新帳戶。

   * **虛擬網路**和**子網路**： 指定 hello 虛擬網路與虛擬機器會連接到的子網路。 您可以使用現有的網路和子網路，也可以建立新的網路和子網路。 如果您選取**建立新**，會顯示下列對話方塊中的 hello:

      ![hello 建立新的虛擬網路 對話方塊][CR06]

9. 在 hello**相關聯的資源**視窗中，輸入下列資訊的 hello:
   ...
   * **可用性設定組**︰指定虛擬機器可以加入的選用可用性設定組。 您可以選取現有的可用性設定組或建立新的可用性設定組或，如果您的虛擬機器將不屬於 tooan 可用性設定組，您可以選取**（無）**。

   ![hello 相關聯資源的視窗][CR07]

9. 按一下 [完成] 。  
  新的虛擬機器會顯示 hello Azure 總管工具視窗中。

   ![新虛擬機器][CR08]

## <a name="restart-a-virtual-machine-in-eclipse"></a>在 Eclipse 中重新啟動虛擬機器

toorestart 使用 hello Azure 總管在 Eclipse 中，虛擬機器 hello 遵循：
...
關閉執行中虛擬機器在 Eclipse 中，使用 hello Azure 總管 tooshut hello 遵循：
...
toodelete 使用 hello Azure 總管在 Eclipse 中，虛擬機器 hello 遵循：
