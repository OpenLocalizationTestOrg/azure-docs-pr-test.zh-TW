...
---
# <a name="step-11-run-a-test-failover-of-physical-servers-tooazure"></a>步驟 11： 執行測試容錯移轉的實體伺服器 tooAzure

本文說明如何 toorun 測試容錯移轉從內部部署實體伺服器 tooAzure，使用 hello [Azure Site Recovery](site-recovery-overview.md) hello Azure 入口網站中的服務。
...
執行測試容錯移轉，我們建議您確認 hello 伺服器屬性，並進行任何變更之前，您需要。 您可以存取在 hello VM 屬性**複寫項目**。 hello **Essentials**刀鋒視窗會顯示電腦的設定狀態的資訊。
...
- 當您啟用保護的伺服器時，VM 資料會複寫 tooa 儲存體帳戶。 受管理的磁碟建立並附加 toohello VM，只會容錯移轉發生時。
- 受管理的磁碟只可以建立 Azure Vm 部署使用 hello 資源管理員模型。  
...
我們建議您執行容錯移轉之前，確認 hello 來源伺服器的 hello 屬性。

1. 在**受保護項目**，按一下 **複寫的項目**，，按一下 hello 機器。
2. 在 [hello**複寫項目**] 窗格中，您可以看到電腦資訊、 健全狀況狀態，以及最新可用的復原點 hello 的摘要。 按一下**屬性**tooview 更多詳細資訊。
3. 在 [計算與網路] 中，您可以：
...
1. 透過單一電腦、 toofail 中**設定** > **複寫的項目**，按一下 hello 機器 > **+ 測試容錯移轉**圖示。
...
4. 按一下**確定**toobegin hello 容錯移轉。 您可以追蹤進度，藉由按 hello 機器 tooopen 其屬性，或是在 hello**測試容錯移轉**作業在保存庫名稱 >**設定** > **作業** > **站台復原工作**。

5. Hello 容錯移轉完成之後，您也應該要能夠 toosee hello 複本 hello Azure 入口網站中出現的 Azure VM >**虛擬機器**。 您應該確定該 hello VM 是 hello 適當大小，它已連接 toohello 適當的網路，而且它正在執行。

6. 如果您在容錯移轉之後連接備妥，您應該能夠 tooconnect toohello Azure VM。

### <a name="delete-test-failover-vms"></a>刪除測試容錯移轉 VM

1. 完成後，按一下**清除測試容錯移轉**hello 復原計劃或電腦上。
2. 在**備忘稿**、 記錄及儲存與 hello 測試容錯移轉相關聯的任何觀察。
3. hello 清理動作會刪除在測試容錯移轉期間所建立的 Azure Vm。

## <a name="summary"></a>摘要

如果您已成功完成 hello 測試容錯移轉，複寫您的實體伺服器，並在您檢查，可以在容錯移轉 tooAzure。 現在，您可以根據您的組織需求來執行容錯移轉。 

請記住，您目前無法無法從 Azure tooa 實體伺服器。 您有 toofail 回 tooa VMware VM。 這表示您需要在順序 toofail 回在內部部署 VMware 基礎結構。 [進一步了解](site-recovery-failback-azure-to-vmware.md)有關失敗後的 Azure Vm tooVMware。
