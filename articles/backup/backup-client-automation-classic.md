...
本文說明如何設定 Windows Server 或 Windows 工作站資料 tooa toouse PowerShell tooback 備份保存庫。 Microsoft 建議讓大部分的新部署使用復原服務保存庫。 如果您是新的 Azure 備份使用者，且您的訂用帳戶中建立備份保存庫，使用 hello 文章[部署和管理使用 PowerShell 的 Data Protection Manager 資料 tooAzure](backup-client-automation.md)讓您將資料儲存在復原服務保存庫中。 
...
如果您想 toouse 指令碼寫入 hello 為 0.9.8 環境，請在 hello 1.0 或更新版本的環境中，您應該仔細測試 hello 指令碼預先生產環境中使用生產 tooavoid 之前先預期的影響。
...
使用 hello **Get AzureRMBackupVault** cmdlet toolist hello 備份保存庫的訂用帳戶中。
...
您可以向 hello Azure 備份服務之前，您需要 tooensure 該 hello[必要條件](backup-configure-vault.md)符合。 您必須：

* 具備有效的 Azure 訂用帳戶
* 具備備份保存庫

toodownload hello 保存庫認證，執行 hello **Get AzureRMBackupVaultCredentials**它在方便的位置類似 Azure PowerShell 主控台和存放區中的 cmdlet * C:\Downloads\*。
...
Hello 保存庫註冊 hello 機器是使用 hello [Start-obregistration](https://technet.microsoft.com/library/hh770398%28v=wps.630%29.aspx) cmdlet:
...
本文件中要說明如何將備份自動化，因此我們假設還未設定任何選項。 一開始我們先建立新的備份原則使用 hello [New-obpolicy](https://technet.microsoft.com/library/hh770416.aspx) cmdlet，並使用它。
...
### <a name="choosing-a-backup-point-toorestore"></a>選擇備份的點 toorestore
hello 備份點清單可以擷取執行 hello [Get-obrecoverableitem](https://technet.microsoft.com/library/hh770399.aspx) cmdlet 搭配適當的參數。 在本例中，我們將選擇 hello 最新備份點 hello 來源磁碟區*d:* ，並用 toorecover 特定檔案。
...
現在使用 hello 觸發還原[Start-obrecovery](https://technet.microsoft.com/library/hh770402.aspx) hello 選取命令```$item```hello hello 輸出```Get-OBRecoverableItem```cmdlet:
