...
* **問：何時應該使用 Azure 檔案、Azure Blob 或 Azure 磁碟？**  
    ...
    如需詳細資訊，請參閱[決定何時使用 Azure Blob、Azure 檔案或 Azure 資料磁碟](storage-decide-blobs-files-disks.md)
...
* **問：如何開始使用 Azure 檔案儲存體？**  
    ...
    * [了解如何使用入口網站建立 Azure 檔案共用](storage-file-how-to-create-file-share.md#create-file-share-through-the-portal)
    * [了解如何使用 PowerShell 建立 Azure 檔案共用](storage-file-how-to-create-file-share.md#create-file-share-through-powershell)
    * [了解如何使用 CLI 建立 Azure 檔案共用](storage-file-how-to-create-file-share.md#create-file-share-through-command-line-interface-cli)
...
* **問：如何啟用 Azure 檔案儲存體的伺服器端加密？**
    ...
    啟用 Azure 檔案儲存體上的 SSE 之後，該儲存體帳戶中寫入檔案儲存體的所有新資料將會自動加密。 這項功能適用於現有或新的儲存體帳戶中，寫入現有或新共用的所有新資料。 此用此功能不會額外收費。 深入了解[如何在 Azure 檔案儲存體上啟用 SSE](storage-service-encryption.md)。
...
* **問：如何針對 Azure 檔案儲存體錯誤進行疑難排解？**
    ...
    您可以參考 [Azure 檔案儲存體疑難排解文章](storage-troubleshoot-file-connection-problems.md)以取得端對端疑難排解指引。 
