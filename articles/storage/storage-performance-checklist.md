...
如需如何 tooget blob 的內容 toodiscover hello 的上次修改日期的範例使用.NET，請參閱[設定和擷取屬性和中繼資料](storage-properties-metadata.md)。 如需條件式下載的詳細資訊，請參閱 [有條件地重新整理 Blob 的本機複本](http://msdn.microsoft.com/library/azure/dd179371.aspx)。  
...
如需如何 tooget blob 的中繼資料使用的.NET，請參閱[設定和擷取屬性和中繼資料](storage-properties-metadata.md)。  
...
如需詳細資訊，請參閱 hello 文章[如何： 變更佇列的訊息內容 hello](storage-dotnet-how-to-use-queues.md#change-the-contents-of-a-queued-message)。  
