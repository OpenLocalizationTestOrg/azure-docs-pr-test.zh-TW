...
在本教學課程中，您就可以擴充 hello 應用程式中建立[Node.js Web 應用程式使用快速]hello Microsoft Azure 用戶端程式庫使用 Node.js toowork 與資料管理服務的教學課程。 您將擴充您的應用程式 toocreate 您可以部署 tooAzure web 為基礎的工作清單應用程式。 hello 工作清單可讓使用者擷取工作、 加入新的工作，並將工作標示為已完成。

hello 工作項目會儲存在 Azure 儲存體。 Azure 儲存體提供可容錯且高度可用的非結構化資料儲存體。 Azure 儲存體包含許多資料結構，其中您可以儲存和存取資料，而且您可以利用從 hello 納入 hello Azure SDK for Node.js 或透過 REST Api 的應用程式開發介面的 hello 儲存體服務。 如需詳細資訊，請參閱[在 Azure 中儲存和存取資料]。
...
tooaccess Azure 儲存體，您需要在儲存體認證 toopass。 toodo，運用 web.config 應用程式設定。
這些設定會傳遞為環境變數 tooNode，然後讀取由 hello Azure SDK。
...
3. 從 hello Azure Powershell 視窗中，輸入下列 cmdlet tooretrieve hello 儲存體帳戶資訊的 hello:
   ...
   這樣會擷取儲存體帳戶和帳戶與您的託管服務相關聯的索引鍵的 hello 清單。
...
這一節中，您將延伸 hello 基本應用程式建立 hello **express**命令藉由新增**task.js**檔案包含您的工作的 hello 模型。 您也會修改現有的 hello **app.js**並建立新**tasklist.js**使用 hello 模式的檔案。
...
4. 接下來，您會加入程式碼 toodefine 以及匯出 hello 工作物件。 此物件負責連接 toohello 資料表。
...
2. 取代 hello hello 內容**index.jade** hello 的下列程式碼檔案。 這會定義 hello 檢視以顯示現有的工作，以及加入新的工作，並將現有的標記為已完成的表單。
...
hello **layout.jade**檔案在 hello**檢視**目錄作為全域範本其他**.jade**檔案。 在此步驟中您會修改它 toouse [Twitter Bootstrap](https://github.com/twbs/bootstrap)，這是一項工具組，可讓您輕鬆 toodesign nice 尋找網站。
...
2. 從 hello**檢視**資料夾中，開啟 hello **layout.jade** hello 下列您文字編輯器和取代 hello 內容中：
...
如往常一般，因為指定 hello **-啟動**選項，hello 瀏覽器會開啟並顯示您在發行完成時，在 Azure 中執行的應用程式。

![瀏覽器視窗，顯示 hello [我的工作清單] 頁面。 hello URL 表示 hello 頁面現在會裝載於 Azure。](./media/storage-nodejs-use-table-storage-cloud-service-app/getting-started-1.png)
