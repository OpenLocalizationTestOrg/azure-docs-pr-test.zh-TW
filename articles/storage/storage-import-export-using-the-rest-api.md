...
hello Microsoft Azure 匯入/匯出服務會公開 REST API tooenable 以程式設計方式控制匯入/匯出工作。 您可以使用 hello REST API tooperform hello 的所有匯入/匯出作業，您可以執行以 hello [Azure 入口網站](https://portal.azure.com/)。 此外，您可以使用 hello REST API tooperform 特定精細的作業，例如查詢 hello 百分比完成的工作，不是目前可用 hello 傳統入口網站中。
