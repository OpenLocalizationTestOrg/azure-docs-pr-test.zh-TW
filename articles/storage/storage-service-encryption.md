...
Azure 檔案儲存體提供使用標準 SMB 通訊協定 hello hello 雲端中的檔案共用。 您可以從內部部署或 Azure 中的用戶端掛接檔案共用。 掛接之後，如 Robocopy 工具可以使用的 toocopy 檔案經由 tooAzure 檔案共用。 如需詳細資訊，請參閱[如何 toomount Azure 檔案共用上 Windows](storage-file-how-to-use-files-windows.md)和[toomount Azure 檔案在 Linux 上的共用方式](storage-how-to-use-files-linux.md)。
...
toolearn 詳細資訊，請造訪我們[開始使用適用於.NET 的 Azure Blob 儲存體使用](storage-dotnet-how-to-use-blobs.md)。
