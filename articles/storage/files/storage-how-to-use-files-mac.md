...
1. **Finder 開啟**： 搜尋已在 macOS 預設會開啟，但是您可以確保它是 hello 上目前選取的應用程式即可 hello"macOS 面臨的圖示"hello 停駐：  
    ![hello macOS 面臨圖示](./media/storage-how-to-use-files-mac/mount-via-finder-1.png)

2. **選取 「 連接 tooServer"hello"Go"功能表從**： 使用 hello UNC 路徑從 hello[必要條件](#preq)，轉換 hello 開頭雙反斜線 (`\\`) 太`smb://`和所有其他反斜線 (`\`)tooforwards 斜線 (`/`)。 您的連結起來會像下列 hello: ![hello 「 連接 tooServer 」 對話方塊](./media/storage-how-to-use-files-mac/mount-via-finder-2.png)
