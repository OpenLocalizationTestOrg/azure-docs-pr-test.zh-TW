...
tootest 您的應用程式在本機的 Windows 電腦，您可以使用 Microsoft Azure 的 hello[儲存體模擬器](../common/storage-use-emulator.md?toc=%2fazure%2fstorage%2fqueues%2ftoc.json)會隨 hello [Azure SDK](https://azure.microsoft.com/downloads/)。 hello 儲存體模擬器是模擬 hello Blob、 佇列和表格服務在本機開發電腦上可用在 Azure 中的公用程式。 hello 下列範例顯示如何宣告靜態欄位 toohold hello 連接字串 tooyour 本機儲存體模擬器：  
