...
Hello 下載作業之後，hello 目錄`C:\myfolder`會包含下列檔案的 hello:
...
假設 hello 下列 blob 位於 hello 指定的容器。 以 hello 前置詞開頭的所有 blob`a`會下載：
...
Hello 下載作業之後，hello 資料夾`C:\myfolder`會包含下列檔案的 hello:

    C:\myfolder\abc.txt
    C:\myfolder\abc1.txt
    C:\myfolder\abc2.txt

hello 前置詞會套用 toohello 虛擬目錄，可形成 hello hello blob 名稱第一個部分。 Hello 虛擬目錄在 hello 上述範例中，它不符合 hello 指定之前置詞，因此它不會下載。 此外，如果 hello 選項`\S`未指定，AzCopy 將不會下載任何 blob。
...
如果 hello 指定的目的地容器不存在，AzCopy 會建立檔案，並將 hello 檔案上傳到它。
...
如果 hello 指定虛擬目錄不存在，AzCopy 會將上傳 hello 檔案 tooinclude hello 虛擬目錄名稱 (*例如*， `vd/abc.txt` hello 上述範例中)。
...
指定選項`/S`hello 上傳 hello 內容所指定目錄 tooBlob 儲存體以遞迴方式，也就是說，所有子資料夾及檔案將會上傳以及。 比方說，假設 hello 下列檔案位於資料夾`C:\myfolder`:
...
Hello 上傳作業之後，hello 容器將包含下列檔案的 hello:
...
如果您未指定選項 `/S`，AzCopy 將不會以遞迴方式上傳。 Hello 上傳作業之後，hello 容器將包含下列檔案的 hello:
...
如果您指定`/SetContentType`沒有值，然後 AzCopy 會將每個 blob 或根據 tooits 副檔名的檔案的內容類型。
...
Hello 複製作業之後，hello blob 和其快照集，將包含 hello 目標容器。 假設在 hello 上述範例中的 hello blob 具有兩個快照集，hello 容器將包含 hello 下列 blob 和快照集：
...
根據預設，AzCopy 會以非同步方式在兩個儲存體端點之間複製資料。 因此，會使用不具備任何 SLA 的備用頻寬容量 hello 背景中執行 hello 複製作業方面速度將會複製 blob，和 AzCopy 會定期檢查 hello 複製狀態，直到 hello 複製完成或失敗。

hello`/SyncCopy`選項可確保 hello 複製作業會取得一致的速度。 AzCopy 下載 hello blob 執行 hello 同步複本 toocopy hello 從指定來源 toolocal 記憶體，並再將其上傳 toohello Blob 儲存體目的地。
...
如果 hello 指定來源是 Azure 的檔案共用，則您必須指定 hello 確切的檔名，(*例如* `abc.txt`) toodownload 單一檔案，或指定選項`/S`toodownload 所有檔案在 hello 共用以遞迴方式。 檔案模式和選項，嘗試 toospecify`/S`一起會導致錯誤。
...
您可以指定 hello`/SyncCopy`選項 toocopy 資料檔案儲存體 tooFile 存放裝置，從檔案儲存體 tooBlob 存放裝置，然後從 Blob 儲存體 tooFile 存放同步執行，AzCopy 會下載 hello 來源資料 toolocal 記憶體並將它上傳再次 toodestination。 此作業會產生標準輸出成本。
...
hello 資料分割索引鍵範圍索引將會是 0，如果使用者未指定選項`/PKRS`。
...
請注意該 hello 最小可能值選項`/SplitSize`為 32 MB。 如果 hello 指定目的地 Blob 儲存體，AzCopy 會將 hello 資料檔案分割一次，將其大小到達 hello blob 大小上限 (200 GB)，不論是否選項`/SplitSize`hello 使用者尚未指定。
...
AzCopy 時指定 hello CSV 裝載格式，也會產生結構描述檔案具有副檔名`.schema.csv`每個資料檔。
...
AzCopy hello 使用者指定 選項時，會啟動並行作業 tooexport 實體`/PKRS`。 每個作業分別會匯出一個資料分割索引鍵範圍。
...
請注意，當匯出資料表 tooblobs，AzCopy 會下載 hello 資料表實體 toolocal 暫存資料檔，並再將這些實體 toohello blob 上傳。 這些暫存資料檔案會放入與 hello 預設路徑 hello 日誌檔案的資料夾"<code>%LocalAppData%\Microsoft\Azure\AzCopy</code>」，您可以指定選項/z: [日誌-檔案-資料夾] toochange hello 日誌檔案資料夾位置，並因此變更 hello 暫存資料檔案位置。 hello 雖然一旦它已在本機磁碟中的 hello 暫存資料檔案也會立即刪除，檔案的大小由資料表實體的大小和您指定了與 hello 選項 /SplitSize，hello 大小來決定暫存資料上傳 toohello blob，請確定您在刪除前有足夠的本機磁碟空間 toostore 這些暫存資料的檔案。
...
請注意，您不能指定選項`/PKRS`hello 匯入案例中。 不同於 hello 匯出案例中，您必須在其中指定選項`/PKRS`toostart 並行作業，AzCopy 會依預設並行作業時啟動匯入資料表。 並行作業啟動的 hello 預設數目是相等的 toohello 核心處理器數目;不過，您可以指定不同數目的並行選項`/NC`。 如需詳細資訊，請輸入`AzCopy /?:NC`在 hello 命令列。
...
AzCopy 如果將會失敗 hello 參數分成兩行，如下所示為 hello`/sourcekey`參數：
...
如果 hello 日誌檔不存在，AzCopy 會檢查您輸入的 hello 命令列是否符合 hello hello 筆記本檔案中的命令列。 如果兩個命令列 hello 相符，AzCopy 會繼續 hello 未完成作業。 如果不相符，您將會提示的 tooeither 覆寫 hello 日誌檔案 toostart，新增作業或 toocancel hello 目前的作業。
...
如果來源是 Azure 的檔案共用，則您必須指定 hello 確切的檔名 (例如 abc.txt)，指定 hello toocopy 單一檔案，或指定選項 /S toocopy hello 共用以遞迴方式中的所有檔案。 檔案模式和選項，嘗試 toospecify /S 一起會導致錯誤。
...
如果 hello 來源資源是 blob 容器，並提供索引鍵都 SAS，將會透過匿名存取讀取 hello blob 容器。
...
根據預設，名為 hello 詳細的記錄檔中的 AzCopyVerbose.log `%LocalAppData%\Microsoft\Azure\AzCopy`。 如果您指定這個選項是現有的檔案位置，hello 詳細資訊記錄會附加的 toothat 檔案。  
...
如果未指定此選項，或指定不包含資料夾路徑，AzCopy 會在 hello 預設位置為 %localappdata%\microsoft\azure\azcopy 建立 hello 日誌檔。
...
AzCopy 會解譯 hello 使用這個選項做為執行 hello 命令列未執行此動作的模擬選項 /L 和計算多少物件會被複製，您可以指定選項在 hello /V 相同時間的 toocheck hello 詳細資訊記錄檔中將複製的物件。
...
### <a name="xn"></a>/XN
排除較新的來源資源。 如果 hello hello 來源的上次修改的時間為 hello 相同或更新版本比目的地，將不會複製 hello 資源。

**適用於：** Blob、檔案

### <a name="xo"></a>/XO
排除較舊的來源資源。 如果 hello hello 來源的上次修改的時間為 hello 相同或早於目的地，將不會複製 hello 資源。
...
如果未指定此選項，AzCopy 會匯出資料表資料 toosingle 檔。

如果 hello 資料表資料是匯出的 tooa blob hello 匯出的檔案的大小到達的 blob 大小的 hello 200 GB 限制，然後 AzCopy 會分割 hello 匯出的檔案，即使沒有指定這個選項。
...
這個選項是選擇性的 hello 匯出作業期間，AzCopy 會產生資訊清單檔案具有預先定義的名稱，如果沒有指定這個選項。
...
如果您指定這個選項沒有值，然後將設定 AzCopy，每個 blob 或根據 tooits 副檔名的檔案的內容類型。
...
True-屬性 」 AzureStorageUseV1MD5"• hello 預設值，AzCopy 會使用.NET MD5 實作。
...
如需 Azure 儲存體和 AzCopy 的詳細資訊，請參閱下列資源 toohello。

### <a name="azure-storage-documentation"></a>Azure 儲存體文件：
* [簡介 tooAzure 儲存體](storage-introduction.md)
* [如何 toouse 與.NET 的 Blob 儲存體](storage-dotnet-how-to-use-blobs.md)
* [如何 toouse 檔案與.NET 的儲存體](storage-dotnet-how-to-use-files.md)
* [如何 toouse 資料表與.NET 的儲存體](storage-dotnet-how-to-use-tables.md)
