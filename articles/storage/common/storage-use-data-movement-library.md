...
hello Microsoft Azure 儲存體的資料移動程式庫是設計用來上傳、 下載和 Azure 儲存體 Blob 和檔案複製的高效能的開放原始碼跨平台程式庫。 此程式庫是 hello 核心資料移動架構提供[AzCopy](../storage-use-azcopy.md)。 hello 資料移動程式庫提供方便的方法不提供的我們傳統[.NET Azure 儲存體用戶端程式庫](../blobs/storage-dotnet-how-to-use-blobs.md)。 這包括 hello 能力 tooset hello 平行作業數目，追蹤傳輸進度，輕鬆地繼續已取消的傳輸，以及執行更多。  
