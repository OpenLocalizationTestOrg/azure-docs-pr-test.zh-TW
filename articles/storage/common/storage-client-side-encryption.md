...
hello [.NET Nuget 套件的 Azure 儲存體用戶端程式庫](https://www.nuget.org/packages/WindowsAzure.Storage)支援 tooAzure 存放裝置，將上傳和下載 toohello 用戶端時解密資料之前加密用戶端應用程式內的資料。 hello 程式庫也支援與整合[Azure 金鑰保存庫](https://azure.microsoft.com/services/key-vault/)儲存體帳戶金鑰管理。

將引導您完成加密 blob 使用用戶端加密與 Azure 金鑰保存庫的 hello 程序的逐步教學課程，請參閱[中使用 Azure 金鑰保存庫的 Microsoft Azure 儲存體加密及解密 blob](../blobs/storage-encrypt-decrypt-blobs-key-vault.md)。
...
### <a name="blobs"></a>Blob
hello 用戶端程式庫目前支援整個 blob 只的加密。 當使用者使用 hello 時，特別是，支援加密**UploadFrom*** 方法或 hello **OpenWrite**方法。 針對下載，則皆支援完整與範圍下載。
...
加密的 blob 下載牽涉到擷取的 hello 整個 blob 使用 hello hello 內容**DownloadTo***/**BlobReadStream** 方便方法。 hello 包裝的 CEK 解除包裝，且與 hello IV 一起使用 （儲存為 blob 中繼資料在此情況下） tooreturn hello 解密資料 toohello 使用者。
...
* Microsoft.Azure.KeyVault.Core 包含 hello IKey 和 IKeyResolver。 這是不具有相依性的小型封裝。 hello.NET 的儲存體用戶端程式庫將其定義為相依性。
* Microsoft.Azure.KeyVault 包含 hello 金鑰保存庫 REST 用戶端。
* Microsoft.Azure.KeyVault.Extensions 包含延伸模組程式碼，其中包含密碼編譯演算法的實作及 RSAKey 和 SymmetricKey。 它相依於 hello 核心和 KeyVault 命名空間並提供功能 toodefine 彙總的解析程式 （當使用者想 toouse 多個索引鍵的提供者） 和快取索引鍵的解析程式。 雖然 hello 儲存體用戶端程式庫不會直接依賴這個封裝，如果使用者想 toouse Azure 金鑰保存庫 toostore 其索引鍵或 toouse hello 金鑰保存庫的擴充功能 tooconsume hello 本機和雲端密碼編譯提供者，它們會需要此封裝。
...
金鑰保存庫的使用方式的更多關於位於 hello[加密程式碼範例](https://github.com/Azure/azure-storage-net/tree/master/Samples/GettingStarted/EncryptionSamples)。

## <a name="best-practices"></a>最佳作法
加密已支援只能在 hello.NET 的儲存體用戶端程式庫中。 Windows Phone 和 Windows 執行階段目前不支援加密。
...
> * 如果您在 hello 加密的 blob 上設定中繼資料，您可能會覆寫 hello 加密相關中繼資料所需來解密，因為設定中繼資料不是加總。 快照集也是如此。在為加密的 Blob 建立快照集時，請避免指定中繼資料。 如果必須設定中繼資料，是確定 toocall hello **FetchAttributes**方法的第一個 tooget hello 目前的加密中繼資料和中繼資料在設定時，避免並行寫入。
> * 啟用 hello **RequireEncryption** hello 應該僅適用於的使用者的預設要求選項中的屬性是否已加密資料。 如需詳細資訊請參閱下方內容。
> 
> 
...
hello[加密範例](https://github.com/Azure/azure-storage-net/tree/master/Samples/GettingStarted/EncryptionSamples)示範更詳細的端對端案例，適用於 blob、 佇列和資料表，以及與金鑰保存庫整合。

### <a name="requireencryption-mode"></a>RequireEncryption 模式
使用者可以針對所有上傳和下載都必須加密的作業模式，從中選擇啟用。 在此模式中，嘗試 tooupload 資料，而不會加密 hello 服務的加密原則或下載資料不會失敗 hello 用戶端上。 hello **RequireEncryption** hello 要求選項物件的屬性會控制此行為。 如果您的應用程式將會加密儲存在 Azure 儲存體中的所有物件，則您可以將 hello **RequireEncryption** hello hello 服務用戶端物件的預設要求選項上的屬性。 例如，設定**CloudBlobClient.DefaultRequestOptions.RequireEncryption**太**true** toorequire 加密所有的 blob 作業透過該用戶端物件來執行。
...
此外 toocreating 加密原則和設定要求的選項，您必須指定**EncryptionResolver**中**TableRequestOptions**，或在 hello [EncryptProperty] 屬性hello 實體。
...
如前所述，如果 hello 實體實作 TableEntity，然後 hello 屬性可以以裝飾 hello [EncryptProperty] 屬性，而不是指定 hello **EncryptionResolver**。
...
* 下載 hello [.NET NuGet 套件的 Azure 儲存體用戶端程式庫](https://www.nuget.org/packages/WindowsAzure.Storage)
* 下載 Azure 金鑰保存庫 NuGet hello[核心](http://www.nuget.org/packages/Microsoft.Azure.KeyVault.Core/)，[用戶端](http://www.nuget.org/packages/Microsoft.Azure.KeyVault/)，和[延伸](http://www.nuget.org/packages/Microsoft.Azure.KeyVault.Extensions/)封裝  
