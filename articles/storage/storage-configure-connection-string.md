...
您可能希望 toospecify 了明確的端點的其中一個案例是當您已對應您的 Blob 儲存體端點 tooa[自訂網域](storage-custom-domain-name.md)。 在此情況下，您可以在連接字串中指定 Blob 儲存體的自訂端點。 您可以選擇性地指定 hello hello 預設端點的其他服務如果應用程式使用它們。
