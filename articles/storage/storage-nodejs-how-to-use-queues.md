...
建立空白的 Node.js 應用程式。 建立 Node.js 應用程式的指示，請參閱[Node.js web 應用程式建立 Azure App Service 中]，[建置和部署 Azure 雲端服務的 Node.js 應用程式 tooan]使用 Windows PowerShell 或[建置和部署使用 Web Matrix Node.js web 應用程式 tooAzure]。
...
* 請瀏覽 hello [Azure 儲存體團隊部落格][Azure Storage Team Blog]。
