...
toocreate Azure 檔案共用，您可以使用的儲存體帳戶已經存在，或[建立新的 Azure 儲存體帳戶](storage-create-storage-account.md)。 toocreate 使用 PowerShell 的 Azure 檔案共用，您將需要 hello 帳號金鑰及儲存體帳戶名稱... 如果您計劃 toouse Powershell 或 CLI，您需要儲存體帳戶金鑰。
...
2. **按一下 [新增檔案共用] 按鈕**：    
    ![按一下 hello 新增檔案共用 按鈕](media/storage-file-how-to-create-file-share/create-file-share-portal2.png)

3. **提供名稱和配額。配額目前上限可以是 5TB**：    
    ![提供的名稱和所需的配額的 hello 新檔案共用](media/storage-file-how-to-create-file-share/create-file-share-portal3.png)
...
1. **建立您的儲存體帳戶和金鑰內容**hello 內容封裝 hello 儲存體帳戶名稱和帳戶金鑰。 如需從 [Azure 入口網站](https://portal.azure.com/)複製帳戶金鑰的指示，請參閱[檢視和複製儲存體存取金鑰](storage-create-storage-account.md#view-and-copy-storage-access-keys)。
