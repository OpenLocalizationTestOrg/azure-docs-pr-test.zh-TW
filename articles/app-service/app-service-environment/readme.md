...
ASEs 隔離的 toorunning 只有單一客戶的應用程式，而且一律會部署到 Azure 虛擬網路。 客戶可以使用[網路安全性群組][NSGs]，同時精確控制輸入和輸出應用程式網路流量。 應用程式也可以建立安全的高速連線透過虛擬網路 tooon 內部公司資源。

應用程式經常需要 tooaccess 公司資源，例如內部資料庫和 web 服務。 執行於 ASE 上的應用程式可透過[站對站][SiteToSite] VPN 連線和[Azure ExpressRoute][ExpressRoute] 連線來存取資源。
...
* [網路考量和 hello App Service 環境][ASENetwork]
* [從範本建立 App Service Environment][MakeASEfromTemplate]


## <a name="videos"></a>影片
Hello 企業版與 Azure 應用程式服務的主要現代化 PaaS
