...
本教學課程會引導您實作與使用 Java 的 Azure 媒體服務 (AMS) 應用程式的基本 Video-on-Demand (VoD) 內容傳遞服務的 hello 步驟。
...
* hello Azure Libraries for Java，您可以從 hello 安裝[Azure Java 開發人員中心][Azure Java Developer Center]。
...
hello 下列程式碼說明如何 toocreate 資產上, 傳媒體檔案 toohello 資產、 執行工作與工作 tootransform hello 的資產，並建立定位器 toostream 視訊。

您需要建立 Media Services 帳戶 tooset 才能使用這個程式碼。 設定帳戶的詳細資訊，請參閱[如何 tooCreate Media Services 帳戶](media-services-portal-create-account.md)。

替換為您的 hello 'clientId' 和 'Clientid' 變數的值。 hello 程式碼也依賴本機儲存檔案。 您將需要您自己的檔案 toouse tooprovide。
