...
---
# <a name="how-tooperform-live-streaming-with-on-premises-encoders-using-net"></a>如何 tooperform 即時資料流與內部部署編碼器使用的.NET
...
本教學課程會引導您逐步使用 hello Azure Media Services.NET SDK toocreate hello**通道**針對傳遞的傳遞設定。 
...
## <a name="example"></a>範例
hello，下列程式碼範例示範如何 tooachieve hello 下列工作：

* TooMedia 服務連接
* 建立通道
* 更新 hello 頻道
* 擷取 hello 通道輸入的端點。 toohello 內部部署即時編碼器應提供 hello 輸入的端點。 從傳送 toohello 瑎堀熁檓 hello 相機 toostreams hello 即時編碼器將訊號 （內嵌） 端點。
* 擷取 hello 通道預覽端點
* 建立並啟動程式
* 建立定位器需要 tooaccess hello 程式
...
如需詳細資訊 tooconfigure 即時編碼程式，請參閱[Azure Media Services RTMP 支援和即時編碼程式](https://azure.microsoft.com/blog/2014/09/18/azure-media-services-rtmp-support-and-live-encoders/)。
