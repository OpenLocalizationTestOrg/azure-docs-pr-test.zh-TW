...
本主題顯示 hello`H264 Single Bitrate 720p for Android`預設 XML 和 JSON 格式。  
  
此預設會產生位元速率為 2000 kbps 的單一 MP4 檔案，而且是立體聲 AAC。 如需設定檔的詳細資訊，位元速率，取樣率、 等等。 這個預設值，請檢查 hello XML 或 JSON 定義如下。 如需每個項目哪些這些預設值表示與 hello 有效的值中的每個項目說明，請參閱 hello[媒體編碼器標準結構描述](media-services-mes-schema.md)主題。  
