---
title: "aaaAutomated 指令碼 toocreate 與在 OMS 中的 IT 服務管理連接器服務管理員 Web 應用程式 tooconnect |Microsoft 文件"
description: "建立 Service Manager Web 應用程式與 IT 服務管理連接器在 OMS 中，使用自動化的指令碼 tooconnect 和集中監視和管理 hello ITSM 工作項目。"
...
---
# <a name="create-service-manager-web-app-using-hello-automated-script-preview"></a><span data-ttu-id="e408d-103">建立 Service Manager Web 應用程式使用 hello 自動化指令碼 （預覽）</span><span class="sxs-lookup"><span data-stu-id="e408d-103">Create Service Manager Web app using hello automated script (Preview)</span></span>

<span data-ttu-id="e408d-104">使用下列指令碼 toocreate hello Web 應用程式服務管理員執行個體的 hello。</span><span class="sxs-lookup"><span data-stu-id="e408d-104">Use hello following script toocreate hello Web app for your Service Manager instance.</span></span> <span data-ttu-id="e408d-105">可在這裡找到 Service Manager 連線的相關詳細資訊：[Service Manager Web 應用程式](log-analytics-itsmc-connections.md#create-and-deploy-service-manager-web-app-service)</span><span class="sxs-lookup"><span data-stu-id="e408d-105">More information about Service Manager connection is here: [Service Manager Web app](log-analytics-itsmc-connections.md#create-and-deploy-service-manager-web-app-service)</span></span>

<span data-ttu-id="e408d-106">藉由提供下列必要的詳細資料的 hello 執行 hello 指令碼：</span><span class="sxs-lookup"><span data-stu-id="e408d-106">Run hello script by providing hello following required details:</span></span>

- <span data-ttu-id="e408d-107">Azure 訂用帳戶詳細資料</span><span class="sxs-lookup"><span data-stu-id="e408d-107">Azure subscription details</span></span>
- <span data-ttu-id="e408d-108">資源群組名稱</span><span class="sxs-lookup"><span data-stu-id="e408d-108">Resource group name</span></span>
- <span data-ttu-id="e408d-109">位置</span><span class="sxs-lookup"><span data-stu-id="e408d-109">Location</span></span>
- <span data-ttu-id="e408d-110">Service Manager 伺服器詳細資料 (伺服器名稱、網域、使用者名稱和密碼)</span><span class="sxs-lookup"><span data-stu-id="e408d-110">Service Manager server details (server name,    domain, username and password)</span></span>
- <span data-ttu-id="e408d-111">Web 應用程式的網站名稱前置詞</span><span class="sxs-lookup"><span data-stu-id="e408d-111">Site name prefix for your Web app</span></span>
- <span data-ttu-id="e408d-112">服務匯流排命名空間。</span><span class="sxs-lookup"><span data-stu-id="e408d-112">ServiceBus Namespace.</span></span>

<span data-ttu-id="e408d-113">hello 指令碼會建立使用您指定的 hello 名稱 hello Web 應用程式 (以及幾個額外字串 toomake 它唯一)。</span><span class="sxs-lookup"><span data-stu-id="e408d-113">hello script will create hello Web app using hello name that you specified (along with few additional strings toomake it unique).</span></span> <span data-ttu-id="e408d-114">它會產生 hello **Web 應用程式 URL**，**用戶端識別碼**和**用戶端密碼**。</span><span class="sxs-lookup"><span data-stu-id="e408d-114">It generates hello **Web app URL**, **client ID** and **client secret**.</span></span>

<span data-ttu-id="e408d-115">將這些值儲存，當您使用 IT 服務管理連接器建立連線時會用到這些值。</span><span class="sxs-lookup"><span data-stu-id="e408d-115">Save these values, you will need these when you create a connection with IT Service Management Connector.</span></span>

## <a name="prerequisites"></a><span data-ttu-id="e408d-116">必要條件</span><span class="sxs-lookup"><span data-stu-id="e408d-116">Prerequisites</span></span>

 <span data-ttu-id="e408d-117">Windows Management Framework 5.0 或更新版本。</span><span class="sxs-lookup"><span data-stu-id="e408d-117">Windows Management Framework 5.0 or above.</span></span>
<span data-ttu-id="e408d-118">Windows 10 依預設包含 5.1。</span><span class="sxs-lookup"><span data-stu-id="e408d-118">Windows 10 has 5.1 by default.</span></span> <span data-ttu-id="e408d-119">您可以下載從 hello framework[這裡](https://www.microsoft.com/download/details.aspx?id=53347):</span><span class="sxs-lookup"><span data-stu-id="e408d-119">You can download hello framework from [here](https://www.microsoft.com/download/details.aspx?id=53347):</span></span>

<span data-ttu-id="e408d-120">使用下列指令碼的 hello:</span><span class="sxs-lookup"><span data-stu-id="e408d-120">Use hello following script:</span></span>
...
```
## <a name="next-steps"></a><span data-ttu-id="e408d-121">後續步驟</span><span class="sxs-lookup"><span data-stu-id="e408d-121">Next steps</span></span>
<span data-ttu-id="e408d-122">[設定 hello 混合式連接](log-analytics-itsmc-connections.md#configure-the-hybrid-connection)。</span><span class="sxs-lookup"><span data-stu-id="e408d-122">[Configure hello Hybrid connection](log-analytics-itsmc-connections.md#configure-the-hybrid-connection).</span></span>
