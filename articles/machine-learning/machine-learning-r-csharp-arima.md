...
這[服務](https://datamarket.azure.com/dataset/aml_labs/arima)實作根據 hello hello 使用者所提供的歷程記錄資料的自動迴歸整合式移動平均 (ARIMA) tooproduce 預測。 將 hello 加大本年度的特定產品的需求？ 可以我預測 hello 產品銷售聖誕季節，讓我可以有效地計劃我清查？ 預測模型是 apt tooaddress 這類問題。 指定 hello 過去的資料，這些模型會檢查隱藏的趨勢和季節性 toopredict 未來的趨勢。 
...
此服務會接受 4 個引數，並計算 hello ARIMA 預測。
hello 輸入引數為：
...
有多種方式來使用 hello 服務，以自動化方式 (範例應用程式是[這裡](http://microsoftazuremachinelearning.azurewebsites.net/ArimaForecasting.aspx))。
...
Azure Machine Learning 中已建立新的空白實驗， 並已使用預先定義的資料結構描述上傳範例輸入資料。 連結的 toohello 資料結構描述是[執行 R 指令碼][ execute-r-script]模組，使用 'auto.arima' 和 '趨勢預測' 函式，從 r 所產生 hello ARIMA 預測模型 
...
這是一個非常簡單的 ARIMA 預測範例。 從上述的 hello 範例程式碼可以看到，不實作攔截任何錯誤時，以及 hello 服務會假設所有 hello 變數都是連續/positive 值而 hello 頻率應大於 1 的整數。 hello hello 的日期和值的向量長度應該 hello 相同。 hello 日期變數應該遵守 toohello ' mm/dd/yyyy 格式 '。

## <a name="faq"></a>常見問題集
常見問題集 hello web 服務或發行 toomarketplace 耗用量問題，請參閱[這裡](machine-learning-marketplace-faq.md)。
