...
Azure Machine Learning 中已建立新的空白實驗， 並已使用預先定義的資料結構描述上傳範例輸入資料。 已連結至資料結構描述的[執行 R 指令碼][execute-r-script]模組會使用 R 的 ‘ets’ 和 ‘forecast’ 函式，以產生 ETS 預測模型。 
