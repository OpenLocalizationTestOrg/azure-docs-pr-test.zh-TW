...
這[web 服務](https://datamarket.azure.com/dataset/aml_labs/demand_forecast)實作季節性趨勢分解 (STL) 和指數平滑效果 (ETS) 模型 tooproduce 預測根據 hello 使用者所提供的 hello 歷程記錄資料。 將 hello 加大本年度的特定產品的需求？ 可以我預測 hello 產品銷售聖誕季節，讓我可以有效地計劃我清查？ 預測模型是 apt tooaddress 這類問題。 指定 hello 過去的資料，這些模型會檢查隱藏的趨勢和季節性 toopredict 未來的趨勢。 
...
有多種方式來使用 hello 服務，以自動化方式 (範例應用程式是[這裡](http://microsoftazuremachinelearning.azurewebsites.net/StlEtsForecasting.aspx))。
...
Azure Machine Learning 中已建立新的空白實驗， 並已使用預先定義的資料結構描述上傳範例輸入資料。 連結的 toohello 資料結構描述是[執行 R 指令碼][ execute-r-script]模組，如此就會產生 STL 和 ETS 使用 'stl'、 'ets'，預測模型，並從 R '預測' 函式 
...
這是一個非常簡單的 ETS + STL 預測範例。 從上述的 hello 範例程式碼可以看到，不實作攔截任何錯誤時，以及 hello 服務會假設所有 hello 變數都是連續/positive 值而 hello 頻率應大於 1 的整數。 hello hello 的日期和值的向量的長度應該是 hello 相同，而且 hello hello 時間序列的長度應該大於 2 * 頻率。 hello 日期變數應該遵守 toohello ' mm/dd/yyyy 格式 '。
