...
應用程式閘道支援 hello 能力 tooredirect 流量根據定義的組態。 深入了解一般造訪中的重新導向 toolearn[應用程式閘道重新導向概觀](application-gateway-redirect-overview.md)。 這篇文章會提供 HTTP tooHTTPS 重新導向的範例，基礎的路徑重新導向，多站台重新導向，並重新導向 tooexternal 站台。

## <a name="http-toohttps-redirect-on-an-existing-application-gateway"></a>HTTP tooHTTPS 重新導向上現有的應用程式閘道

hello 下列範例會建立新的 HTTP 接聽程式接聽連接埠 80 tooredirect 要求 toohello 現有連接埠 443。
...
一旦更新 hello 應用程式閘道時，測試 hello 新接聽程式通訊埠 80 上。  hello 下列範例顯示如何使用`curl`測試 hello 重新導向。
...
hello 下列範例會建立新的接聽程式和基礎的路徑規則上現有的應用程式閘道重新導向流量會議 hello 特定的 url 路徑。
...
hello 下列範例會建立新的應用程式閘道 2 連接埠 80 上的多站台接聽程式。 hello 接聽程式是 adatum.com 和 adatum.org。從 adatum.org tooadatum.com 建立 tooredirect 流量重新導向規則。其他設定，才能設定 CNAME 別名 toohello 應用程式閘道公用 IP 位址，如需委派您網域 tooAzure DNS 的詳細資訊，建立 CNAME 記錄為您的網域瀏覽，[委派網域tooAzure DNS](../dns/dns-delegate-domain-azure-dns.md)。
...
## <a name="redirect-tooan-external-site"></a>重新導向 tooan 外部站台

hello 下列範例會重新導向流量進入 hello 應用程式閘道 tooan 外部網站 (bing.com)。 當使用 hello`TargetUrl`交換器 hello`IncludePath`和`IncludeQuery`參數不允許。
...
請瀏覽[結束 tooend SSL 設定與使用 PowerShell 的應用程式閘道](application-gateway-end-to-end-ssl-powershell.md)
