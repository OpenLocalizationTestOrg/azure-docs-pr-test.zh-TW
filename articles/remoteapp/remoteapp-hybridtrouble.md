...
> [!NOTE]
> 您必須確定 VNET 中的 DNS 伺服器都已啟動，而且永遠能夠解析裝載於 VNET 中的虛擬機器。 請勿在此使用 Google DNS。
> 
> 

對於混合式集合，您使用專屬 DNS 伺服器。 您可以在網路組態結構描述中或在建立虛擬網路時透過管理入口網站來指定它們。 DNS 伺服器的使用順序就是針對進行容錯移轉所指定的順序 (而非循環配置資源)。  
請參閱 [VM 與角色執行個體的名稱解析](../virtual-network/virtual-networks-name-resolution-for-vms-and-role-instances.md) ，以確定您的 DNS 伺服器已正確設定。
