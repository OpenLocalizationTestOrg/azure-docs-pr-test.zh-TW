...
在此教學課程中，您學會如何 toointegrate EasyTerritory 與 Azure Active Directory (Azure AD)。

與 Azure AD 整合 EasyTerritory 可以提供下列優點 hello:

- 您可以控制存取 tooEasyTerritory Azure AD 中。
- 您可以啟用您的使用者 tooautomatically get 登入 tooEasyTerritory （單一登入） 具有其 Azure AD 帳戶。
...
tooconfigure EasyTerritory 與 Azure AD 整合，您需要下列項目 hello:

- Azure AD 訂用帳戶
- 已啟用 EasyTerritory 單一登入的訂用帳戶
...
1. 從 hello 圖庫加入 EasyTerritory
2. 設定並測試 Azure AD 單一登入

## <a name="adding-easyterritory-from-hello-gallery"></a>從 hello 圖庫加入 EasyTerritory
tooconfigure hello 整合 EasyTerritory 到 Azure AD，您需要 tooadd EasyTerritory hello 圖庫 tooyour 清單中的受管理的 SaaS 應用程式。

**tooadd EasyTerritory 從 hello 組件庫中，執行下列步驟的 hello:**
...
4. 在 [hello] 搜尋方塊中，輸入**EasyTerritory**，選取**EasyTerritory**然後按一下 [從結果面板**新增**按鈕 tooadd hello 應用程式。
...
單一登入 toowork，Azure AD 需要 tooknow hello 的對等項目的使用者中 EasyTerritory 是 tooa 使用者在 Azure AD 中。 換句話說，Azure AD 使用者與 hello EasyTerritory 中相關的使用者之間的連結關聯性需要 toobe 建立。

EasyTerritory 中, 指派的 hello hello 值**使用者名**做為 hello hello 值的 Azure AD 中**Username** tooestablish hello 連結關聯性。

tooconfigure 及 EasyTerritory 與 Azure AD 單一登入的測試，您必須遵循的建置組塊 toocomplete hello:
...
3. **[建立測試使用者 EasyTerritory](#create-a-easyterritory-test-user) ** -toohave 許 Simon EasyTerritory 所連結的 toohello Azure AD 使用者表示法中對應項目。
...
在本節中，您可以啟用 Azure AD 單一登入 hello Azure 入口網站中，並 EasyTerritory 應用程式中設定單一登入。

**tooconfigure Azure AD 單一登入與 EasyTerritory，執行下列步驟的 hello:**

1. 在 Azure 入口網站上 hello hello **EasyTerritory**應用程式整合頁面上，按一下 [**單一登入**。
...
3. 在 [hello **EasyTerritory 網域和 Url**區段中，執行下列步驟，如果您想在 IDP tooconfigure hello 應用程式起始模式 hello:

    ![EasyTerritory 網域和 URL 單一登入資訊](./media/active-directory-saas-easyterritory-tutorial/tutorial_easyterritory_url.png)

    a. 在 [hello**識別碼**文字方塊中，輸入 URL，使用下列模式的 hello:`https://apps.easyterritory.com/<tenant id>/dev/`

    b. 在 [hello**回覆 URL**文字方塊中，輸入 URL，使用下列模式的 hello:`https://apps.easyterritory.com/<tenant id>/dev/authservices/acs`
...
4. 請檢查**顯示進階的 URL 設定**並執行下列步驟，如果您想 tooconfigure hello 應用程式中的 hello **SP**初始模式：
    ...
    在 [hello**登入 URL**文字方塊中，輸入 URL，使用下列模式的 hello:`https://<company name>.easyterritory.com/`
     
    > [!NOTE] 
    > 這些都不是真正的值。 更新這些值與實際的 hello，識別項、 回覆 URL 和登入 URL。 請連絡[EasyTerritory 用戶端支援小組](mailto:sales@easyterritory.com)tooget 這些值。 
...
7. tooconfigure 單一登入上**EasyTerritory**端，您需要下載 toosend hello**中繼資料 XML**太[EasyTerritory 支援小組](mailto:sales@easyterritory.com)。 設定此設定 toohave hello SAML SSO 連線兩端上正確設定這些欄位。
...
### <a name="create-a-easyterritory-test-user"></a>建立 EasyTerritory 測試使用者

在本節中，您要在 EasyTerritory 中建立名為 Britta Simon 的使用者。 請使用[EasyTerritory 支援小組](mailto:sales@easyterritory.com)tooadd hello hello EasyTerritory 平台的使用者。

### <a name="assign-hello-azure-ad-test-user"></a>指派給 Azure AD hello 測試使用者

在本節中，您可以授與存取 tooEasyTerritory 啟用許 Simon toouse Azure 單一登入。

![指派 hello 使用者角色][200] 

**tooassign 許 Simon tooEasyTerritory，執行下列步驟的 hello:**

1. 在 hello Azure 入口網站，開啟 hello 應用程式檢視，然後導覽 toohello 目錄檢視，並跳過**企業應用程式**然後按一下 [**所有應用程式**。

    ![指派使用者][201] 

2. 在 [hello] 應用程式清單中，選取**EasyTerritory**。
...
當您按一下 hello EasyTerritory 磚 hello 存取面板中的時，您應該取得自動登入 tooyour EasyTerritory 應用程式。
