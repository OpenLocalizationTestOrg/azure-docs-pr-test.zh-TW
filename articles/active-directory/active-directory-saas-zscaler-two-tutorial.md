...
在此教學課程中，您學會如何 toointegrate Zscaler Two 與 Azure Active Directory (Azure AD)。

整合與 Azure AD 的 Zscaler Two 可以提供下列優點 hello:

- 您可以控制存取 tooZscaler 兩個 Azure AD 中
- 您可以啟用您的使用者 tooautomatically get 登入 tooZscaler 兩個 （單一登入） 具有其 Azure AD 帳戶
...
tooconfigure Azure AD 與 Zscaler Two 整合，您需要下列項目 hello:

- Azure AD 訂用帳戶
- 已啟用 Zscaler Two 單一登入的訂用帳戶
...
1. 加入從 hello 組件庫的 Zscaler Two
2. 設定並測試 Azure AD 單一登入

## <a name="adding-zscaler-two-from-hello-gallery"></a>加入從 hello 組件庫的 Zscaler Two
tooconfigure hello 整合 Zscaler Two 到 Azure AD，您需要 tooadd Zscaler Two hello 圖庫 tooyour 清單中的受管理的 SaaS 應用程式。

**tooadd Zscaler Two 從 hello 組件庫中，執行下列步驟的 hello:**
...
4. 在 [hello] 搜尋方塊中，輸入**Zscaler Two**。

    ![建立 Azure AD 測試使用者](./media/active-directory-saas-zscaler-two-tutorial/tutorial_zscalertwo_search.png)

5. 在 hello 結果 窗格中，選取  **Zscaler Two**，然後按一下**新增**按鈕 tooadd hello 應用程式。

    ![建立 Azure AD 測試使用者](./media/active-directory-saas-zscaler-two-tutorial/tutorial_zscalertwo_addfromgallery.png)

##  <a name="configuring-and-testing-azure-ad-single-sign-on"></a>設定並測試 Azure AD 單一登入
在本節中，您會以名為 "Britta Simon" 的測試使用者身分，設定及測試與 Zscaler Two 搭配運作的 Azure AD 單一登入。

單一登入 toowork，Azure AD 需要 tooknow hello 的對等項目的使用者中 Zscaler Two 是 tooa 使用者在 Azure AD 中。 換句話說，Azure AD 使用者與 hello Zscaler Two 中相關的使用者之間的連結關聯性需要 toobe 建立。

在 Zscaler Two 中，將指派的 hello hello 值**使用者名稱**做為 hello hello 值的 Azure AD 中**Username** tooestablish hello 連結關聯性。

tooconfigure 及測試 Azure AD 單一登入 Zscaler Two，您必須遵循的建置組塊 toocomplete hello:
...
4. **[建立測試使用者 Zscaler Two](#creating-a-zscaler-two-test-user)**  -toohave 許 Simon 中 Zscaler Two 連結的 toohello Azure AD 使用者表示法的對應項目。
...
在本節中，您可以啟用 Azure AD 單一登入 hello Azure 入口網站中，並設定單一登入 Zscaler Two 應用程式中。

**tooconfigure Azure AD 單一登入 Zscaler Two 中，執行下列步驟的 hello:**

1. 在 Azure 入口網站上 hello hello **Zscaler Two**應用程式整合頁面上，按一下 **單一登入**。
...
3. 在 hello **Zscaler 兩個網域和 Url**區段中，執行下列步驟的 hello:

    ![設定單一登入](./media/active-directory-saas-zscaler-two-tutorial/tutorial_zscalertwo_url.png)

   在 hello 登入 URL 文字方塊中，輸入您的使用者 」 toosign 上 ZScaler Two 應用程式 tooyour 用 hello URL。

    > [!NOTE] 
    > 具有這個值與 hello tooupdate 實際的登入 URL。 請連絡[Zscaler 兩個用戶端支援小組](https://www.zscaler.com/company/contact)tooget 這些值。
...
6. 在 hello **Zscaler 兩個組態**區段中，按一下**設定 Zscaler Two** tooopen**設定登入**視窗。 複製 hello **SAML 單一登入服務 URL**從 hello**快速參考 > 一節。**

    ![設定單一登入](./media/active-directory-saas-zscaler-two-tutorial/tutorial_zscalertwo_configure.png) 

7. 在不同的網頁瀏覽器視窗中，系統管理員身分登入 tooyour ZScaler Two 公司網站。
...
5. 在 hello Proxy 伺服器區段中，執行下列步驟的 hello:   
    ...
    b. 在 [hello 位址] 文字方塊中輸入**gateway.zscalertwo.net**。
...
### <a name="creating-a-zscaler-two-test-user"></a>建立 Zscaler Two 測試使用者

tooenable Azure AD 使用者 toolog 中 tooZScaler 兩個，它們必須已佈建的 tooZScaler 兩個。 中的 ZScaler Two hello 案例中，佈建須手動進行。

### <a name="tooconfigure-user-provisioning-perform-hello-following-steps"></a>tooconfigure 使用者佈建，執行下列步驟的 hello:

1. 登入 tooyour **Zscaler Two**租用戶。
...
3. 按一下 [使用者管理] 。   
     ...
     ![新增](./media/active-directory-saas-zscaler-two-tutorial/ic781036.png "新增")
...
> [!NOTE]
> 您可以使用任何其他 ZScaler Two 使用者帳戶建立工具或 Api 提供 ZScaler Two tooprovision Azure AD 使用者帳戶。

### <a name="assigning-hello-azure-ad-test-user"></a>指派 hello Azure AD 的測試使用者

在本節中，您可以授與存取 tooZscaler 兩個啟用許 Simon toouse Azure 單一登入。

![指派使用者][200] 

**tooassign 許 Simon tooZscaler 二時，執行下列步驟的 hello:**

1. 在 hello Azure 入口網站，開啟 hello 應用程式檢視，然後導覽 toohello 目錄檢視，並跳過**企業應用程式**然後按一下 **所有應用程式**。

    ![指派使用者][201] 

2. 在 [hello] 應用程式清單中，選取**Zscaler Two**。
...
當您按一下的 hello Zscaler Two 磚 hello 存取面板中時，您應該取得自動登入 tooyour Zscaler Two 應用程式。
