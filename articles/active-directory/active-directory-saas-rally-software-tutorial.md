...
在此教學課程中，您學會如何 toointegrate Rally Software 與 Azure Active Directory (Azure AD)。

整合 Azure AD 與 Rally Software 可以提供下列優點 hello:

- 您可以控制存取 tooRally 軟體的 Azure AD 中。
- 您可以啟用您的使用者 tooautomatically get 登入 tooRally （單一登入） 的軟體與他們的 Azure AD 帳戶。
...
tooconfigure Azure AD 與 Rally Software 的整合，您需要下列項目 hello:

- Azure AD 訂用帳戶
- 已啟用 Rally Software 單一登入的訂用帳戶
...
1. 從 hello 圖庫加入 Rally Software
2. 設定並測試 Azure AD 單一登入

## <a name="adding-rally-software-from-hello-gallery"></a>從 hello 圖庫加入 Rally Software
tooconfigure hello 整合 Rally Software 的 Azure AD，您需要從受管理的 SaaS 應用程式的 hello 圖庫 tooyour 清單 tooadd Rally Software。

**tooadd Rally Software 從 hello 組件庫中，執行下列步驟的 hello:**
...
4. 在 [hello] 搜尋方塊中，輸入**Rally Software**，選取**Rally Software**然後按一下 [從結果面板**新增**按鈕 tooadd hello 應用程式。
...
在本節中，您會以名為 "Britta Simon" 的測試使用者為基礎，設定及測試與 Rally Software 搭配運作的 Azure AD 單一登入。

單一登入 toowork，Azure AD 需要 tooknow Rally Software 中的 hello 對等項目的使用者是 tooa 使用者在 Azure AD 中。 換句話說，Azure AD 使用者與 Rally Software 中的 hello 相關的使用者之間的連結關聯性需要 toobe 建立。

在 Rally Software，將指派的 hello hello 值**使用者名稱**做為 hello hello 值的 Azure AD 中**Username** tooestablish hello 連結關聯性。

tooconfigure 及測試 Azure AD 單一登入 Rally Software，您必須遵循的建置組塊 toocomplete hello:
...
3. **[建立測試使用者 Rally Software](#create-a-rally-software-test-user) ** -toohave 許 Simon 是連結的 toohello Azure AD 使用者表示法的 Rally Software 中對應項目。
...
在本節中，您可以啟用 Azure AD 單一登入 hello Azure 入口網站中，並設定單一登入 Rally Software 應用程式中。

**tooconfigure Azure AD 單一登入 Rally Software，執行下列步驟的 hello:**

1. 在 Azure 入口網站上 hello hello **Rally Software**應用程式整合頁面上，按一下 [**單一登入**。
...
3. 在 [hello **Rally 軟體網域和 Url**區段中，執行下列步驟的 hello:

    ![Rally Software 網域與 URL 單一登入資訊](./media/active-directory-saas-rally-software-tutorial/tutorial_rallysoftware_url.png)

    a. 在 [hello**登入 URL**文字方塊中，輸入 URL，使用下列模式的 hello:`https://<tenant-name>.rally.com`

    b. 在 [hello**識別碼**文字方塊中，輸入 URL，使用下列模式的 hello:`https://<tenant-name>.rally.com`

    > [!NOTE] 
    > 這些都不是真正的值。 更新這些值與實際的 hello 登入 URL 和識別項。 請連絡[Rally 軟體用戶端支援小組](https://help.rallydev.com/)tooget 這些值。 
...
6. 在 [hello **Rally 軟體組態**區段中，按一下**設定 Rally Software** tooopen**設定登入**視窗。 複製 hello**登出 URL 和 SAML 實體識別碼**從 hello**快速參考 > 一節。**

    ![Rally Software 設定](./media/active-directory-saas-rally-software-tutorial/tutorial_rallysoftware_configure.png) 

7. 登入 tooyour **Rally Software**租用戶。

8. 在 hello hello 上方的工具列中按一下**安裝**，然後選取**訂用帳戶**。
   
    ![訂用帳戶](./media/active-directory-saas-rally-software-tutorial/ic769531.png "訂用帳戶")

9. 按一下 hello**動作**] 按鈕。 選取**編輯訂用帳戶**在 hello 右上方的 hello 工具列。

10. 在 [hello**訂用帳戶**對話方塊頁面上，執行下列步驟，hello，然後按一下 [**儲存並關閉**:
   
    ![驗證](./media/active-directory-saas-rally-software-tutorial/ic769542.png "驗證")
   
    a. 從 [驗證] 下拉式清單中選取 [Rally 或 SSO 驗證]。

    b. 在 [hello**身分識別提供者 URL**文字方塊中，貼上 hello 值**SAML 實體識別碼**，從 Azure 入口網站複製的。 

    c. 在 [hello **SSO 登出**文字方塊中，貼上 hello 值**登出 URL**，從 Azure 入口網站複製的。
...
Azure AD 使用者 toobe 無法 toosign 中，它們必須已佈建的 toohello Rally Software 的應用程式使用他們的 Azure Active Directory 使用者名稱。

**tooconfigure 使用者佈建，執行下列步驟的 hello:**

1. 登入 tooyour Rally Software 租用戶。

2. 跳過**安裝\>使用者**，然後按一下 [ **+ 加入新**。
   
    ![使用者](./media/active-directory-saas-rally-software-tutorial/ic781039.png "使用者")

3. 在 hello 新使用者] 文字方塊中輸入 hello 名稱，然後按一下**新增詳細資料**。

4. 在 [hello **Create User**區段中，執行下列步驟的 hello:
   
    ![建立使用者](./media/active-directory-saas-rally-software-tutorial/ic781040.png "建立使用者")

    a. 在 [hello**使用者名稱**文字方塊中，像是使用者的型別 hello 名稱**Brittsimon**。
   
    b. 在**電子郵件地址**文字方塊中，輸入 hello 電子郵件的使用者，例如** brittasimon@contoso.com **。

    c. 在**名字**文字方塊中，輸入 hello 名字，例如使用者的**許**。

    d. 在**姓氏**文字方塊中，輸入 hello 姓氏的使用者，例如**Simon**。

    e. 按一下 [儲存並關閉]。

   >[!NOTE]
   >您可以使用任何其他 Rally Software 使用者帳戶建立工具或 Api 提供 Rally Software tooprovision Azure AD 使用者帳戶。

### <a name="assign-hello-azure-ad-test-user"></a>指派給 Azure AD hello 測試使用者

在本節中，以啟用許 Simon toouse Azure 單一登入授與存取 tooRally 軟體。

![指派 hello 使用者角色][200] 

**tooassign 許 Simon tooRally 軟體，執行下列步驟的 hello:**

1. 在 hello Azure 入口網站，開啟 hello 應用程式檢視，然後導覽 toohello 目錄檢視，並跳過**企業應用程式**然後按一下 [**所有應用程式**。

    ![指派使用者][201] 

2. 在 [hello] 應用程式清單中，選取**Rally Software**。
...
當您按一下 hello Rally Software 磚 hello 存取面板中的時，您應該取得自動登入 tooyour Rally Software 的應用程式。
