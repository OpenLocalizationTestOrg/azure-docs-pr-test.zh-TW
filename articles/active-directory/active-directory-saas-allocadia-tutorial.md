...
在此教學課程中，您學會如何 toointegrate Allocadia 與 Azure Active Directory (Azure AD)。

與 Azure AD 整合 Allocadia 可以提供下列優點 hello:

- 您可以控制存取 tooAllocadia Azure AD 中
- 您可以啟用您的使用者 tooautomatically get 登入 tooAllocadia （單一登入） 具有其 Azure AD 帳戶
...
tooconfigure Allocadia 與 Azure AD 整合，您需要下列項目 hello:
...
1. 從 hello 圖庫加入 Allocadia
2. 設定並測試 Azure AD 單一登入

## <a name="adding-allocadia-from-hello-gallery"></a>從 hello 圖庫加入 Allocadia
tooconfigure hello 整合 Allocadia 到 Azure AD，您需要 tooadd Allocadia hello 圖庫 tooyour 清單中的受管理的 SaaS 應用程式。

**tooadd Allocadia 從 hello 組件庫中，執行下列步驟的 hello:**
...
單一登入 toowork，Azure AD 需要 tooknow hello 的對等項目的使用者中 Allocadia 是 tooa 使用者在 Azure AD 中。 換句話說，Azure AD 使用者與 hello Allocadia 中相關的使用者之間的連結關聯性需要 toobe 建立。

Allocadia 中, 指派的 hello hello 值**使用者名**做為 hello hello 值的 Azure AD 中**Username** tooestablish hello 連結關聯性。

tooconfigure 及 Allocadia 與 Azure AD 單一登入的測試，您必須遵循的建置組塊 toocomplete hello:

1. **[設定 Azure AD 單一登入](#configuring-azure-ad-single-sign-on)** -tooenable 使用者 toouse 這項功能。
2. **[建立 Azure AD 測試使用者](#creating-an-azure-ad-test-user)** -tootest Azure AD 單一登入與許 Simon。
3. **[建立測試使用者 Allocadia](#creating-an-allocadia-test-user) ** -toohave 許 Simon Allocadia 所連結的 toohello Azure AD 使用者表示法中對應項目。
4. **[指派 hello Azure AD 的測試使用者](#assigning-the-azure-ad-test-user)** -tooenable 許 Simon toouse Azure AD 單一登入。
5. **[測試單一登入](#testing-single-sign-on)** -tooverify 是否 hello 組態工作。

### <a name="configuring-azure-ad-single-sign-on"></a>設定 Azure AD 單一登入

在本節中，您可以啟用 Azure AD 單一登入 hello Azure 入口網站中，並 Allocadia 應用程式中設定單一登入。

**tooconfigure Azure AD 單一登入與 Allocadia，執行下列步驟的 hello:**

1. 在 Azure 入口網站上 hello hello **Allocadia**應用程式整合頁面上，按一下 [**單一登入**。
...
3. 在 [hello **Allocadia 網域和 Url**區段中，執行下列步驟的 hello:
    ...
    > [!NOTE] 
    > 這些都不是真正的值。 以 hello 實際的識別項和回覆 URL 更新這些值。 請連絡[Allocadia 支援小組](mailTo:support@allocadia.com)tooget 這些值。

4. Allocadia 應用程式預期 hello SAML 判斷提示，以特定格式。 設定下列宣告為此應用程式的 hello。 您可以從 hello 管理 hello 這些屬性的值"**使用者屬性**」 一節，在應用程式整合頁面上。 hello 下列螢幕擷取畫面顯示此組態範例。 
...
8. tooconfigure 單一登入上**Allocadia**端，您需要下載 toosend hello**中繼資料 XML**太[Allocadia 支援小組](mailTo:support@allocadia.com)。 設定此設定 toohave hello SAML SSO 連線兩端上正確設定這些欄位。
...
應用程式支援 Just In Time 使用者佈建。 在驗證後使用者會在 hello 應用程式中自動建立。

### <a name="assigning-hello-azure-ad-test-user"></a>指派 hello Azure AD 的測試使用者

在本節中，您可以授與存取 tooAllocadia 啟用許 Simon toouse Azure 單一登入。

![指派使用者][200] 

**tooassign 許 Simon tooAllocadia，執行下列步驟的 hello:**

1. 在 hello Azure 入口網站，開啟 hello 應用程式檢視，然後導覽 toohello 目錄檢視，並跳過**企業應用程式**然後按一下 [**所有應用程式**。

    ![指派使用者][201] 

2. 在 [hello] 應用程式清單中，選取**Allocadia**。
...
當您按一下 hello Allocadia 磚 hello 存取面板中的時，您應該取得自動登入 tooyour Allocadia 應用程式。
