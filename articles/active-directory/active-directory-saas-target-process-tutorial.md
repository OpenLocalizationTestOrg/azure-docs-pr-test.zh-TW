...
在此教學課程中，您學會如何 toointegrate TargetProcess 與 Azure Active Directory (Azure AD)。

與 Azure AD 整合 TargetProcess 可以提供下列優點 hello:

- 您可以控制存取 tooTargetProcess Azure AD 中
- 您可以啟用您的使用者 tooautomatically get 登入 tooTargetProcess （單一登入） 具有其 Azure AD 帳戶
...
tooconfigure TargetProcess 與 Azure AD 整合，您需要下列項目 hello:
...
1. 從 hello 圖庫新增 TargetProcess
2. 設定和測試 Azure AD 單一登入

## <a name="add-targetprocess-from-hello-gallery"></a>從 hello 圖庫新增 TargetProcess
tooconfigure hello 整合 TargetProcess 到 Azure AD，您需要 tooadd TargetProcess hello 圖庫 tooyour 清單中的受管理的 SaaS 應用程式。

**tooadd TargetProcess 從 hello 組件庫中，執行下列步驟的 hello:**
...
4. 在 hello 搜尋方塊中，輸入**TargetProcess**，選取**TargetProcess**然後按一下 從結果面板**新增**按鈕 tooadd hello 應用程式。
...
單一登入 toowork，Azure AD 需要 tooknow hello 的對等項目的使用者中 TargetProcess 是 tooa 使用者在 Azure AD 中。 換句話說，Azure AD 使用者與 hello TargetProcess 中相關的使用者之間的連結關聯性需要 toobe 建立。

TargetProcess 中, 指派的 hello hello 值**使用者名**做為 hello hello 值的 Azure AD 中**Username** tooestablish hello 連結關聯性。

tooconfigure 及 TargetProcess 與 Azure AD 單一登入的測試，您必須遵循的建置組塊 toocomplete hello:
...
3. **[建立測試使用者 TargetProcess](#create-a-targetprocess-test-user)**  -toohave 許 Simon TargetProcess 所連結的 toohello Azure AD 使用者表示法中對應項目。
...
在本節中，您可以啟用 Azure AD 單一登入 hello Azure 入口網站中，並 TargetProcess 應用程式中設定單一登入。

**tooconfigure Azure AD 單一登入與 TargetProcess，執行下列步驟的 hello:**

1. 在 Azure 入口網站上 hello hello **TargetProcess**應用程式整合頁面上，按一下 **單一登入**。
...
3. 在 hello **TargetProcess 網域和 Url**區段中，執行下列步驟的 hello:

    ![[TargetProcess 網域與 URL] 區段](./media/active-directory-saas-target-process-tutorial/tutorial_target-process_url.png)

    a. 在 hello**登入 URL**文字方塊中，輸入 URL，使用下列模式的 hello:`https://<subdomain>.tpondemand.com/`

    b. 在 hello**識別碼**文字方塊中，輸入 URL，使用下列模式的 hello:`https://<subdomain>.tpondemand.com/`

    > [!NOTE] 
    > 這些都不是真正的值。 更新這些值與實際的 hello 登入 URL 和識別項。 請連絡[TargetProcess 用戶端支援小組](mailto:support@targetprocess.com)tooget 這些值。 
...
6. 在 hello **TargetProcess 組態**區段中，按一下**設定 TargetProcess** tooopen**設定登入**視窗。 複製 hello **SAML 單一登入服務 URL**從 hello**快速參考 > 一節。**

    ![[TargetProcess 組態] 區段](./media/active-directory-saas-target-process-tutorial/tutorial_target-process_configure.png) 

7. 登入 tooyour TargetProcess 應用程式，以系統管理員。

8. 在 hello 最上層顯示 hello 功能表上，按一下**安裝**。
...
11. 在 hello 單一登入設定 對話方塊，執行下列步驟的 hello:
    ...
    c. 開啟 [記事本]，複製 hello 內容，您所下載的憑證，然後將它貼到 hello**憑證**文字方塊。
...
hello 本節目標在於 toocreate TargetProcess 中呼叫許 Simon 的使用者。
...
在本節中，您可以授與存取 tooTargetProcess 啟用許 Simon toouse Azure 單一登入。

![指派使用者][200] 

**tooassign 許 Simon tooTargetProcess，執行下列步驟的 hello:**

1. 在 hello Azure 入口網站，開啟 hello 應用程式檢視，然後導覽 toohello 目錄檢視，並跳過**企業應用程式**然後按一下 **所有應用程式**。

    ![指派使用者][201] 

2. 在 [hello] 應用程式清單中，選取**TargetProcess**。
...
當您按一下的 hello TargetProcess 磚 hello 存取面板中時，您應該取得自動登入 tooyour TargetProcess 應用程式。 如需 hello 存取面板的詳細資訊，請參閱[簡介 toohello 存取面板](active-directory-saas-access-panel-introduction.md)。
