...
在此教學課程中，您學會如何 toointegrate 也就是向 Azure Active Directory (Azure AD)。

也就整合與 Azure AD 可以提供下列優點 hello:

- 您可以控制存取 tooNamely Azure AD 中
- 您可以啟用您的使用者 tooautomatically get 登入 tooNamely （單一登入） 具有其 Azure AD 帳戶
...
tooconfigure Azure AD 整合與那就是，您需要下列項目 hello:

- Azure AD 訂用帳戶
- 已啟用 Namely 單一登入的訂用帳戶
...
## <a name="adding-namely-from-hello-gallery"></a>也就加入從 hello 組件庫
tooconfigure hello 整合至 Azure AD 也就是，您需要 tooadd 也就是從受管理的 SaaS 應用程式的 hello 圖庫 tooyour 清單。

**tooadd 也就是從 hello 圖庫中，執行下列步驟的 hello:**
...
4. 在 [hello] 搜尋方塊中，輸入**即**。

    ![建立 Azure AD 測試使用者](./media/active-directory-saas-namely-tutorial/tutorial_namely_search.png)

5. 在 hello 結果 窗格中，選取 **也就是**，然後按一下**新增**按鈕 tooadd hello 應用程式。

    ![建立 Azure AD 測試使用者](./media/active-directory-saas-namely-tutorial/tutorial_namely_addfromgallery.png)

##  <a name="configuring-and-testing-azure-ad-single-sign-on"></a>設定並測試 Azure AD 單一登入
在本節中，您會以名為 "Britta Simon" 的測試使用者身分，使用 Namely 設定及測試 Azure AD 單一登入。

單一登入 toowork，Azure AD 需要 tooknow hello 對應項目中的使用者也就是為 tooa 使用者在 Azure AD 中。 換句話說，Azure AD 使用者與 hello 中相關的使用者之間的連結關聯性也就是需要 toobe 建立。

中也就是指派的 hello hello 值**使用者名**做為 hello hello 值的 Azure AD 中**Username** tooestablish hello 連結關聯性。

tooconfigure 和測試 Azure AD 單一登入與那就是，您需要遵循的建置組塊 toocomplete hello:

1. **[設定 Azure AD 單一登入](#configuring-azure-ad-single-sign-on)** -tooenable 使用者 toouse 這項功能。
2. **[建立 Azure AD 測試使用者](#creating-an-azure-ad-test-user)** -tootest Azure AD 單一登入與許 Simon。
3. **[建立也就是測試使用者](#creating-a-namely-test-user)** -toohave 也就是也就是連結 toohello Azure AD 使用者表示法的許 Simon 的對應項目。
4. **[指派 hello Azure AD 的測試使用者](#assigning-the-azure-ad-test-user)** -tooenable 許 Simon toouse Azure AD 單一登入。
5. **[測試單一登入](#testing-single-sign-on)** -tooverify 是否 hello 組態工作。

### <a name="configuring-azure-ad-single-sign-on"></a>設定 Azure AD 單一登入

在本節中，您可以啟用 Azure AD 單一登入 hello Azure 入口網站中，並設定單一登入，您也就是應用程式。

**tooconfigure Azure AD 單一登入也就是執行下列步驟的 hello:**

1. 在 Azure 入口網站上 hello hello**即**應用程式整合頁面上，按一下 **單一登入**。
...
3. 在 hello**也就是網域和 Url**區段中，執行下列步驟的 hello:

    ![設定單一登入](./media/active-directory-saas-namely-tutorial/tutorial_namely_url.png)

    a. 在 hello**登入 URL**文字方塊中，輸入 URL，使用下列模式的 hello:`https://<subdomain>.namely.com`

    b. 在 hello**識別碼**文字方塊中，輸入 URL，使用下列模式的 hello:`https://<subdomain>.namely.com/saml/metadata`

    > [!NOTE] 
    > 這些都不是真正的值。 更新這些值與實際的 hello 登入 URL 和識別項。 請連絡[也就是用戶端支援小組](https://www.namely.com/contact/)tooget 這些值。 
...
6. 在 hello**也就是組態**區段中，按一下**即設定**tooopen**設定登入**視窗。 複製 hello **SAML 單一登入服務 URL**從 hello**快速參考 > 一節。**

    ![設定單一登入](./media/active-directory-saas-namely-tutorial/tutorial_namely_configure.png) 

7. 在另一個瀏覽器視窗中，身分登入 tooyour 也就是公司網站的系統管理員。

8. 在 hello hello 上方的工具列中按一下**公司**。
...
11. 在 hello **SAML 設定**頁面上，執行下列步驟的 hello:
    ...
    b. 在 hello**身分識別提供者 SSO url**文字方塊中，貼上 hello 值**SAML 單一登入服務 URL**，從 Azure 入口網站複製的。
    
    c. 開啟 [記事本]，複製 hello 內容，您所下載的憑證，然後將它貼到 hello**身分識別提供者憑證**文字方塊。
...
### <a name="creating-a-namely-test-user"></a>建立 Namely 測試使用者

hello 本節目標在於 toocreate 中也就是呼叫許 Simon 的使用者。

**toocreate 使用者，也就是呼叫許 Simon 執行下列步驟的 hello:**

1. 登入 tooyour 也就是公司系統管理員身分的站台。

2. 在 hello hello 上方的工具列中按一下**人員**。
...
5. 在 [hello**新增人員**] 對話方塊中，執行下列步驟的 hello:
    ...
    c. 在 hello**電子郵件**文字方塊中，型別 hello**電子郵件地址**BrittaSimon。

    d. 按一下 [儲存] 。

### <a name="assigning-hello-azure-ad-test-user"></a>指派 hello Azure AD 的測試使用者

在本節中，您可以授與存取 tooNamely 啟用許 Simon toouse Azure 單一登入。

![指派使用者][200] 

**tooassign 許 Simon tooNamely，執行下列步驟的 hello:**

1. 在 hello Azure 入口網站，開啟 hello 應用程式檢視，然後導覽 toohello 目錄檢視，並跳過**企業應用程式**然後按一下 **所有應用程式**。

    ![指派使用者][201] 

2. 在 [hello] 應用程式清單中，選取**即**。
...
當您按一下 hello 也就是在 hello 存取面板 磚中，您應該取得自動登入 tooyour 也就是應用程式
