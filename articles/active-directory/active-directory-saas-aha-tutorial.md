...
在此教學課程中，您學會如何 toointegrate Aha ！ 與 Azure Active Directory (Azure AD) 整合。

將 Aha! 使用 Azure AD 提供下列優點 hello:

- 您可以控制存取 tooAha Azure AD 中 ！
- 您可以啟用您的使用者 tooautomatically get 登入 tooAha ！ (單一登入)
...
tooconfigure Azure AD 整合與 Aha ！，您需要下列項目 hello:
...
1. 新增 Aha! 從 hello 組件庫
2. 設定並測試 Azure AD 單一登入

## <a name="adding-aha-from-hello-gallery"></a>新增 Aha! 從 hello 組件庫
tooconfigure hello 整合的 Aha ！ 到 Azure AD，您需要 tooadd Aha ！ 從受管理的 SaaS 應用程式的 hello 圖庫 tooyour 清單。

**tooadd Aha ！從 hello 圖庫中，執行下列步驟的 hello:**
...
單一登入 toowork，Azure AD 需要 tooknow hello 的對等項目的使用者中 Aha ！ 在 Azure AD 中為 tooa 使用者。 換句話說，Azure AD 使用者與 hello 之間的連結關聯性相關的使用者在 Aha ！ 需要 toobe 建立。

在 Aha ！，hello hello 值指派**使用者名稱**做為 hello hello 值的 Azure AD 中**Username** tooestablish hello 連結關聯性。

tooconfigure 和測試 Azure AD 單一登入與 Aha ！，您需要遵循的建置組塊 toocomplete hello:

1. **[設定 Azure AD 單一登入](#configuring-azure-ad-single-sign-on)** -tooenable 使用者 toouse 這項功能。
2. **[建立 Azure AD 測試使用者](#creating-an-azure-ad-test-user)** -tootest Azure AD 單一登入與許 Simon。
3. **[建立 Aha ！測試使用者](#creating-an-aha-test-user)** -toohave 對應項目許 Simon 中 Aha ！ 這是連結的 toohello Azure AD 使用者表示法。
4. **[指派 hello Azure AD 的測試使用者](#assigning-the-azure-ad-test-user)** -tooenable 許 Simon toouse Azure AD 單一登入。
5. **[測試單一登入](#testing-single-sign-on)** -tooverify 是否 hello 組態工作。

### <a name="configuring-azure-ad-single-sign-on"></a>設定 Azure AD 單一登入

在本節中，您可以啟用 Azure AD 單一登入 hello Azure 入口網站中，並設定單一登入您的 Aha ！ 應用程式中設定單一登入。

**Azure AD 單一登入的 tooconfigure 與 Aha ！，執行下列步驟的 hello:**

1. 在 Azure 入口網站上 hello hello **Aha ！** 應用程式整合頁面上，按一下 [單一登入]。
...
3. 在 hello **Aha ！網域和 Url**區段中，執行下列步驟的 hello:

    ![設定單一登入](./media/active-directory-saas-aha-tutorial/tutorial_aha_url.png)

    a. 在 hello**登入 URL**文字方塊中，輸入 URL，使用下列模式的 hello:`https://<companyname>.aha.io/session/new`

    b. 在 hello**識別碼**文字方塊中，輸入 URL，使用下列模式的 hello:`https://<companyname>.aha.io`

    > [!NOTE] 
    > 這些都不是真正的值。 更新這些值與實際的 hello 登入 URL 和識別項。 請連絡 [Aha!用戶端支援小組](https://www.aha.io/company/contact)tooget 這些值。 
...
6. 在不同的網頁瀏覽器視窗中，登入 tooyour Aha ！ 公司網站。
...
11. 在 hello**單一登入**組態頁面上，執行下列步驟的 hello:
    ...
    ![單一登入](./media/active-directory-saas-aha-tutorial/IC798954.png "單一登入")
       ...
       c. tooupload 您下載的中繼資料檔案中，按一下 **瀏覽**。
...
在 tooAha tooenable Azure AD 使用者 toolog ！，您必須佈建到 Aha ！。  

在 hello 案例中的 Aha ！，佈建是自動化的工作。 沒有您適用的動作項目。

使用者會自動建立必要 hello 第一個單一登入嘗試期間。

>[!NOTE]
>您可以使用任何其他的 Aha! 使用者帳戶建立工具或 Aha! tooprovision AAD 使用者帳戶。

### <a name="assigning-hello-azure-ad-test-user"></a>指派 hello Azure AD 的測試使用者

在本節中，以啟用許 Simon toouse Azure 單一登入授與存取 tooAha ！。

![指派使用者][200] 

**tooassign 許 Simon tooAha ！，執行下列步驟的 hello:**

1. 在 hello Azure 入口網站，開啟 hello 應用程式檢視，然後導覽 toohello 目錄檢視，並跳過**企業應用程式**然後按一下 **所有應用程式**。

    ![指派使用者][201] 

2. 在 [hello] 應用程式清單中，選取**Aha ！**。
