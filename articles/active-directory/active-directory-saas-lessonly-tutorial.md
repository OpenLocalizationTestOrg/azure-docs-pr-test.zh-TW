...
在此教學課程中，您學會如何 toointegrate Lesson.ly 與 Azure Active Directory (Azure AD)。

與 Azure AD 整合 Lesson.ly 可以提供下列優點 hello:

- 您可以控制存取 tooLesson.ly Azure AD 中
- 您可以啟用您的使用者 tooautomatically get 登入 tooLesson.ly （單一登入） 具有其 Azure AD 帳戶
...
tooconfigure Lesson.ly 與 Azure AD 整合，您需要下列項目 hello:
...
1. 從 hello 圖庫加入 Lesson.ly
2. 設定並測試 Azure AD 單一登入

## <a name="adding-lessonly-from-hello-gallery"></a>從 hello 圖庫加入 Lesson.ly
tooconfigure hello 整合 Lesson.ly 到 Azure AD，您需要 tooadd Lesson.ly hello 圖庫 tooyour 清單中的受管理的 SaaS 應用程式。

**tooadd Lesson.ly 從 hello 組件庫中，執行下列步驟的 hello:**
...
單一登入 toowork，Azure AD 需要 tooknow hello 的對等項目的使用者中 Lesson.ly 是 tooa 使用者在 Azure AD 中。 換句話說，Azure AD 使用者與 hello Lesson.ly 中相關的使用者之間的連結關聯性需要 toobe 建立。

Lesson.ly 中, 指派的 hello hello 值**使用者名**做為 hello hello 值的 Azure AD 中**Username** tooestablish hello 連結關聯性。

tooconfigure 及 Lesson.ly 與 Azure AD 單一登入的測試，您必須遵循的建置組塊 toocomplete hello:

1. **[設定 Azure AD 單一登入](#configuring-azure-ad-single-sign-on)** -tooenable 使用者 toouse 這項功能。
2. **[建立 Azure AD 測試使用者](#creating-an-azure-ad-test-user)** -tootest Azure AD 單一登入與許 Simon。
3. **[建立測試使用者 Lesson.ly](#creating-a-lessonly-test-user)**  -toohave 許 Simon Lesson.ly 所連結的 toohello Azure AD 使用者表示法中對應項目。
4. **[指派 hello Azure AD 的測試使用者](#assigning-the-azure-ad-test-user)** -tooenable 許 Simon toouse Azure AD 單一登入。
5. **[測試單一登入](#testing-single-sign-on)** -tooverify 是否 hello 組態工作。

### <a name="configuring-azure-ad-single-sign-on"></a>設定 Azure AD 單一登入

在本節中，您可以啟用 Azure AD 單一登入 hello Azure 入口網站中，並 Lesson.ly 應用程式中設定單一登入。

**tooconfigure Azure AD 單一登入與 Lesson.ly，執行下列步驟的 hello:**

1. 在 Azure 入口網站上 hello hello **Lesson.ly**應用程式整合頁面上，按一下 **單一登入**。
...
3. 在 hello **Lesson.ly 網域和 Url**區段中，執行下列步驟的 hello:
    ...
    >[!NOTE]
    >當參考泛型名稱**companyname**需要 toobe 以實際名稱取代。
    ...
    > [!NOTE] 
    > 這些都不是真正的值。 更新這些值與實際的 hello 登入 URL 和識別項。 請連絡[Lesson.ly 用戶端支援小組](mailto:dev@lessonly.com)tooget 這些值。 
...
5. hello Lesson.ly 應用程式需要特定格式，這需要您 tooadd 自訂屬性對應 tooyour hello SAML 判斷提示**SAML 權杖屬性**configuration.hello 下列螢幕擷取畫面顯示的範例此。
...
8. 在 hello **Lesson.ly 組態**區段中，按一下**設定 Lesson.ly** tooopen**設定登入**視窗。 複製 hello**登出 URL、 SAML 實體識別碼、 和 SAML 單一登入服務 URL**從 hello**快速參考 > 一節。**

    ![設定單一登入](./media/active-directory-saas-lessonly-tutorial/tutorial_lesson.ly_configure.png)

9. tooconfigure 單一登入上**Lesson.ly**端，您需要下載 toosend hello **Certificate(Base64)**和**登出 URL、 SAML 實體識別碼、 和 SAML 單一登入服務 URL**太[Lesson.ly 支援小組](mailto:dev@lessonly.com)。
...
hello 本節目標在於 toocreate Lesson.ly 中呼叫許 Simon 的使用者。 Lesson.ly 支援預設啟用的 Just-In-Time 佈建。

在這一節沒有您需要進行的動作項目。 如果尚未存在，將會嘗試 tooaccess Lesson.ly 期間建立新使用者。

> [!NOTE]
> 若要手動 toocreate 使用者，您需要 toocontact hello [Lesson.ly 支援小組](mailto:dev@lessonly.com)。

### <a name="assigning-hello-azure-ad-test-user"></a>指派 hello Azure AD 的測試使用者

在本節中，您可以授與存取 tooLesson.ly 啟用許 Simon toouse Azure 單一登入。

![指派使用者][200] 

**tooassign 許 Simon tooLesson.ly，執行下列步驟的 hello:**

1. 在 hello Azure 入口網站，開啟 hello 應用程式檢視，然後導覽 toohello 目錄檢視，並跳過**企業應用程式**然後按一下 **所有應用程式**。

    ![指派使用者][201] 

2. 在 [hello] 應用程式清單中，選取**Lesson.ly**。
...
當您按一下 hello Lesson.ly 磚 hello 存取面板中的時，您應該取得自動登入 tooyour Lesson.ly 應用程式。
