...
在此教學課程中，您學會如何 toointegrate InTime 與 Azure Active Directory (Azure AD)。

與 Azure AD 整合 InTime 可以提供下列優點 hello:

- 您可以控制存取 tooInTime Azure AD 中。
- 您可以啟用您的使用者 tooautomatically get 登入 tooInTime （單一登入） 具有其 Azure AD 帳戶。
...
tooconfigure InTime 與 Azure AD 整合，您需要下列項目 hello:
...
1. 從 hello 圖庫加入 InTime
2. 設定並測試 Azure AD 單一登入

## <a name="adding-intime-from-hello-gallery"></a>從 hello 圖庫加入 InTime
tooconfigure hello 整合 InTime 到 Azure AD，您需要 tooadd InTime hello 圖庫 tooyour 清單中的受管理的 SaaS 應用程式。

**tooadd InTime 從 hello 組件庫中，執行下列步驟的 hello:**
...
4. 在 hello 搜尋方塊中，輸入**InTime**，選取**InTime**然後按一下 從結果面板**新增**按鈕 tooadd hello 應用程式。
...
單一登入 toowork，Azure AD 需要 tooknow hello 的對等項目的使用者中 InTime 是 tooa 使用者在 Azure AD 中。 換句話說，Azure AD 使用者與 hello InTime 中相關的使用者之間的連結關聯性需要 toobe 建立。

InTime 中, 指派的 hello hello 值**使用者名**做為 hello hello 值的 Azure AD 中**Username** tooestablish hello 連結關聯性。

tooconfigure 及 InTime 與 Azure AD 單一登入的測試，您必須遵循的建置組塊 toocomplete hello:
...
3. **[建立 InTime 測試使用者](#create-a-intime-test-user)** -toohave 許 Simon InTime 所連結的 toohello Azure AD 使用者表示法中對應項目。
...
在本節中，您可以啟用 Azure AD 單一登入 hello Azure 入口網站中，並 InTime 應用程式中設定單一登入。

**tooconfigure Azure AD 單一登入與 InTime，執行下列步驟的 hello:**

1. 在 Azure 入口網站上 hello hello **InTime**應用程式整合頁面上，按一下 **單一登入**。
...
3. 在 hello **InTime 網域和 Url**區段中，執行下列步驟的 hello:

    ![InTime 網域與 URL 單一登入資訊](./media/active-directory-saas-intime-tutorial/tutorial_intime_url.png)

    a. 在 hello**登入 URL**文字方塊中，型別 hello URL:`https://intime6.intimesoft.com/mytime/login/login.xhtml`

    b. 在 hello**識別碼**文字方塊中，型別 hello URL:`https://auth.intimesoft.com/auth/realms/master`
...
5. InTime 應用程式預期 hello SAML 判斷提示，以特定格式，這需要您 tooadd 自訂屬性對應 tooyour SAML token 屬性設定。 hello 下列螢幕擷取畫面會顯示這個範例。 hello 預設值是**使用者識別碼**是**user.userprincipalname**但 InTime 預期這個 toobe hello 使用者的電子郵件地址的對應。 您可以使用該**user.mail**屬性從 hello 清單或使用您組織的組態為基礎的 hello 適當的屬性值 
...
7. 在 hello **InTime 組態**區段中，按一下**InTime 設定**tooopen**設定登入**視窗。 複製 hello**登出 URL 和 SAML 單一登入服務 URL**從 hello**快速參考 > 一節。**

    ![InTime 設定](./media/active-directory-saas-intime-tutorial/tutorial_intime_configure.png) 

8. tooconfigure 單一登入上**InTime**端，您需要下載 toosend hello**中繼資料 XML**，**登出 URL 和 SAML 單一登入服務 URL**太[InTime 支援小組](mailto:hdollard@intimesoft.com)。 設定此設定 toohave hello SAML SSO 連線兩端上正確設定這些欄位。
...
在本節中，您要在 InTime 中建立名為 Britta Simon 的使用者。 使用[InTime 支援小組](mailto:hdollard@intimesoft.com)tooadd hello hello InTime 平台的使用者。 您必須先建立和啟動使用者，然後才能使用單一登入。

### <a name="assign-hello-azure-ad-test-user"></a>指派給 Azure AD hello 測試使用者

在本節中，您可以授與存取 tooInTime 啟用許 Simon toouse Azure 單一登入。

![指派 hello 使用者角色][200] 

**tooassign 許 Simon tooInTime，執行下列步驟的 hello:**

1. 在 hello Azure 入口網站，開啟 hello 應用程式檢視，然後導覽 toohello 目錄檢視，並跳過**企業應用程式**然後按一下 **所有應用程式**。

    ![指派使用者][201] 

2. 在 [hello] 應用程式清單中，選取**InTime**。
...
當您按一下 hello InTime 磚中的 hello 存取面板，您應該取得應用程式 InTime hello 登入頁面。 按一下 hello**登入**按鈕，則一系列的 IdPs 將會顯示在清單中的按鈕。 按一下**IDP 名稱**給定[InTime 支援小組](mailto:hdollard@intimesoft.com)toologin InTime 應用程式。 如需有關存取面板的詳細資訊，請參閱[簡介 toohello 存取面板](active-directory-saas-access-panel-introduction.md)。 
