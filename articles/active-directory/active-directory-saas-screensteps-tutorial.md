...
在此教學課程中，您學會如何 toointegrate ScreenSteps 與 Azure Active Directory (Azure AD)。

整合 Azure AD 與 ScreenSteps 可以提供下列優點 hello:

- 您可以控制存取 tooScreenSteps Azure AD 中。
- 您可以啟用您的使用者 tooautomatically get 登入 tooScreenSteps （單一登入） 具有其 Azure AD 帳戶。
...
tooconfigure Azure AD 與 ScreenSteps 的整合，您需要下列項目 hello:
...
1. 從 hello 圖庫加入 ScreenSteps
2. 設定並測試 Azure AD 單一登入

## <a name="adding-screensteps-from-hello-gallery"></a>從 hello 圖庫加入 ScreenSteps
tooconfigure hello 整合 ScreenSteps 的 Azure AD，您需要 tooadd ScreenSteps hello 圖庫 tooyour 清單中的受管理的 SaaS 應用程式。

**tooadd ScreenSteps 從 hello 組件庫中，執行下列步驟的 hello:**
...
4. 在 [hello] 搜尋方塊中，輸入**ScreenSteps**，選取**ScreenSteps**然後按一下 [從結果面板**新增**按鈕 tooadd hello 應用程式。
...
單一登入 toowork，Azure AD 需要 tooknow hello 的對等項目的使用者中 ScreenSteps 是 tooa 使用者在 Azure AD 中。 換句話說，Azure AD 使用者與 ScreenSteps 中的 hello 相關的使用者之間的連結關聯性需要 toobe 建立。

ScreenSteps 中, 指派的 hello hello 值**使用者名**做為 hello hello 值的 Azure AD 中**Username** tooestablish hello 連結關聯性。

tooconfigure 及 Azure AD 單一登入與 ScreenSteps 的測試，您必須遵循的建置組塊 toocomplete hello:
...
3. **[建立測試使用者 ScreenSteps](#create-a-screensteps-test-user) ** -toohave 許 Simon ScreenSteps 所連結的 toohello Azure AD 使用者表示法中對應項目。
...
在本節中，您可以啟用 Azure AD 單一登入 hello Azure 入口網站中，並設定單一登入 ScreenSteps 的應用程式中。

**tooconfigure Azure AD 單一登入與 ScreenSteps，執行下列步驟的 hello:**

1. 在 Azure 入口網站上 hello hello **ScreenSteps**應用程式整合頁面上，按一下 [**單一登入**。
...
3. 在 [hello **ScreenSteps 網域和 Url**區段中，執行下列步驟的 hello:

    ![ScreenSteps 網域及 URL 單一登入資訊](./media/active-directory-saas-screensteps-tutorial/tutorial_screensteps_url.png)

    在 [hello**登入 URL**文字方塊中，輸入 URL，使用下列模式的 hello:`https://<tenantname>.ScreenSteps.com`

    > [!NOTE] 
    > 這不是真實的值。 更新此值以 hello 實際登入 URL，在本教學課程稍後會說明。 
...
6. 在 [hello **ScreenSteps 組態**區段中，按一下**設定 ScreenSteps** tooopen**設定登入**視窗。 複製 hello**登出 URL 和 SAML 單一登入服務 URL**從 hello**快速參考 > 一節。**

    ![ScreenSteps 設定](./media/active-directory-saas-screensteps-tutorial/tutorial_screensteps_configure.png) 

7. 在不同的 Web 瀏覽器視窗中，以系統管理員身分登入您的 ScreenSteps 公司網站。
...
11. 在 [hello**建立單一登入端點**區段中，執行下列步驟的 hello:

    ![建立驗證端點](./media/active-directory-saas-screensteps-tutorial/ic778526.png "建立驗證端點")
    
    a. 在 [hello**標題**文字方塊中，輸入標題。
    
    b. 從 hello**模式**清單中，選取**SAML**。
...
13. 在 [hello**編輯單一登入端點**區段中，執行下列步驟的 hello:

    ![遠端驗證端點](./media/active-directory-saas-screensteps-tutorial/ic778527.png "遠端驗證端點")

    a. 按一下**上傳新的 SAML 憑證檔**，，然後上傳 hello 憑證，以便您從 Azure 入口網站下載。
    
    b. 貼上**SAML 單一登入服務 URL**值，您從 hello Azure 入口網站複製到 hello**遠端登入 URL**文字方塊。
    
    c. 貼上**登出 URL**值，您從 hello Azure 入口網站複製到 hello**登出 URL**文字方塊。
    
    d. 選取**群組**tooassign 使用者 toowhen 他們佈建。
    
    e. 按一下 [更新] 。

    f. 複製 hello **SAML 取用者 URL** toohello 剪貼簿並貼上 toohello**登入 URL** ] 文字方塊中的**ScreenSteps 網域和 Url** > 一節。
    
    g. 傳回 toohello**編輯單一登入端點**。
    
    h. 按一下 hello**設成預設值為帳戶**toouse 此端點的所有使用者登入 ScreenSteps] 按鈕。 或者，您可以按一下 hello**新增 tooSite**按鈕中的特定網站的這個端點 toouse **ScreenSteps**。
...
### <a name="create-a-screensteps-test-user"></a>建立 ScreenSteps 測試使用者

在本節中，您要在 ScreenSteps 中建立名為 Britta Simon 的使用者。 使用[ScreenSteps 用戶端支援小組](https://www.screensteps.com/contact)hello ScreenSteps 平台中新增 hello 使用者。 您必須先建立和啟動使用者，然後才能使用單一登入。 

### <a name="assign-hello-azure-ad-test-user"></a>指派給 Azure AD hello 測試使用者

在本節中，您可以授與存取 tooScreenSteps 啟用許 Simon toouse Azure 單一登入。

![指派 hello 使用者角色][200] 

**tooassign 許 Simon tooScreenSteps，執行下列步驟的 hello:**

1. 在 hello Azure 入口網站，開啟 hello 應用程式檢視，然後導覽 toohello 目錄檢視，並跳過**企業應用程式**然後按一下 [**所有應用程式**。

    ![指派使用者][201] 

2. 在 [hello] 應用程式清單中，選取**ScreenSteps**。
...
當您按一下的 hello ScreenSteps 磚 hello 存取面板中時，您應該取得自動登入 tooyour ScreenSteps 的應用程式。
