...
3. **[建立 Expensify 測試使用者](#creating-an-expensify-test-user)** - 在 Expensify 中建立 Britta Simon 的對應項目，且該項目與 Azure AD 中代表使用者的項目連結。
...
1. 在 Azure 入口網站的 [Expensify] 應用程式整合頁面上，按一下 [單一登入]。
...
3. 在 [Expensify 網域與 URL] 區段上，執行下列步驟：

    ![設定單一登入](./media/active-directory-saas-expensify-tutorial/tutorial_expensify_url.png)

    a. 在 [登入 URL] 文字方塊中，使用下列模式輸入 URL︰`https://www.expensify.com/authentication/saml/login`

    b.這是另一個 C# 主控台應用程式。 在 [識別碼 URL] 文字方塊中，使用下列模式輸入 URL：`https://www.<companyname>.expensify.com/` 

    > [!NOTE] 
    > 這些都不是真正的值。 使用實際的登入 URL 和識別碼 URL 來更新這些值。 請連絡 [Expensify 用戶端支援小組](mailto:help@expensify.com)以取得這些值。 
...
6. 若要在 Expensify 中啟用 SSO，您必須先在應用程式中啟用 [網域控制]。 您可以透過[這裡](http://help.expensify.com/domain-control)所列的步驟，在應用程式中啟用 [網域控制]。 如需其他支援，請與 [Expensify 用戶端支援小組](mailto:help@expensify.com)合作。 一旦啟用了 [網域控制]，請遵循下列步驟：
    ...
    f. 從記事本中的 Azure AD 開啟下載的同盟中繼資料，複製其內容並貼到 [識別提供者中繼資料] 文字方塊中。
