...
在此教學課程中，您學會如何 toointegrate Zoho 與 Azure Active Directory (Azure AD)。

與 Azure AD 整合 Zoho 可以提供下列優點 hello:

- 您可以控制存取 tooZoho Azure AD 中。
- 您可以啟用您的使用者 tooautomatically get 登入 tooZoho （單一登入） 具有其 Azure AD 帳戶。
...
tooconfigure 與 Zoho 的 Azure AD 整合，您需要下列項目 hello:
...
## <a name="adding-zoho-from-hello-gallery"></a>從 hello 圖庫加入 Zoho
tooconfigure hello 整合 Zoho 到 Azure AD，您需要 tooadd Zoho hello 圖庫 tooyour 清單中的受管理的 SaaS 應用程式。

**tooadd Zoho hello 圖庫中，從執行下列步驟的 hello:**
...
4. 在 hello 搜尋方塊中，輸入**Zoho**，選取**Zoho**然後按一下 從結果面板**新增**按鈕 tooadd hello 應用程式。
...
單一登入 toowork，Azure AD 需要 tooknow hello 的對等項目的使用者在 Zoho 是 tooa 使用者在 Azure AD 中。 換句話說，Azure AD 使用者與 hello Zoho 中相關的使用者之間的連結關聯性需要 toobe 建立。

在 Zoho，將指派的 hello hello 值**使用者名稱**做為 hello hello 值的 Azure AD 中**Username** tooestablish hello 連結關聯性。

tooconfigure 及測試 Azure AD 單一登入 Zoho，您必須遵循的建置組塊 toocomplete hello:
...
3. **[建立測試使用者 Zoho](#create-a-zoho-test-user)**  -toohave 許 Simon Zoho 所連結的 toohello Azure AD 使用者表示法中對應項目。
...
在本節中，您可以啟用 Azure AD 單一登入 hello Azure 入口網站中，並設定單一登入 Zoho 應用程式中。

**tooconfigure Azure AD 單一登入 Zoho，執行下列步驟的 hello:**

1. 在 Azure 入口網站上 hello hello **Zoho**應用程式整合頁面上，按一下 **單一登入**。
...
3. 在 hello **Zoho 網域和 Url**區段中，執行下列步驟的 hello:

    ![Zoho 網域與 URL 單一登入資訊](./media/active-directory-saas-zoho-mail-tutorial/tutorial_zoho_url.png)

    a. 在 hello**登入 URL**文字方塊中，輸入 URL，使用下列模式的 hello:`https://<company name>.zohomail.com`

    > [!NOTE] 
    > 這不是真實的值。 更新此值以 hello 實際的登入 URL。 請連絡[Zoho 用戶端支援小組](https://www.zoho.com/mail/contact.html)tooget 此值。 
...
6. 在 hello **Zoho 組態**區段中，按一下**設定 Zoho** tooopen**設定登入**視窗。 複製 hello**登出 URL、 變更密碼 URL 和 SAML 單一登入服務 URL**從 hello**快速參考 > 一節。**
...
9. 按一下 hello **SAML 驗證** 索引標籤。
   
    ![SAML 驗證](./media/active-directory-saas-zoho-mail-tutorial/ic789608.png "SAML 驗證")

10. 在 hello **SAML 驗證詳細資料**區段中，執行下列步驟的 hello:
   
    ![SAML 驗證詳細資料](./media/active-directory-saas-zoho-mail-tutorial/ic789609.png "SAML 驗證詳細資料")
   
    a. 在 hello**登入 URL**文字方塊中，貼上**SAML 單一登入服務 URL**您從 Azure 入口網站複製的。
   
    b. 在 hello**登出 URL**文字方塊中，貼上**登出 URL**您從 Azure 入口網站複製的。
   
    c. 在 hello**變更密碼 URL**文字方塊中，貼上**變更密碼 URL**您從 Azure 入口網站複製的。
       
    d. 開啟您從 [記事本]，複製到剪貼簿，其內容的 hello Azure 入口網站下載的 base-64 編碼的憑證，然後將它貼入 toohello **PublicKey**文字方塊。
...
在訂單 tooenable Azure AD 使用者 toolog 入 Zoho Mail，您必須是佈建到 Zoho Mail。 在 Zoho Mail 的 hello 案例中，佈建須手動進行。

> [!NOTE]
> 您可以使用任何其他 Zoho Mail 使用者帳戶建立工具或 Api 提供 Zoho Mail tooprovision AAD 使用者帳戶。

### <a name="tooprovision-a-user-account-perform-hello-following-steps"></a>tooprovision 使用者帳戶，執行下列步驟的 hello:

1. 登入 tooyour **Zoho Mail**系統管理員身分的公司網站。
...
4. 在 [hello**將使用者新增**] 對話方塊中，執行下列步驟的 hello:
   
    ![新增使用者](./media/active-directory-saas-zoho-mail-tutorial/ic789612.png "新增使用者")
   
    a. 在 hello**名字**文字方塊中，型別 hello 名字，例如使用者的**許**。

    b. 在 hello**姓氏**文字方塊中，型別 hello 姓氏的使用者，例如**Simon**。

    c. 在 hello**電子郵件識別碼**文字方塊中，例如使用者類型 hello 電子郵件識別碼 **brittasimon@contoso.com** 。

    d. 在 hello**密碼**文字方塊中，輸入使用者的密碼。
   
    e. 按一下 [確定] 。  
      
    > [!NOTE]
    > hello Azure Active Directory 帳戶持有者將會收到含有連結 tooconfirm hello 帳戶的電子郵件之前就會生效。

### <a name="assign-hello-azure-ad-test-user"></a>指派給 Azure AD hello 測試使用者

在本節中，您可以授與存取 tooZoho 啟用許 Simon toouse Azure 單一登入。

![指派 hello 使用者角色][200] 

**tooassign 許 Simon tooZoho，執行下列步驟的 hello:**

1. 在 hello Azure 入口網站，開啟 hello 應用程式檢視，然後導覽 toohello 目錄檢視，並跳過**企業應用程式**然後按一下 **所有應用程式**。

    ![指派使用者][201] 

2. 在 [hello] 應用程式清單中，選取**Zoho**。
...
當您按一下 hello Zoho 磚 hello 存取面板中的時，您應該取得自動登入 tooyour Zoho 應用程式。
