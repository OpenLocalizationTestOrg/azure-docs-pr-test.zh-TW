...
在此教學課程中，您學會如何 toointegrate ClickTime 與 Azure Active Directory (Azure AD)。

整合 Azure AD 與 ClickTime 可以提供下列優點 hello:

- 您可以控制存取 tooClickTime Azure AD 中
- 您可以啟用您的使用者 tooautomatically get 登入 tooClickTime （單一登入） 具有其 Azure AD 帳戶
...
tooconfigure Azure AD 與 ClickTime 的整合，您需要下列項目 hello:
...
1. 從 hello 圖庫加入 ClickTime
2. 設定並測試 Azure AD 單一登入

## <a name="adding-clicktime-from-hello-gallery"></a>從 hello 圖庫加入 ClickTime
tooconfigure hello 整合 ClickTime 的 Azure AD，您需要從受管理的 SaaS 應用程式的 hello 圖庫 tooyour 清單 tooadd ClickTime。

**tooadd ClickTime 從 hello 組件庫中，執行下列步驟的 hello:**
...
4. 在 hello 搜尋方塊中，輸入**ClickTime**，選取**ClickTime**然後按一下 從結果面板**新增**按鈕 tooadd hello 應用程式。
...
單一登入 toowork，Azure AD 需要 tooknow hello 的對等項目的使用者在 ClickTime 中是 tooa 使用者在 Azure AD 中。 換句話說，Azure AD 使用者與 hello ClickTime 中相關的使用者之間的連結關聯性需要 toobe 建立。

在 ClickTime 中，將指派的 hello hello 值**使用者名**做為 hello hello 值的 Azure AD 中**Username** tooestablish hello 連結關聯性。

tooconfigure 和測試 Azure AD 單一登入與 ClickTime，您需要遵循的建置組塊 toocomplete hello:
...
3. **[建立測試使用者 ClickTime](#create-a-clicktime-test-user)**  -toohave 許 Simon ClickTime 所連結的 toohello Azure AD 使用者表示法中對應項目。
...
在本節中，您可以啟用 Azure AD 單一登入 hello Azure 入口網站中，並設定單一登入 ClickTime 的應用程式中。

**tooconfigure Azure AD 單一登入與 ClickTime，執行下列步驟的 hello:**

1. 在 Azure 入口網站上 hello hello **ClickTime**應用程式整合頁面上，按一下 **單一登入**。
...
3. 在 hello **ClickTime 網域和 Url**區段中，執行下列步驟的 hello:

    ![ClickTime 網域與 URL 單一登入資訊](./media/active-directory-saas-clicktime-tutorial/tutorial_clicktime_url.png)

    a. 在 hello**識別碼**文字方塊中，輸入 URL:`https://app.clicktime.com/sp/`
...
6. 在 hello **ClickTime 組態**區段中，按一下**設定 ClickTime** tooopen**設定登入**視窗。 複製 hello **SAML 單一登入服務 URL**從 hello**快速參考 > 一節。**
...
8. 在 hello hello 上方的工具列中按一下**喜好設定**，然後按一下**安全性設定**。

9. 在 hello**單一登入喜好設定**組態區段中，執行下列步驟的 hello:
   
    ![安全性設定](./media/active-directory-saas-clicktime-tutorial/tic777280.png "安全性設定")
   
    a.  選取 [允許]，以搭配 **Azure AD** 使用單一登入 (SSO) 進行登入。
   
    b. 在 hello**身分識別提供者端點**文字方塊中，貼上**SAML 單一登入服務 URL**您從 Azure 入口網站複製的。
   
    c.  開啟 hello **base 64 編碼憑證**從 Azure 入口網站中下載**記事本**、 hello 內容複製並貼到 hello **X.509 憑證**文字方塊。
...
### <a name="create-a-clicktime-test-user"></a>建立 ClickTime 測試使用者

在訂單 tooenable Azure AD 使用者 toolog 入 ClickTime，它們必須佈建到 ClickTime。  
在 ClickTime 的 hello 案例中，佈建須手動進行。

> [!NOTE]
> 您可以使用任何其他 ClickTime 使用者帳戶建立工具或 Api 提供 ClickTime tooprovision Azure AD 使用者帳戶。

**tooprovision 使用者帳戶，執行下列步驟的 hello:**
1. 登入 tooyour **ClickTime**租用戶。
2. 在 hello hello 上方的工具列中按一下**公司**，然後按一下**人員**。
...
4. 在 hello 人員 區段中，執行下列步驟的 hello:
   
    ![人員](./media/active-directory-saas-clicktime-tutorial/tic777284.png "人員")
   
    a.  在 hello**全名**文字方塊中，例如使用者的完整名稱的輸入**許 Simon**。 
  
    b.  在 hello**電子郵件地址**文字方塊中，型別 hello 電子郵件的使用者喜歡 **brittasimon@contoso.com** 。
       
    > [!NOTE]
    > 如果您想要您可以設定新人員物件 hello 的其他屬性。
   
    c.  按一下 [儲存] 。

### <a name="assign-hello-azure-ad-test-user"></a>指派給 Azure AD hello 測試使用者

在本節中，您可以授與存取 tooClickTime 啟用許 Simon toouse Azure 單一登入。

![指派 hello 使用者角色][200] 

**tooassign 許 Simon tooClickTime，執行下列步驟的 hello:**

1. 在 hello Azure 入口網站，開啟 hello 應用程式檢視，然後導覽 toohello 目錄檢視，並跳過**企業應用程式**然後按一下 **所有應用程式**。

    ![指派使用者][201] 

2. 在 [hello] 應用程式清單中，選取**ClickTime**。
...
當您按一下 hello ClickTime 磚 hello 存取面板中的時，您應該取得自動登入 tooyour ClickTime 的應用程式。
