...
在此教學課程中，您學會如何 toointegrate Symantec 網站安全性服務 (WSS) 與 Azure Active Directory (Azure AD)。

與 Azure AD 整合 Symantec 網站安全性服務 (WSS) 可以提供下列優點 hello:

- 您可以控制存取 tooSymantec Web 安全性服務 (WSS) 的 Azure AD 中
- 您可以使用其 Azure AD 帳戶啟用您的使用者 tooautomatically get 登入 tooSymantec Web 安全性服務 (WSS) （單一登入）
...
1. 從 hello 圖庫加入 Symantec 網站安全性服務 (WSS)
...
4. 在 [hello] 搜尋方塊中，輸入**Symantec 網站安全性服務 (WSS)**。
...
5. 在 hello 結果 窗格中，選取  **Symantec 網站安全性服務 (WSS)**，然後按一下**新增**按鈕 tooadd hello 應用程式。
...
3. **[建立 Symantec 網站安全性服務 (WSS) 測試使用者](#creating-a-symantec-web-security-service-wss-test-user)** -toohave 許 Simon 中 Symantec Web 安全性服務 (WSS) 表示法連結的 toohello Azure AD 使用者的對應項目。
...
3. 在 hello **Symantec 網站安全性服務 (WSS) 網域和 Url**區段中，執行下列步驟的 hello:
    ...
    a. 在 hello**登入 URL**文字方塊中，於 hello url 想 tooblock 根據 toohello 封鎖原則的 hello Symantec 網站安全性服務 (WSS)。  
    
    b. 在 hello**識別碼**文字方塊中，型別 hello URL:`https://saml.threatpulse.net:8443/saml/saml_realm`

    > [!NOTE] 
    > 這些都不是真正的值。 更新這些值與實際的 hello 登入 URL 和識別項。 請連絡[Symantec 網站安全性服務 (WSS) 用戶端支援小組](https://www.symantec.com/contact-us)tooget 這些值。 
...
6. tooconfigure 單一登入上**Symantec 網站安全性服務 (WSS)**端，您需要下載 toosend hello**中繼資料 XML**太[Symantec 網站安全性服務 (WSS) 支援小組](https://www.symantec.com/contact-us).
...
### <a name="creating-a-symantec-web-security-service-wss-test-user"></a>建立 Symantec Web Security Service (WSS) 測試使用者

在本節中，您要在 Symantec Web Security Service (WSS) 中建立名為 Britta Simon 的使用者。 使用[Symantec 網站安全性服務 (WSS) 支援小組](https://www.symantec.com/contact-us)新增 hello 使用者在 hello Symantec 網站安全性服務 (WSS) 平台。 您必須先建立和啟動使用者，然後才能使用單一登入。 也以及 hello 使用者將詳細說明您需要 toosend hello hello 機器嘗試 tooaccess hello 應用程式的公用 ip 位址。
...
當您按一下的 hello Symantec 網站安全性服務 (WSS) 磚 hello 存取面板中時，您會取得重新導向的 toohello 封鎖的頁面已設定 hello 封鎖原則。
