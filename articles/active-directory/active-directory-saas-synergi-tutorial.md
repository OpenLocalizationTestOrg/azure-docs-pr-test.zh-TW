...
在此教學課程中，您學會如何 toointegrate Synergi 與 Azure Active Directory (Azure AD)。

與 Azure AD 整合 Synergi 可以提供下列優點 hello:

- 您可以控制存取 tooSynergi Azure AD 中。
- 您可以啟用您的使用者 tooautomatically get 登入 tooSynergi （單一登入） 具有其 Azure AD 帳戶。
...
tooconfigure Synergi 與 Azure AD 整合，您需要下列項目 hello:
...
1. 從 hello 圖庫加入 Synergi
2. 設定並測試 Azure AD 單一登入

## <a name="adding-synergi-from-hello-gallery"></a>從 hello 圖庫加入 Synergi
tooconfigure hello 整合 Synergi 到 Azure AD，您需要 tooadd Synergi hello 圖庫 tooyour 清單中的受管理的 SaaS 應用程式。

**tooadd Synergi 從 hello 組件庫中，執行下列步驟的 hello:**
...
4. 在 [hello] 搜尋方塊中，輸入**Synergi**，選取**Synergi**然後按一下 [從結果面板**新增**按鈕 tooadd hello 應用程式。
...
單一登入 toowork，Azure AD 需要 tooknow hello 的對等項目的使用者中 Synergi 是 tooa 使用者在 Azure AD 中。 換句話說，Azure AD 使用者與 hello Synergi 中相關的使用者之間的連結關聯性需要 toobe 建立。

Synergi 中, 指派的 hello hello 值**使用者名**做為 hello hello 值的 Azure AD 中**Username** tooestablish hello 連結關聯性。

tooconfigure 及 Synergi 與 Azure AD 單一登入的測試，您必須遵循的建置組塊 toocomplete hello:
...
3. **[建立測試使用者 Synergi](#create-a-synergi-test-user) ** -toohave 許 Simon Synergi 所連結的 toohello Azure AD 使用者表示法中對應項目。
...
在本節中，您可以啟用 Azure AD 單一登入 hello Azure 入口網站中，並 Synergi 應用程式中設定單一登入。

**tooconfigure Azure AD 單一登入與 Synergi，執行下列步驟的 hello:**

1. 在 Azure 入口網站上 hello hello **Synergi**應用程式整合頁面上，按一下 [**單一登入**。
...
3. 在 [hello **Synergi 網域和 Url**區段中，執行下列步驟的 hello:

    ![Synergi 網域與 URL 單一登入資訊](./media/active-directory-saas-synergi-tutorial/tutorial_synergi_url.png)

    a. 在 [hello**識別碼**文字方塊中，輸入 URL，使用下列模式的 hello:`https://<company name>.irmsecurity.com`

    b. 在 [hello**回覆 URL**文字方塊中，輸入 URL，使用下列模式的 hello:`https://<company name>.irmsecurity.com/sso/<organization id>`

    > [!NOTE] 
    > 這些都不是真正的值。 以 hello 實際的識別項和回覆 URL 更新這些值。 請連絡[Synergi 支援小組](https://www.irmsecurity.com/contact/)tooget 這些值。
...
6. 在 [hello **Synergi 組態**區段中，按一下**設定 Synergi** tooopen**設定登入**視窗。 複製 hello**登出 URL 和 SAML 實體識別碼**從 hello**快速參考 > 一節。**

    ![Synergi 設定](./media/active-directory-saas-synergi-tutorial/tutorial_synergi_configure.png) 

7. tooconfigure 單一登入上**Synergi**端，您需要下載 toosend hello **Certificate(Base64)、 登出 URL 和 SAML 實體識別碼**太[Synergi 支援小組](https://www.irmsecurity.com/contact/)。
...
在本節中，您要在 Synergi 中建立名為 Britta Simon 的使用者。 使用[Synergi 支援小組](https://www.irmsecurity.com/contact/)hello Synergi 平台中新增 hello 使用者。 您必須先建立和啟動使用者，然後才能使用單一登入。

### <a name="assign-hello-azure-ad-test-user"></a>指派給 Azure AD hello 測試使用者

在本節中，您可以授與存取 tooSynergi 啟用許 Simon toouse Azure 單一登入。

![指派 hello 使用者角色][200] 

**tooassign 許 Simon tooSynergi，執行下列步驟的 hello:**

1. 在 hello Azure 入口網站，開啟 hello 應用程式檢視，然後導覽 toohello 目錄檢視，並跳過**企業應用程式**然後按一下 [**所有應用程式**。

    ![指派使用者][201] 

2. 在 [hello] 應用程式清單中，選取**Synergi**。
...
當您按一下 hello Synergi 磚 hello 存取面板中的時，您應該取得自動登入 tooyour Synergi 應用程式。
