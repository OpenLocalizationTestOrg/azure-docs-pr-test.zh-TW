...
在此教學課程中，您學會如何 toointegrate UNIFI 與 Azure Active Directory (Azure AD)。

與 Azure AD 整合 UNIFI 可以提供下列優點 hello:

- 您可以控制存取 tooUNIFI Azure AD 中
- 您可以啟用您的使用者 tooautomatically get 登入 tooUNIFI （單一登入） 具有其 Azure AD 帳戶
...
tooconfigure UNIFI 與 Azure AD 整合，您需要下列項目 hello:
...
1. 從 hello 圖庫加入 UNIFI
2. 設定並測試 Azure AD 單一登入

## <a name="adding-unifi-from-hello-gallery"></a>從 hello 圖庫加入 UNIFI
tooconfigure hello 整合 UNIFI 到 Azure AD，您需要 tooadd UNIFI hello 圖庫 tooyour 清單中的受管理的 SaaS 應用程式。

**tooadd UNIFI 從 hello 組件庫中，執行下列步驟的 hello:**
...
4. 在 [hello] 搜尋方塊中，輸入**UNIFI**。

    ![建立 Azure AD 測試使用者](./media/active-directory-saas-unifi-tutorial/tutorial_unifi_search.png)

5. 在 hello 結果 窗格中，選取  **UNIFI**，然後按一下**新增**按鈕 tooadd hello 應用程式。

    ![建立 Azure AD 測試使用者](./media/active-directory-saas-unifi-tutorial/tutorial_unifi_addfromgallery.png)

##  <a name="configuring-and-testing-azure-ad-single-sign-on"></a>設定並測試 Azure AD 單一登入
在本節中，您會以名為 "Britta Simon" 的測試使用者身分，使用 UNIFI 設定及測試 Azure AD 單一登入。

單一登入 toowork，Azure AD 需要 tooknow hello 的對等項目的使用者中 UNIFI 是 tooa 使用者在 Azure AD 中。 換句話說，Azure AD 使用者與 hello UNIFI 中相關的使用者之間的連結關聯性需要 toobe 建立。

UNIFI 中, 指派的 hello hello 值**使用者名**做為 hello hello 值的 Azure AD 中**Username** tooestablish hello 連結關聯性。

tooconfigure 及 UNIFI 與 Azure AD 單一登入的測試，您必須遵循的建置組塊 toocomplete hello:

1. **[設定 Azure AD 單一登入](#configuring-azure-ad-single-sign-on)** -tooenable 使用者 toouse 這項功能。
2. **[建立 Azure AD 測試使用者](#creating-an-azure-ad-test-user)** -tootest Azure AD 單一登入與許 Simon。
3. **[建立 UNIFI 測試使用者](#creating-a-unifi-test-user)** -toohave 許 Simon UNIFI 所連結的 toohello Azure AD 使用者表示法中對應項目。
4. **[指派 hello Azure AD 的測試使用者](#assigning-the-azure-ad-test-user)** -tooenable 許 Simon toouse Azure AD 單一登入。
5. **[測試單一登入](#testing-single-sign-on)** -tooverify 是否 hello 組態工作。

### <a name="configuring-azure-ad-single-sign-on"></a>設定 Azure AD 單一登入

在本節中，您可以啟用 Azure AD 單一登入 hello Azure 入口網站中，並 UNIFI 應用程式中設定單一登入。

**tooconfigure Azure AD 單一登入與 UNIFI，執行下列步驟的 hello:**

1. 在 Azure 入口網站上 hello hello **UNIFI**應用程式整合頁面上，按一下 **單一登入**。
...
3. 在 hello **UNIFI 網域和 Url**區段中，如果您想 tooconfigure hello 應用程式中的**IDP**初始模式：

    ![設定單一登入](./media/active-directory-saas-unifi-tutorial/tutorial_unifi_url1.png)

    在 hello**識別碼**文字方塊中，類型 hello 值：`INVIEWlabs` 
...
4. 請檢查**顯示進階的 URL 設定**，如果您想 tooconfigure hello 應用程式中的**SP**初始模式：
    ...
    在 hello**登入 URL**文字方塊中，型別 hello URL:`https://app.discoverunifi.com/login`
...
7. 在 hello **UNIFI 組態**區段中，按一下**設定 UNIFI** tooopen**設定登入**視窗。 複製 hello **SAML 單一登入服務 URL**從 hello**快速參考 > 一節。**

    ![設定單一登入](./media/active-directory-saas-unifi-tutorial/tutorial_unifi_configure.png)

8. 在不同的網頁瀏覽器視窗中，登入 tooyour **UNIFI**以系統管理員身分的公司網站。

9. 按一下 hello**使用者**。

    ![設定單一登入](./media/active-directory-saas-unifi-tutorial/app1.png) 

10. 按一下 hello**新增身分識別提供者**。

    ![設定單一登入](./media/active-directory-saas-unifi-tutorial/app2.png)

11. 在 hello**新增身分識別提供者**區段中，執行下列步驟的 hello:   

    ![設定單一登入](./media/active-directory-saas-unifi-tutorial/app3.png) 

    a. 在 hello**提供者名稱**文字方塊中，型別 hello hello 身分識別提供者名稱...

    b. 在 hello hello**提供者 URL**文字方塊中貼上 hello **SAML 單一登入服務 URL**您從 Azure 入口網站複製的值。

    c. 開啟 hello hello Azure 入口網站，在記事本中，從網站下載的憑證移除 hello **---BEGIN CERTIFICATE---**和**---憑證結尾---**標記，然後貼上剩餘的內容中的 hellohello**憑證**文字方塊。

    d. 選取 hello**是預設提供者**核取方塊。
...
### <a name="creating-a-unifi-test-user"></a>建立 UNIFI 測試使用者

在本節中，您會建立名為 Britta Simon 的使用者。 **UNIFI** 支援自動使用者佈建，因此不不需要任何手動步驟。 使用者會自動建立從 hello Azure AD 成功驗證之後。

### <a name="assigning-hello-azure-ad-test-user"></a>指派 hello Azure AD 的測試使用者

在本節中，您可以授與存取 tooUNIFI 啟用許 Simon toouse Azure 單一登入。

![指派使用者][200] 

**tooassign 許 Simon tooUNIFI，執行下列步驟的 hello:**

1. 在 hello Azure 入口網站，開啟 hello 應用程式檢視，然後導覽 toohello 目錄檢視，並跳過**企業應用程式**然後按一下 **所有應用程式**。

    ![指派使用者][201] 

2. 在 [hello] 應用程式清單中，選取**UNIFI**。
...
當您按一下的 hello UNIFI 磚 hello 存取面板中時，您應該取得自動登入 tooyour UNIFI 應用程式。
