...
3. **[建立 New Relic 測試使用者](#creating-a-new-relic-test-user)** - 使 New Relic 中對應的 Britta Simon 連結到該使用者在 Azure AD 中的代表項目。
...
3. 在 [New Relic 網域與 URL] 區段上，執行下列步驟：
    ...
    在 [登入 URL] 文字方塊中，使用下列模式輸入 URL︰`https://<subdomain>.newrelic.com`

    > [!NOTE] 
    > 這不是真正的值。 請使用實際的「登入 URL」來更新此值。 請連絡 [New Relic 客戶支援小組](https://support.newrelic.com/)以取得此值。 
...
6. 在 [New Relic 組態] 區段上，按一下 [設定 New Relic] 以開啟 [設定登入] 視窗。 從 [快速參考] 區段中複製 [登出 URL 和 SAML 單一登入服務 URL]。
