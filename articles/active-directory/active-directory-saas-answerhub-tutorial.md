...
在此教學課程中，您學會如何 toointegrate AnswerHub 與 Azure Active Directory (Azure AD)。

與 Azure AD 整合 AnswerHub 可以提供下列優點 hello:

- 您可以控制存取 tooAnswerHub Azure AD 中
- 您可以啟用您的使用者 tooautomatically get 登入 tooAnswerHub （單一登入） 具有其 Azure AD 帳戶
...
tooconfigure 與 AnswerHub 的 Azure AD 整合，您需要下列項目 hello:
...
1. 從 hello 圖庫加入 AnswerHub
2. 設定並測試 Azure AD 單一登入

## <a name="adding-answerhub-from-hello-gallery"></a>從 hello 圖庫加入 AnswerHub
tooconfigure hello 整合 AnswerHub 的 Azure AD，您需要從受管理的 SaaS 應用程式的 hello 圖庫 tooyour 清單 tooadd AnswerHub。

**tooadd AnswerHub 從 hello 組件庫中，執行下列步驟的 hello:**
...
單一登入 toowork，Azure AD 需要 tooknow hello 的對等項目的使用者在 AnswerHub 中是 tooa 使用者在 Azure AD 中。 換句話說，Azure AD 使用者與 hello AnswerHub 中相關的使用者之間的連結關聯性需要 toobe 建立。

在 AnswerHub 中，將指派的 hello hello 值**使用者名**做為 hello hello 值的 Azure AD 中**Username** tooestablish hello 連結關聯性。

tooconfigure 及測試 Azure AD 單一登入 AnswerHub，您必須遵循的建置組塊 toocomplete hello:

1. **[設定 Azure AD 單一登入](#configuring-azure-ad-single-sign-on)** -tooenable 使用者 toouse 這項功能。
2. **[建立 Azure AD 測試使用者](#creating-an-azure-ad-test-user)** -tootest Azure AD 單一登入與許 Simon。
3. **[建立測試使用者 AnswerHub](#creating-an-answerhub-test-user)**  -toohave 許 Simon AnswerHub 所連結的 toohello Azure AD 使用者表示法中對應項目。
4. **[指派 hello Azure AD 的測試使用者](#assigning-the-azure-ad-test-user)** -tooenable 許 Simon toouse Azure AD 單一登入。
5. **[測試單一登入](#testing-single-sign-on)** -tooverify 是否 hello 組態工作。

### <a name="configuring-azure-ad-single-sign-on"></a>設定 Azure AD 單一登入

在本節中，您可以啟用 Azure AD 單一登入 hello Azure 入口網站中，並設定單一登入 AnswerHub 的應用程式中。

**tooconfigure Azure AD 單一登入 AnswerHub，執行下列步驟的 hello:**

1. 在 Azure 入口網站上 hello hello **AnswerHub**應用程式整合頁面上，按一下 **單一登入**。
...
3. 在 hello **AnswerHub 網域和 Url**區段中，執行下列步驟的 hello:

    ![設定單一登入](./media/active-directory-saas-answerhub-tutorial/tutorial_answerhub_url.png)

    a. 在 hello**登入 URL**文字方塊中，輸入 URL，使用下列模式的 hello:`https://<company>.answerhub.com`

    b. 在 hello**識別碼**文字方塊中，輸入 URL，使用下列模式的 hello:`https://<company>.answerhub.com`

    > [!NOTE] 
    > 這些都不是真正的值。 更新這些值與實際的 hello 登入 URL 和識別項。 請連絡[AnswerHub 用戶端支援小組](mailto:success@answerhub.com)tooget 這些值。 
...
6. 在 hello **AnswerHub 組態**區段中，按一下**設定 AnswerHub** tooopen**設定登入**視窗。 複製 hello**登出 URL 和 SAML 單一登入服務 URL**從 hello**快速參考 > 一節。**
...
9. 按一下 hello**使用者和群組** 索引標籤。

10. 左方、 hello 中的 hello hello 瀏覽窗格中**社交設定**區段中，按一下**SAML 設定**。

11. 按一下 [IDP 設定]  索引標籤。

12. 在 hello **IDP 設定**索引標籤上，執行下列步驟的 hello:
     ...
     c. 在**IDP 姓名識別碼格式**文字方塊中，輸入的識別碼值相同中已選取的 Azure 入口網站中的 hello 使用者**使用者屬性**> 一節。
  
     d. 按一下 [金鑰和憑證] 。

13. Hello 金鑰和憑證 索引標籤上，執行下列步驟的 hello:
    
     ![金鑰和憑證](./media/active-directory-saas-answerhub-tutorial/ic785173.png "金鑰和憑證")  
 
     a. 開啟 base-64 編碼的憑證其從 [記事本]，複製到剪貼簿，其內容的 hello Azure 入口網站下載，然後將它貼入 toohello **IDP 公開金鑰 (x x509 格式）**文字方塊。
  
     b. 按一下 [儲存] 。

14. 在 hello **IDP 設定**索引標籤上，按一下 **儲存**。
...
tooenable Azure AD 使用者 toolog 中 tooAnswerHub，它們必須佈建到 AnswerHub。  
在 AnswerHub 的 hello 案例中，佈建須手動進行。

**tooprovision 使用者帳戶，執行下列步驟的 hello:**

1. 登入 tooyour **AnswerHub**以系統管理員身分的公司網站。

2. 跳過**管理**。

3. 按一下 hello**使用者和群組** 索引標籤。

4. 左方、 hello 中的 hello hello 瀏覽窗格中**管理使用者**區段中，按一下**建立或匯入使用者**。
   
   ![使用者和群組](./media/active-directory-saas-answerhub-tutorial/ic785175.png "使用者和群組")

5. 型別 hello**電子郵件地址**， **Username**和**密碼**的有效 azure Active Directory 帳戶想成 hello tooprovision 相關的文字方塊，，然後按一下 **儲存**。

>[!NOTE]
>您可以使用任何其他 AnswerHub 使用者帳戶建立工具或 Api 提供 AnswerHub tooprovision AAD 使用者帳戶。

### <a name="assigning-hello-azure-ad-test-user"></a>指派 hello Azure AD 的測試使用者

在本節中，您可以授與存取 tooAnswerHub 啟用許 Simon toouse Azure 單一登入。

![指派使用者][200] 

**tooassign 許 Simon tooAnswerHub，執行下列步驟的 hello:**

1. 在 hello Azure 入口網站，開啟 hello 應用程式檢視，然後導覽 toohello 目錄檢視，並跳過**企業應用程式**然後按一下 **所有應用程式**。

    ![指派使用者][201] 

2. 在 [hello] 應用程式清單中，選取**AnswerHub**。
...
當您按一下 hello AnswerHub 磚 hello 存取面板中的時，您應該取得自動登入 tooyour AnswerHub 的應用程式。
