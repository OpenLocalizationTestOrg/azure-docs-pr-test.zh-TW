...
在此教學課程中，您學會如何 toointegrate Onit 與 Azure Active Directory (Azure AD)。

與 Azure AD 整合 Onit 可以提供下列優點 hello:

- 您可以控制存取 tooOnit Azure AD 中。
- 您可以啟用您的使用者 tooautomatically get 登入 tooOnit （單一登入） 具有其 Azure AD 帳戶。
...
tooconfigure 與 Onit 的 Azure AD 整合，您需要下列項目 hello:
...
## <a name="adding-onit-from-hello-gallery"></a>從 hello 圖庫加入 Onit
tooconfigure hello 整合 Onit 的 Azure AD，您需要從受管理的 SaaS 應用程式的 hello 圖庫 tooyour 清單 tooadd Onit。

**tooadd Onit 從 hello 組件庫中，執行下列步驟的 hello:**
...
4. 在 hello 搜尋方塊中，輸入**Onit**，選取**Onit**然後按一下 從結果面板**新增**按鈕 tooadd hello 應用程式。
...
單一登入 toowork，Azure AD 需要 tooknow hello 的對等項目的使用者在 Onit 中是 tooa 使用者在 Azure AD 中。 換句話說，Azure AD 使用者與 hello Onit 中相關的使用者之間的連結關聯性需要 toobe 建立。

在 Onit 中，將指派的 hello hello 值**使用者名**做為 hello hello 值的 Azure AD 中**Username** tooestablish hello 連結關聯性。

tooconfigure 及測試 Azure AD 單一登入 Onit，您必須遵循的建置組塊 toocomplete hello:
...
3. **[建立測試使用者 Onit](#create-an-onit-test-user)**  -toohave 許 Simon Onit 所連結的 toohello Azure AD 使用者表示法中對應項目。
...
在本節中，您可以啟用 Azure AD 單一登入 hello Azure 入口網站中，並設定單一登入 Onit 應用程式中。

**tooconfigure Azure AD 單一登入 Onit，執行下列步驟的 hello:**

1. 在 Azure 入口網站上 hello hello **Onit**應用程式整合頁面上，按一下 **單一登入**。
...
3. 在 hello **Onit 網域和 Url**區段中，執行下列步驟的 hello:

    ![Onit 網域與 URL 單一登入資訊](./media/active-directory-saas-onit-tutorial/tutorial_onit_url.png)

    a. 在 hello**登入 URL**文字方塊中，輸入 URL，使用下列模式的 hello:`https://<sub-domain>.onit.com`

    b. 在 hello**識別碼**文字方塊中，輸入 URL，使用下列模式的 hello:`https://<sub-domain>.onit.com`

    > [!NOTE] 
    > 這些都不是真正的值。 更新這些值與實際的 hello 登入 URL 和識別項。 請連絡[Onit 用戶端支援小組](https://www.onit.com/support)tooget 這些值。 
...
5. Onit 應用程式預期 hello SAML 判斷提示，以特定格式。 請設定下列宣告為此應用程式的 hello。 您可以從 hello 管理這些屬性的 hello 值**」 屬性 「** hello 應用程式 索引標籤。 hello 下列螢幕擷取畫面會顯示這個範例。 
...
8. 在 hello **Onit 組態**區段中，按一下**設定 Onit** tooopen**設定登入**視窗。 複製 hello**登出 URL、 SAML 單一登入服務 URL**從 hello**快速參考 > 一節。**

    ![Onit 設定](./media/active-directory-saas-onit-tutorial/tutorial_onit_configure.png)

9. 在不同的 Web 瀏覽器視窗中，以系統管理員身分登入您的 Onit 公司網站。

10. 在 hello 最上層顯示 hello 功能表上，按一下**管理**。
...
12. 按一下 hello**安全性** 索引標籤。
    
    ![編輯公司資訊](./media/active-directory-saas-onit-tutorial/IC791176.png "編輯公司資訊")

13. 在 hello**安全性**索引標籤上，執行下列步驟的 hello:

    ![單一登入](./media/active-directory-saas-onit-tutorial/IC791177.png "單一登入")

    a. 對於**驗證策略**，請選取 [單一登入和密碼]。
    
    b. 在**Idp 目標 URL**文字方塊中，貼上 hello 值**SAML 單一登入服務 URL**，從 Azure 入口網站複製的。

    c. 在**Idp 登出 URL**文字方塊中，貼上 hello 值**登出 URL**，從 Azure 入口網站複製的。

    d. 在**Idp 憑證指紋 (SHA1)**文字方塊中，貼上 hello**指紋**憑證，您從 Azure 入口網站複製的值。
...
在訂單 tooenable Azure AD 使用者 toolog 入 Onit，它們必須佈建到 Onit。  

在 Onit 的 hello 案例中，佈建須手動進行。

**tooconfigure 使用者佈建，執行下列步驟的 hello:**

1. 登入 tooyour **Onit**系統管理員身分的公司網站。
2. 按一下 [加入使用者] 。
...
3. 在 hello**新增使用者**對話方塊頁面上，執行下列步驟的 hello:
  ...
  1. 型別 hello**名稱**和 hello**電子郵件地址**的有效 azure AD 帳戶想成 hello tooprovision 相關的文字方塊。
  2. 按一下 [建立] 。    
...
在本節中，您可以授與存取 tooOnit 啟用許 Simon toouse Azure 單一登入。

![指派 hello 使用者角色][200] 

**tooassign 許 Simon tooOnit，執行下列步驟的 hello:**

1. 在 hello Azure 入口網站，開啟 hello 應用程式檢視，然後導覽 toohello 目錄檢視，並跳過**企業應用程式**然後按一下 **所有應用程式**。

    ![指派使用者][201] 

2. 在 [hello] 應用程式清單中，選取**Onit**。
...
當您按一下 hello Onit 磚 hello 存取面板中的時，您應該取得自動登入 tooyour Onit 應用程式。
