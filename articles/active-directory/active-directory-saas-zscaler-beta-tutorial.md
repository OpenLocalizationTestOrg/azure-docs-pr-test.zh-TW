...
在此教學課程中，您學會如何 toointegrate Zscaler Beta 與 Azure Active Directory (Azure AD)。

Zscaler Beta 整合與 Azure AD 可以提供下列優點 hello:

- 您可以控制存取 tooZscaler beta 版的 Azure AD 中
- 您可以啟用您的使用者 tooautomatically get 登入 tooZscaler Beta （單一登入） 具有其 Azure AD 帳戶
...
tooconfigure Azure AD 與 Zscaler Beta 整合，您需要下列項目 hello:
...
1. 從 hello 圖庫加入 Zscaler Beta
2. 設定並測試 Azure AD 單一登入

## <a name="adding-zscaler-beta-from-hello-gallery"></a>從 hello 圖庫加入 Zscaler Beta
tooconfigure hello 整合 Zscaler Beta 到 Azure AD，您需要從受管理的 SaaS 應用程式的 hello 圖庫 tooyour 清單 tooadd Zscaler Beta。

**tooadd Zscaler Beta 從 hello 組件庫中，執行下列步驟的 hello:**
...
單一登入 toowork，Azure AD 需要 tooknow hello 的對等項目的使用者 Zscaler beta 是 tooa 使用者在 Azure AD 中。 換句話說，Azure AD 使用者與 Zscaler Beta 中的 hello 相關的使用者之間的連結關聯性需要 toobe 建立。

Zscaler Beta 中指派的 hello hello 值**使用者名稱**做為 hello hello 值的 Azure AD 中**Username** tooestablish hello 連結關聯性。

tooconfigure 和測試 Azure AD 單一登入 Zscaler beta，您需要遵循的建置組塊 toocomplete hello:
...
4. **[建立 Zscaler Beta 測試使用者](#creating-a-zscaler-beta-test-user)** -toohave 許 Simon 是連結的 toohello Azure AD 使用者表示法的 Zscaler Beta 中對應項目。
...
在本節中，您可以啟用 Azure AD 單一登入 hello Azure 入口網站中，並設定單一登入 Zscaler Beta 應用程式中。

**tooconfigure Azure AD 單一登入 Zscaler beta，執行下列步驟的 hello:**

1. 在 Azure 入口網站上 hello hello **Zscaler Beta**應用程式整合頁面上，按一下 **單一登入**。
...
3. 在 hello **Zscaler Beta 網域和 Url**區段中，執行下列步驟的 hello:

    ![設定單一登入](./media/active-directory-saas-zscaler-beta-tutorial/tutorial_zscalerbeta_url.png)

    在 hello 登入 URL 文字方塊中，輸入您的使用者 」 toosign 上 Zscaler Beta 應用程式 tooyour 用 hello URL。

    > [!NOTE] 
    > 具有這個值與 hello tooupdate 實際的登入 URL。 請連絡[Zscaler Beta 用戶端支援小組](https://www.zscaler.com/company/contact)tooget 此值。 
...
6. 在 hello **Zscaler Beta 設定**區段中，按一下**設定 Zscaler Beta** tooopen**設定登入**視窗。 複製 hello **SAML 單一登入服務 URL**從 hello**快速參考 > 一節。**

    ![設定單一登入](./media/active-directory-saas-zscaler-beta-tutorial/tutorial_zscalerbeta_configure.png) 

7. 在不同的網頁瀏覽器視窗中，以登入 tooyour Zscaler Beta 公司網站的系統管理員。
...
5. 在 hello Proxy 伺服器區段中，執行下列步驟的 hello:   
    ...
    b. 在 [hello 位址] 文字方塊中輸入**gateway.zscalerbeta.net**。
...
tooenable Azure AD 使用者 toolog 中 tooZscaler Beta，它們必須已佈建的 tooZscaler Beta。 在 Zscaler beta 版的 hello 案例中，佈建須手動進行。

### <a name="tooconfigure-user-provisioning-perform-hello-following-steps"></a>tooconfigure 使用者佈建，執行下列步驟的 hello:

1. 登入 tooyour **Zscaler Beta**租用戶。
...
> [!NOTE]
> 您可以使用任何其他 Zscaler Beta 使用者帳戶建立工具或 Api 提供 Zscaler Beta tooprovision Azure AD 使用者帳戶。

### <a name="assigning-hello-azure-ad-test-user"></a>指派 hello Azure AD 的測試使用者

在本節中，以啟用許 Simon toouse Azure 單一登入授與存取 tooZscaler Beta。

![指派使用者][200] 

**tooassign 許 Simon tooZscaler Beta，執行下列步驟的 hello:**

1. 在 hello Azure 入口網站，開啟 hello 應用程式檢視，然後導覽 toohello 目錄檢視，並跳過**企業應用程式**然後按一下 **所有應用程式**。

    ![指派使用者][201] 

2. 在 [hello] 應用程式清單中，選取**Zscaler Beta**。
...
當您按一下的 hello Zscaler Beta 磚 hello 存取面板中時，您應該取得自動登入 tooyour Zscaler Beta 應用程式。
