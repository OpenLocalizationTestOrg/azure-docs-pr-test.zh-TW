...
在此教學課程中，您學會如何 toointegrate SCC LifeCycle 與 Azure Active Directory (Azure AD)。

SCC LifeCycle 整合與 Azure AD 可以提供下列優點 hello:

- 您可以控制存取 tooSCC 生命週期的 Azure AD 中
- 您可以啟用您的使用者 tooautomatically get 登入 tooSCC 生命週期 （單一登入） 具有其 Azure AD 帳戶
...
tooconfigure 與 SCC LifeCycle 的 Azure AD 整合，您需要下列項目 hello:
...
1. 從 hello 圖庫加入 SCC LifeCycle
2. 設定並測試 Azure AD 單一登入

## <a name="adding-scc-lifecycle-from-hello-gallery"></a>從 hello 圖庫加入 SCC LifeCycle
tooconfigure hello 整合 SCC LifeCycle 的 Azure AD，您需要從受管理的 SaaS 應用程式的 hello 圖庫 tooyour 清單 tooadd SCC LifeCycle。

**tooadd SCC LifeCycle 從 hello 組件庫中，執行下列步驟的 hello:**
...
單一登入 toowork，Azure AD 需要 tooknow hello 對應項目中 SCC LifeCycle 的使用者是 tooa 使用者在 Azure AD 中。 換句話說，Azure AD 使用者與 SCC LifeCycle 中的 hello 相關的使用者之間的連結關聯性需要 toobe 建立。

SCC LifeCycle 中指派的 hello hello 值**使用者名稱**做為 hello hello 值的 Azure AD 中**Username** tooestablish hello 連結關聯性。

tooconfigure 及測試 Azure AD 單一登入 SCC LifeCycle，您必須遵循的建置組塊 toocomplete hello:

1. **[設定 Azure AD 單一登入](#configuring-azure-ad-single-sign-on)** -tooenable 使用者 toouse 這項功能。
2. **[建立 Azure AD 測試使用者](#creating-an-azure-ad-test-user)** -tootest Azure AD 單一登入與許 Simon。
3. **[建立 SCC LifeCycle 測試使用者](#creating-an-scc-lifecycle-test-user)** -toohave 許 Simon SCC LifeCycle 所連結的 toohello Azure AD 使用者表示法中對應項目。
4. **[指派 hello Azure AD 的測試使用者](#assigning-the-azure-ad-test-user)** -tooenable 許 Simon toouse Azure AD 單一登入。
5. **[測試單一登入](#testing-single-sign-on)** -tooverify 是否 hello 組態工作。

### <a name="configuring-azure-ad-single-sign-on"></a>設定 Azure AD 單一登入

在本節中，您可以啟用 Azure AD 單一登入 hello Azure 入口網站中，並設定單一登入 SCC LifeCycle 的應用程式中。

**tooconfigure Azure AD 單一登入與 SCC LifeCycle，執行下列步驟的 hello:**

1. 在 Azure 入口網站上 hello hello **SCC LifeCycle**應用程式整合頁面上，按一下 **單一登入**。
...
3. 在 hello **SCC LifeCycle 網域和 Url**區段中，執行下列步驟的 hello:

    ![設定單一登入](./media/active-directory-saas-scclifecycle-tutorial/tutorial_scclifecycle_url.png)

    a. 在 hello**登入 URL**文字方塊中，輸入 URL，使用下列模式的 hello:`https://<sub-domain>.scc.com/ic7/welcome/customer/PICTtest.aspx`
    ...
    > [!NOTE] 
    > 這些都不是真正的值。 更新這些值與實際的 hello 登入 URL 和識別項。 請連絡[SCC LifeCycle 用戶端支援小組](mailto:lifecycle.support@scc.com)tooget 這些值。 
...
6. tooconfigure 單一登入上**SCC LifeCycle**端，您需要下載 toosend hello**中繼資料 XML**太[SCC LifeCycle 支援小組](mailto:lifecycle.support@scc.com)。 設定此設定 toohave hello SAML SSO 連線兩端上正確設定這些欄位。
...
在本節中，以啟用許 Simon toouse Azure 單一登入授與存取 tooSCC 生命週期。

![指派使用者][200] 

**tooassign 許 Simon tooSCC 生命週期，執行下列步驟的 hello:**

1. 在 hello Azure 入口網站，開啟 hello 應用程式檢視，然後導覽 toohello 目錄檢視，並跳過**企業應用程式**然後按一下 **所有應用程式。**

    ![指派使用者][201] 

2. 在 [hello] 應用程式清單中，選取**SCC LifeCycle**。
...
當您按一下的 hello SCC LifeCycle 磚 hello 存取面板中時，您應該取得自動登入 tooSCC LifeCycle 應用程式。
