...
在此教學課程中，您學會如何 toointegrate 機器人學與 Azure Active Directory (Azure AD) 的 Kantega SSO。

與 Azure AD 整合的機器人學 Kantega SSO 可以提供下列優點 hello:

- 您可以控制存取 tooKantega 機器人學的 SSO 的 Azure AD 中
- 您可以啟用您的使用者 tooautomatically get 登入 tooKantega 機器人學 （單一登入） 的 SSO 與他們的 Azure AD 帳戶
...
tooconfigure 機器人學的 Kantega SSO 與 Azure AD 整合，您需要下列項目 hello:
...
1. 從 hello 圖庫新增為機器人學 Kantega SSO
2. 設定並測試 Azure AD 單一登入

## <a name="adding-kantega-sso-for-confluence-from-hello-gallery"></a>從 hello 圖庫新增為機器人學 Kantega SSO
tooconfigure hello Kantega SSO 機器人學至 Azure AD 的整合，您需要 tooadd Kantega SSO 機器人學 hello 圖庫 tooyour 清單中的受管理的 SaaS 應用程式。

**tooadd Kantega SSO 的機器人學 hello 圖庫中，從執行下列步驟的 hello:**
...
4. 在 [hello] 搜尋方塊中，輸入**機器人學的 Kantega SSO**。
...
5. 在 hello 結果 窗格中，選取 **機器人學的 Kantega SSO**，然後按一下**新增**按鈕 tooadd hello 應用程式。
...
單一登入 toowork，Azure AD 需要 tooknow 機器人學的 Kantega SSO 中的 hello 對等項目的使用者是 tooa 使用者在 Azure AD 中。 換句話說，Azure AD 使用者與 hello 機器人學的 Kantega SSO 中相關的使用者之間的連結關聯性需要 toobe 建立。

機器人學的 Kantega SSO 中, 指派的 hello hello 值**使用者名**做為 hello hello 值的 Azure AD 中**Username** tooestablish hello 連結關聯性。

tooconfigure 及測試 Azure AD 單一登入使用 Kantega SSO 機器人學，您必須遵循的建置組塊 toocomplete hello:

1. **[設定 Azure AD 單一登入](#configuring-azure-ad-single-sign-on)** -tooenable 使用者 toouse 這項功能。
2. **[建立 Azure AD 測試使用者](#creating-an-azure-ad-test-user)** -tootest Azure AD 單一登入與許 Simon。
3. **[建立機器人學測試使用者 Kantega SSO](#creating-a-kantega-sso-for-confluence-test-user)**  -toohave 許 Simon 的是連結的 toohello Azure AD 使用者表示法的機器人學 Kantega SSO 中對應項目。
4. **[指派 hello Azure AD 的測試使用者](#assigning-the-azure-ad-test-user)** -tooenable 許 Simon toouse Azure AD 單一登入。
5. **[測試單一登入](#testing-single-sign-on)** -tooverify 是否 hello 組態工作。

### <a name="configuring-azure-ad-single-sign-on"></a>設定 Azure AD 單一登入

在本節中，您可以啟用 Azure AD 單一登入 hello Azure 入口網站中，並設定單一登入您的 Kantega SSO 機器人學應用程式中。

**Azure AD 單一登入的 tooconfigure 機器人學的 Kantega sso 執行 hello 下列步驟：**

1. 在 Azure 入口網站上 hello hello**機器人學的 Kantega SSO**應用程式整合頁面上，按一下 **單一登入**。
...
3. 在**IDP**起始模式，在 hello**機器人學網域和 Url Kantega SSO**區段中，執行下列步驟的 hello:
...
4. 在**SP**初始的模式中，核取**顯示進階的 URL 設定**並執行下列步驟的 hello:
    ...
    > [!NOTE] 
    > 這些都不是真正的值。 更新這些值與實際的 hello，識別項、 回覆 URL 和登入 URL。 Hello 機器人學外掛程式，在 hello 教學課程稍後會說明設定期間，會收到這些值。
...
7. 在不同的網頁瀏覽器視窗中，登入 tooyour**機器人學管理入口**身為系統管理員。
...
10. 搜尋**機器人學 SAML kerberos Kantega SSO**按一下**安裝**按鈕 tooinstall hello 新 SAML 外掛程式。
...
16. 在 hello **SAML** > 一節。 選取**Azure Active Directory (Azure AD)**從 hello**新增身分識別提供者**下拉式清單。
...
18. 在 hello**應用程式屬性**區段中，執行下列步驟： 

    ![設定單一登入](./media/active-directory-saas-kantegassoforconfluence-tutorial/addon6.png)

    a. 複製 hello**應用程式識別碼 URI**值，並使用它作為**識別碼、 回覆 URL 和登入 URL**上 hello**機器人學網域和 Url Kantega SSO** Azure 入口網站中的區段。

    b. 按一下 [下一步] 。

19. 在 hello**中繼資料匯入**區段中，執行下列步驟： 
...
20. 在 hello**名稱和 SSO 位置**區段中，執行下列步驟：
...
24. 在 hello**已知的 Azure AD 網域**區段中，執行下列步驟： 
...
tooenable Azure AD 使用者 toolog 中 tooConfluence，它們必須佈建到機器人學。 中的機器人學的 Kantega SSO hello 案例中，佈建須手動進行。

**tooprovision 使用者帳戶，執行下列步驟的 hello:**

1. 系統管理員身分登入 tooyour Kantega SSO 機器人學公司網站。

2. 將滑鼠停留在齒輪，然後按一下 hello**使用者管理**。

    ![新增員工](./media/active-directory-saas-kantegassoforconfluence-tutorial/user1.png) 

3. 在 使用者 區段下，按一下 新增使用者 索引標籤。在 hello **加入使用者**對話方塊頁面上，執行下列步驟的 hello:
...
在本節中，以啟用許 Simon toouse Azure 單一登入授與存取 tooKantega 機器人學的 SSO。

![指派使用者][200] 

**tooassign 許 Simon tooKantega SSO 的機器人學，執行下列步驟的 hello:**

1. 在 hello Azure 入口網站，開啟 hello 應用程式檢視，然後導覽 toohello 目錄檢視，並跳過**企業應用程式**然後按一下 **所有應用程式**。

    ![指派使用者][201] 

2. 在 [hello] 應用程式清單中，選取**機器人學的 Kantega SSO**。
...
當您按一下 hello Kantega SSO 機器人學磚 hello 存取面板中時，您應該取得自動登入 tooyour Kantega SSO 機器人學應用程式。
