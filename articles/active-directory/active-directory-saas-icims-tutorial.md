...
在此教學課程中，您學會如何 toointegrate ICIMS 與 Azure Active Directory (Azure AD)。

與 Azure AD 整合 ICIMS 可以提供下列優點 hello:

- 您可以控制存取 tooICIMS Azure AD 中
- 您可以啟用您的使用者 tooautomatically get 登入 tooICIMS （單一登入） 具有其 Azure AD 帳戶
...
tooconfigure ICIMS 與 Azure AD 整合，您需要下列項目 hello:
...
1. 從 hello 圖庫加入 ICIMS
2. 設定並測試 Azure AD 單一登入

## <a name="adding-icims-from-hello-gallery"></a>從 hello 圖庫加入 ICIMS
tooconfigure hello 整合 ICIMS 到 Azure AD，您需要 tooadd ICIMS hello 圖庫 tooyour 清單中的受管理的 SaaS 應用程式。

**tooadd ICIMS 從 hello 組件庫中，執行下列步驟的 hello:**
...
4. 在 hello 搜尋方塊中，輸入**ICIMS**，選取**ICIMS**然後按一下 從結果面板**新增**按鈕 tooadd hello 應用程式。

    ![ICIMS hello [結果] 清單中](./media/active-directory-saas-icims-tutorial/tutorial_icims_addfromgallery.png)

##  <a name="configure-and-test-azure-ad-single-sign-on"></a>設定和測試 Azure AD 單一登入
在本節中，您會以名為 "Britta Simon" 的測試使用者身分，設定及測試與 ICIMS 搭配運作的 Azure AD 單一登入。

單一登入 toowork，Azure AD 需要 tooknow hello 的對等項目的使用者中 ICIMS 是 tooa 使用者在 Azure AD 中。 換句話說，Azure AD 使用者與 hello ICIMS 中相關的使用者之間的連結關聯性需要 toobe 建立。

ICIMS 中, 指派的 hello hello 值**使用者名**做為 hello hello 值的 Azure AD 中**Username** tooestablish hello 連結關聯性。

tooconfigure 及 ICIMS 與 Azure AD 單一登入的測試，您必須遵循的建置組塊 toocomplete hello:
...
3. **[建立測試使用者 ICIMS](#create-an-icims-test-user)**  -toohave 許 Simon ICIMS 所連結的 toohello Azure AD 使用者表示法中對應項目。
...
在本節中，您可以啟用 Azure AD 單一登入 hello Azure 入口網站中，並 ICIMS 應用程式中設定單一登入。

**tooconfigure Azure AD 單一登入與 ICIMS，執行下列步驟的 hello:**

1. 在 Azure 入口網站上 hello hello **ICIMS**應用程式整合頁面上，按一下 **單一登入**。
...
3. 在 hello **ICIMS 網域和 Url**區段中，執行下列步驟的 hello:

    ![ICIMS 網域及 URL 單一登入資訊](./media/active-directory-saas-icims-tutorial/tutorial_icims_url.png)

    a. 在 hello**登入 URL**文字方塊中，輸入 URL，使用下列模式的 hello:`https://<tenant name>.icims.com`

    b. 在 hello**識別碼**文字方塊中，輸入 URL，使用下列模式的 hello:`https://<tenant name>.icims.com`

    > [!NOTE] 
    > 這些都不是真正的值。 更新這些值與實際的 hello 登入 URL 和識別項。 請連絡[ICIMS 用戶端支援小組](https://www.icims.com/contact-us)tooget 這些值。 
...
6. 在 hello **ICIMS 組態**區段中，按一下**設定 ICIMS** tooopen**設定登入**視窗。 複製 hello**登出 URL、 SAML 實體識別碼、 和 SAML 單一登入服務 URL**從 hello**快速參考 > 一節。**

    ![ICIMS 組態](./media/active-directory-saas-icims-tutorial/tutorial_icims_configure.png) 

7. tooconfigure 單一登入上**ICIMS**端，您需要下載 toosend hello**中繼資料 XML**，**登出 URL、 SAML 實體識別碼、 和 SAML 單一登入服務 URL**太[ICIMS 支援小組](https://www.icims.com/contact-us)。 設定此設定 toohave hello SAML SSO 連線兩端上正確設定這些欄位。
...
hello 本節目標在於 toocreate ICIMS 中呼叫許 Simon 的使用者。 使用[ICIMS 支援小組](https://www.icims.com/contact-us)tooadd hello hello ICIMS 帳戶的使用者。 

### <a name="assign-hello-azure-ad-test-user"></a>指派給 Azure AD hello 測試使用者

在本節中，您可以授與存取 tooICIMS 啟用許 Simon toouse Azure 單一登入。

![指派 hello 使用者角色][200]

**tooassign 許 Simon tooICIMS，執行下列步驟的 hello:**

1. 在 hello Azure 入口網站，開啟 hello 應用程式檢視，然後導覽 toohello 目錄檢視，並跳過**企業應用程式**然後按一下 **所有應用程式**。

    ![指派使用者][201] 

2. 在 [hello] 應用程式清單中，選取**ICIMS**。
...
當您按一下的 hello ICIMS 磚 hello 存取面板中時，您應該取得自動登入 tooyour ICIMS 應用程式。
