...
在此教學課程中，您學會如何 toointegrate Voyance 與 Azure Active Directory (Azure AD)。

與 Azure AD 整合 Voyance 可以提供下列優點 hello:

- 您可以控制存取 tooVoyance Azure AD 中
- 您可以啟用您的使用者 tooautomatically get 登入 tooVoyance （單一登入） 具有其 Azure AD 帳戶
...
tooconfigure Voyance 與 Azure AD 整合，您需要下列項目 hello:
...
1. 從 hello 圖庫加入 Voyance
2. 設定並測試 Azure AD 單一登入

## <a name="adding-voyance-from-hello-gallery"></a>從 hello 圖庫加入 Voyance
tooconfigure hello 整合 Voyance 到 Azure AD，您需要 tooadd Voyance hello 圖庫 tooyour 清單中的受管理的 SaaS 應用程式。

**tooadd Voyance 從 hello 組件庫中，執行下列步驟的 hello:**
...
4. 在 [hello] 搜尋方塊中，輸入**Voyance**，選取**Voyance**然後按一下 [從結果面板**新增**按鈕 tooadd hello 應用程式。
...
單一登入 toowork，Azure AD 需要 tooknow hello 的對等項目的使用者中 Voyance 是 tooa 使用者在 Azure AD 中。 換句話說，Azure AD 使用者與 hello Voyance 中相關的使用者之間的連結關聯性需要 toobe 建立。

Voyance 中, 指派的 hello hello 值**使用者名**做為 hello hello 值的 Azure AD 中**Username** tooestablish hello 連結關聯性。

tooconfigure 及 Voyance 與 Azure AD 單一登入的測試，您必須遵循的建置組塊 toocomplete hello:
...
3. **[建立測試使用者 Voyance](#create-a-voyance-test-user) ** -toohave 許 Simon Voyance 所連結的 toohello Azure AD 使用者表示法中對應項目。
...
在本節中，您可以啟用 Azure AD 單一登入 hello Azure 入口網站中，並 Voyance 應用程式中設定單一登入。

**tooconfigure Azure AD 單一登入與 Voyance，執行下列步驟的 hello:**

1. 在 Azure 入口網站上 hello hello **Voyance**應用程式整合頁面上，按一下 [**單一登入**。
...
3. 在 [hello **Voyance 網域和 Url**區段中，執行下列步驟，如果您想 tooconfigure hello 應用程式中的 hello **IDP**初始模式：

    ![IDP 的 Voyance 網域和 URL 單一登入資訊](./media/active-directory-saas-voyance-tutorial/tutorial_voyance_url1.png)

    a. 在 [hello**識別碼**文字方塊中，輸入 URL，使用下列模式的 hello:`https://<companyname>.nyansa.com`

    b. 在 [hello**回覆 URL**文字方塊中，輸入 URL，使用下列模式的 hello:`https://<companyname>.nyansa.com/saml/create/`
...
4. 請檢查**顯示進階的 URL 設定**並執行下列步驟，如果您想 tooconfigure hello 應用程式中的 hello **SP**初始模式：
    ...
    在 [hello**登入 URL**文字方塊中，輸入 URL，使用下列模式的 hello:`https://<companyname>.nyansa.com/`
     
    > [!NOTE] 
    > 這些都不是真正的值。 更新這些值與實際的 hello，識別項、 回覆 URL 和登入 URL。 請連絡[Voyance 用戶端支援小組](mailto:support@nyansa.com)tooget 這些值。 
...
7. 在 [hello **Voyance 組態**區段中，按一下**設定 Voyance** tooopen**設定登入**視窗。 複製 hello **SAML 單一登入服務 URL**從 hello**快速參考 > 一節。**

    ![Voyance 組態](./media/active-directory-saas-voyance-tutorial/tutorial_voyance_configure.png) 

8. 在不同的網頁瀏覽器視窗中，以系統管理員身分登入 tooyour Voyance 租用戶。

9. 移 toohello 右上角的 hello 巡覽列，然後按一下 [hello] 下拉式清單，指出 「**Acme 大學**"。
...
12. 按一下 hello"**已停用 SSO**」 按鈕 tooconfigure Azure AD 使用 SAML 2.0 IdP 身分。

    ![在應用程式端設定單一登入 [SSO 已停用] 按鈕](./media/active-directory-saas-voyance-tutorial/tutorial_voyance_004.png)

13. 跳過**SAML v2**區段，然後執行下列步驟：

    ![在應用程式端設定單一登入 SAML v2](./media/active-directory-saas-voyance-tutorial/tutorial_voyance_005.png)
    
    a. 選取 [啟用] 。
    
    b. 貼上**SAML 單一登入服務 URL**，其中您從 hello Azure 入口網站複製到 hello **IdP 登入 URL**文字方塊。

    c. 在 [記事本]，複製到剪貼簿，其內容的 hello 中開啟下載的 Base64 編碼憑證，然後將它貼入 toohello **IdP 憑證**文字方塊。
...
hello 本節目標在於 toocreate Voyance 中呼叫許 Simon 的使用者。 Voyance 支援預設啟用的 Just-In-Time 佈建。 在這一節沒有您需要進行的動作項目。 如果尚未存在期間嘗試 tooaccess Voyance，建立新的使用者。

>[!NOTE]
>若要手動 toocreate 使用者，您需要 toocontact [Voyance 支援小組](maiLto:support@nyansa.com)。

### <a name="assign-hello-azure-ad-test-user"></a>指派給 Azure AD hello 測試使用者

在本節中，您可以授與存取 tooVoyance 啟用許 Simon toouse Azure 單一登入。

![指派 hello 使用者角色][200]

**tooassign 許 Simon tooVoyance，執行下列步驟的 hello:**

1. 在 hello Azure 入口網站，開啟 hello 應用程式檢視，然後導覽 toohello 目錄檢視，並跳過**企業應用程式**然後按一下 [**所有應用程式**。

    ![指派使用者][201] 

2. 在 [hello] 應用程式清單中，選取**Voyance**。
...
當您按一下 hello Voyance 磚 hello 存取面板中的時，您應該取得自動登入 tooyour Voyance 應用程式。
