...
在此教學課程中，您學會如何 toointegrate Jobbadmin 與 Azure Active Directory (Azure AD)。

與 Azure AD 整合 Jobbadmin 可以提供下列優點 hello:

- 您可以控制存取 tooJobbadmin Azure AD 中
- 您可以啟用您的使用者 tooautomatically get 登入 tooJobbadmin （單一登入） 具有其 Azure AD 帳戶
...
tooconfigure Jobbadmin 與 Azure AD 整合，您需要下列項目 hello:
...
1. 從 hello 圖庫加入 Jobbadmin
2. 設定並測試 Azure AD 單一登入

## <a name="adding-jobbadmin-from-hello-gallery"></a>從 hello 圖庫加入 Jobbadmin
tooconfigure hello 整合 Jobbadmin 到 Azure AD，您需要 tooadd Jobbadmin hello 圖庫 tooyour 清單中的受管理的 SaaS 應用程式。

**tooadd Jobbadmin 從 hello 組件庫中，執行下列步驟的 hello:**
...
單一登入 toowork，Azure AD 需要 tooknow hello 的對等項目的使用者中 Jobbadmin 是 tooa 使用者在 Azure AD 中。 換句話說，Azure AD 使用者與 hello Jobbadmin 中相關的使用者之間的連結關聯性需要 toobe 建立。

Jobbadmin 中, 指派的 hello hello 值**使用者名**做為 hello hello 值的 Azure AD 中**Username** tooestablish hello 連結關聯性。

tooconfigure 及 Jobbadmin 與 Azure AD 單一登入的測試，您必須遵循的建置組塊 toocomplete hello:

1. **[設定 Azure AD 單一登入](#configuring-azure-ad-single-sign-on)** -tooenable 使用者 toouse 這項功能。
2. **[建立 Azure AD 測試使用者](#creating-an-azure-ad-test-user)** -tootest Azure AD 單一登入與許 Simon。
3. **[建立測試使用者 Jobbadmin](#creating-a-jobbadmin-test-user)**  -toohave 許 Simon Jobbadmin 所連結的 toohello Azure AD 使用者表示法中對應項目。
4. **[指派 hello Azure AD 的測試使用者](#assigning-the-azure-ad-test-user)** -tooenable 許 Simon toouse Azure AD 單一登入。
5. **[測試單一登入](#testing-single-sign-on)** -tooverify 是否 hello 組態工作。

### <a name="configuring-azure-ad-single-sign-on"></a>設定 Azure AD 單一登入

在本節中，您可以啟用 Azure AD 單一登入 hello Azure 入口網站中，並 Jobbadmin 應用程式中設定單一登入。

**tooconfigure Azure AD 單一登入與 Jobbadmin，執行下列步驟的 hello:**

1. 在 Azure 入口網站上 hello hello **Jobbadmin**應用程式整合頁面上，按一下 **單一登入**。
...
3. 在 hello **Jobbadmin 網域和 Url**區段中，執行下列步驟的 hello:

    ![設定單一登入](./media/active-directory-saas-jobbadmin-tutorial/tutorial_jobbadmin_url.png)

    a. 在 hello**登入 URL**文字方塊中，輸入 URL，使用下列模式的 hello:`https://<instancename>.jobbnorge.no/auth/saml2/login.ashx`

    b. 在 hello**識別碼**文字方塊中，輸入 URL，使用下列模式的 hello:`https://<instancename>.jobnorge.no`

    c. 在 hello**回覆 URL**文字方塊中，輸入 URL，使用下列模式的 hello:`https://<instancename>.jobbnorge.no/auth/saml2/login.ashx`

    > [!NOTE] 
    > 這些都不是真正的值。 更新這些值與實際的 hello 登入 URL 和識別項。 請連絡[Jobbadmin 用戶端支援小組](https://www.jobbnorge.no/om-oss/kontakt-oss)tooget 這些值。 
...
6. tooconfigure 單一登入上**Jobbadmin**端，您需要下載 toosend hello**中繼資料 XML**太[Jobbadmin 支援小組](https://www.jobbnorge.no/om-oss/kontakt-oss)。 設定此設定 toohave hello SAML SSO 連線兩端上正確設定這些欄位。
...
tooenable Azure AD 使用者 toolog 中 tooJobbadmin，它們必須佈建到 Jobbadmin。
 
請連絡[Jobbadmin 支援小組](https://www.jobbnorge.no/om-oss/kontakt-oss)tooget hello 使用者在其端加入。

### <a name="assigning-hello-azure-ad-test-user"></a>指派 hello Azure AD 的測試使用者

在本節中，您可以授與存取 tooJobbadmin 啟用許 Simon toouse Azure 單一登入。

![指派使用者][200] 

**tooassign 許 Simon tooJobbadmin，執行下列步驟的 hello:**

1. 在 hello Azure 入口網站，開啟 hello 應用程式檢視，然後導覽 toohello 目錄檢視，並跳過**企業應用程式**然後按一下 **所有應用程式**。

    ![指派使用者][201] 

2. 在 [hello] 應用程式清單中，選取**Jobbadmin**。
...
當您按一下 hello Jobbadmin 磚 hello 存取面板中的時，您應該取得 Jobbadmin 應用程式的登入頁面。
