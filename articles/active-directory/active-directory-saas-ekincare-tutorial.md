...
在此教學課程中，您學會如何 toointegrate eKincare 與 Azure Active Directory (Azure AD)。

與 Azure AD 整合 eKincare 可以提供下列優點 hello:

- 您可以控制存取 tooeKincare Azure AD 中
- 您可以啟用您的使用者 tooautomatically get 登入 tooeKincare （單一登入） 具有其 Azure AD 帳戶
...
tooconfigure eKincare 與 Azure AD 整合，您需要下列項目 hello:
...
1. 從 hello 圖庫加入 eKincare
2. 設定並測試 Azure AD 單一登入

## <a name="adding-ekincare-from-hello-gallery"></a>從 hello 圖庫加入 eKincare
tooconfigure hello 整合 eKincare 到 Azure AD，您需要 tooadd eKincare hello 圖庫 tooyour 清單中的受管理的 SaaS 應用程式。

**tooadd eKincare 從 hello 組件庫中，執行下列步驟的 hello:**
...
單一登入 toowork，Azure AD 需要 tooknow hello 的對等項目的使用者中 eKincare 是 tooa 使用者在 Azure AD 中。 換句話說，Azure AD 使用者與 hello eKincare 中相關的使用者之間的連結關聯性需要 toobe 建立。

EKincare 中, 指派的 hello hello 值**使用者名**做為 hello hello 值的 Azure AD 中**Username** tooestablish hello 連結關聯性。

tooconfigure 及 eKincare 與 Azure AD 單一登入的測試，您必須遵循的建置組塊 toocomplete hello:

1. **[設定 Azure AD 單一登入](#configuring-azure-ad-single-sign-on)** -tooenable 使用者 toouse 這項功能。
2. **[建立 Azure AD 測試使用者](#creating-an-azure-ad-test-user)** -tootest Azure AD 單一登入與許 Simon。
3. **[建立測試使用者 eKincare](#creating-a-ekincare-test-user) ** -toohave 是連結的 toohello Azure AD 使用者表示法的 eKincare 的許 Simon 的對應項目。
4. **[指派 hello Azure AD 的測試使用者](#assigning-the-azure-ad-test-user)** -tooenable 許 Simon toouse Azure AD 單一登入。
5. **[測試單一登入](#testing-single-sign-on)** -tooverify 是否 hello 組態工作。

### <a name="configuring-azure-ad-single-sign-on"></a>設定 Azure AD 單一登入

在本節中，您可以啟用 Azure AD 單一登入 hello Azure 入口網站中，並 eKincare 應用程式中設定單一登入。

**tooconfigure Azure AD 單一登入與 eKincare，執行下列步驟的 hello:**

1. 在 Azure 入口網站上 hello hello **eKincare**應用程式整合頁面上，按一下 [**單一登入**。
...
3. 在 [hello **eKincare 網域和 Url**區段中，執行下列步驟的 hello:

    ![設定單一登入](./media/active-directory-saas-ekincare-tutorial/tutorial_ekincare_url.png)

    a. 在 [hello**識別碼**文字方塊中，輸入 URL，使用下列模式的 hello:`https://<instancename>.ekincare.com/`

    b. 在 [hello**回覆 URL**文字方塊中，輸入 URL，使用下列模式的 hello:`https://<instancename>.ekincare.com/hul/saml`

    > [!NOTE] 
    > 這些都不是真正的值。 以 hello 實際的識別項和回覆 URL 更新這些值。 請連絡[eKincare 支援小組](mailto:tech@ekincare.com)tooget 這些值。
 
4. eKincare 應用程式預期 hello SAML 判斷提示，以特定格式。 設定下列宣告為此應用程式的 hello。 您可以從 hello 管理 hello 這些屬性的值"**使用者屬性**」 一節，在應用程式整合頁面上。 hello 下列螢幕擷取畫面顯示此組態範例。

    hello 宣告名稱一定會**"employeeid"**和其中我們有對應的 toouser.extensionattribute1 包含 hello employeeid 的 hello 使用者 hello 值。 hello 其他兩個宣告的名稱也就是 **「 organizationid"**和**"organizationname"**一定會是相同，而且其值分別包含 hello hello 組織 hello 使用者的詳細資料。
...
8. tooconfigure 單一登入上**eKincare**端，您需要下載 toosend hello**中繼資料 XML**太[eKincare 支援小組](mailto:tech@ekincare.com)。 設定此設定 toohave hello SAML SSO 連線兩端上正確設定這些欄位。
...
在本節中，您可以授與存取 tooeKincare 啟用許 Simon toouse Azure 單一登入。

![指派使用者][200] 

**tooassign 許 Simon tooeKincare，執行下列步驟的 hello:**

1. 在 hello Azure 入口網站，開啟 hello 應用程式檢視，然後導覽 toohello 目錄檢視，並跳過**企業應用程式**然後按一下 [**所有應用程式**。

    ![指派使用者][201] 

2. 在 [hello] 應用程式清單中，選取**eKincare**。
...
當您按一下 hello eKincare 磚 hello 存取面板中的時，您應該取得自動登入 tooyour eKincare 應用程式。
