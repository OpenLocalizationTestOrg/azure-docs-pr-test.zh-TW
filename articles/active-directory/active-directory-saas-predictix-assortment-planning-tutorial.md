...
在此教學課程中，您學會如何 toointegrate Predictix 規劃與 Azure Active Directory (Azure AD) 的分類。

整合 Predictix 分類的規劃與 Azure AD 可以提供下列優點 hello:

- 您可以控制存取 tooPredictix 分類規劃 Azure AD 中。
- 您可以使用其 Azure AD 帳戶啟用您使用者 tooautomatically get 登入 tooPredictix 分類規劃 （單一登入）。
...
tooconfigure Predictix 分類規劃 Azure AD 整合，您需要下列項目 hello:
...
1. 從 hello 組件庫中加入 Predictix 分類的規劃
2. 設定並測試 Azure AD 單一登入

## <a name="adding-predictix-assortment-planning-from-hello-gallery"></a>從 hello 組件庫中加入 Predictix 分類的規劃
tooconfigure hello 整合 Predictix 分類規劃到 Azure AD，您需要從受管理的 SaaS 應用程式的 hello 圖庫 tooyour 清單 tooadd Predictix 分類的規劃。

**tooadd Predictix 分類規劃從 hello 組件庫中，執行下列步驟的 hello:**
...
4. 在 hello 搜尋方塊中，輸入**Predictix 分類規劃**，選取**Predictix 分類規劃**然後按一下 從結果面板**新增**按鈕 tooadd hello 應用程式。
...
單一登入 toowork，Azure AD 需要 tooknow hello 的對等項目的使用者 Predictix 分類規劃是 tooa 使用者在 Azure AD 中。 換句話說，Azure AD 使用者與 hello Predictix 分類規劃的相關的使用者之間的連結關聯性需要 toobe 建立。

Predictix 分類規劃 hello hello 值指派**使用者名**做為 hello hello 值的 Azure AD 中**Username** tooestablish hello 連結關聯性。

tooconfigure 及 Azure AD 單一登入具有 Predictix 分類計劃的測試，您必須遵循的建置組塊 toocomplete hello:
...
3. **[建立測試使用者 Predictix 分類規劃](#create-a-predictix-assortment-planning-test-user)** -toohave 許 Simon Predictix 分類規劃連結的 toohello Azure AD 使用者表示法的對應項目。
...
在本節中，您可以啟用 Azure AD 單一登入 hello Azure 入口網站中，並 Predictix 分類規劃應用程式中設定單一登入。

**利用 Predictix 分類的規劃，Azure AD 單一登入 tooconfigure 執行 hello 下列步驟：**

1. 在 Azure 入口網站上 hello hello **Predictix 分類規劃**應用程式整合頁面上，按一下 **單一登入**。
...
3. 在 hello **Predictix 分類規劃網域和 Url**區段中，執行下列步驟的 hello:
    ...
    > [!NOTE] 
    > 這些都不是真正的值。 更新這些值與實際的 hello 登入 URL 和識別項。 請連絡[Predictix 分類規劃用戶端支援小組](http://www.infor.com/support)tooget 這些值。 
...
6. 在 hello **Predictix 分類規劃 Configuration**區段中，按一下**設定 Predictix 分類規劃**tooopen**設定登入**視窗。 複製 hello**登出 URL、 SAML 實體識別碼、 和 SAML 單一登入服務 URL**從 hello**快速參考 > 一節。**
...
7. tooconfigure 單一登入上**Predictix 分類規劃**端，您需要下載 toosend hello **Certificate(Base64)**， **SAML 實體識別碼**， **SAML 單一登入服務 URL**，和**登出 URL**太[Predictix 分類規劃支援小組](http://www.infor.com/support)。 設定此設定 toohave hello SAML SSO 連線兩端上正確設定這些欄位。
...
在本節中，您要在 Predictix Assortment Planning 中建立名為 Britta Simon 的使用者。 請使用[Predictix 分類規劃支援小組](http://www.infor.com/contact/)tooadd hello hello Predictix 分類規劃平台的使用者。
 > [!NOTE]
...
在本節中，以啟用許 Simon toouse Azure 單一登入授與存取 tooPredictix 分類的規劃。

![指派 hello 使用者角色][200] 

**tooassign 許 Simon tooPredictix 分類的規劃，執行下列步驟的 hello:**

1. 在 hello Azure 入口網站，開啟 hello 應用程式檢視，然後導覽 toohello 目錄檢視，並跳過**企業應用程式**然後按一下 **所有應用程式**。

    ![指派使用者][201] 

2. 在 [hello] 應用程式清單中，選取**Predictix 分類規劃**。
...
當您按一下 hello Predictix 分類規劃並排顯示 hello 存取面板中的時，您應該取得自動登入 tooyour Predictix 分類規劃應用程式。
