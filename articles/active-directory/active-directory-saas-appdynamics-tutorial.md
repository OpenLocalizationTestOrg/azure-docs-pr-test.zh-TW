...
在此教學課程中，您學會如何 toointegrate AppDynamics 與 Azure Active Directory (Azure AD)。

與 Azure AD 整合 AppDynamics 可以提供下列優點 hello:

- 您可以控制存取 tooAppDynamics Azure AD 中
- 您可以啟用您的使用者 tooautomatically get 登入 tooAppDynamics （單一登入） 具有其 Azure AD 帳戶
...
tooconfigure 與 AppDynamics 的 Azure AD 整合，您需要下列項目 hello:
...
1. 從 hello 圖庫加入 AppDynamics
2. 設定並測試 Azure AD 單一登入

## <a name="adding-appdynamics-from-hello-gallery"></a>從 hello 圖庫加入 AppDynamics
tooconfigure hello 整合 AppDynamics 的 Azure AD，您需要從受管理的 SaaS 應用程式的 hello 圖庫 tooyour 清單 tooadd AppDynamics。

**tooadd AppDynamics 從 hello 組件庫中，執行下列步驟的 hello:**
...
單一登入 toowork，Azure AD 需要 tooknow hello 的對等項目的使用者在 AppDynamics 中是 tooa 使用者在 Azure AD 中。 換句話說，Azure AD 使用者與 hello AppDynamics 中相關的使用者之間的連結關聯性需要 toobe 建立。

在 AppDynamics 中，將指派的 hello hello 值**使用者名**做為 hello hello 值的 Azure AD 中**Username** tooestablish hello 連結關聯性。

tooconfigure 及測試 Azure AD 單一登入 AppDynamics，您必須遵循的建置組塊 toocomplete hello:

1. **[設定 Azure AD 單一登入](#configuring-azure-ad-single-sign-on)** -tooenable 使用者 toouse 這項功能。
2. **[建立 Azure AD 測試使用者](#creating-an-azure-ad-test-user)** -tootest Azure AD 單一登入與許 Simon。
3. **[建立測試使用者 AppDynamics](#creating-an-appdynamics-test-user)**  -toohave 許 Simon AppDynamics 所連結的 toohello Azure AD 使用者表示法中對應項目。
4. **[指派 hello Azure AD 的測試使用者](#assigning-the-azure-ad-test-user)** -tooenable 許 Simon toouse Azure AD 單一登入。
5. **[測試單一登入](#testing-single-sign-on)** -tooverify 是否 hello 組態工作。

### <a name="configuring-azure-ad-single-sign-on"></a>設定 Azure AD 單一登入

在本節中，您可以啟用 Azure AD 單一登入 hello Azure 入口網站中，並設定單一登入 AppDynamics 的應用程式中。

**tooconfigure Azure AD 單一登入 AppDynamics，執行下列步驟的 hello:**

1. 在 Azure 入口網站上 hello hello **AppDynamics**應用程式整合頁面上，按一下 **單一登入**。
...
3. 在 hello **AppDynamics 網域和 Url**區段中，執行下列步驟的 hello:

    ![設定單一登入](./media/active-directory-saas-appdynamics-tutorial/tutorial_appdynamics_url.png)

    a. 在 hello**登入 URL**文字方塊中，輸入 URL，使用下列模式的 hello:`https://<companyname>.saas.appdynamics.com`

    b. 在 hello**識別碼**文字方塊中，輸入 URL，使用下列模式的 hello:`https://<companyname>.saas.appdynamics.com/controller`

    > [!NOTE] 
    > 這些都不是真正的值。 更新這些值與實際的 hello 登入 URL 和識別項。 請連絡[AppDynamics 用戶端支援小組](https://www.appdynamics.com/support/)tooget 這些值。 
...
6. 在 hello **AppDynamics 組態**區段中，按一下**設定 AppDynamics** tooopen**設定登入**視窗。 複製 hello**登出 URL 和 SAML 單一登入服務 URL**從 hello**快速參考 > 一節。**

    ![設定單一登入](./media/active-directory-saas-appdynamics-tutorial/tutorial_appdynamics_configure.png) 

7. 在不同的網頁瀏覽器視窗中，系統管理員身分登入 tooyour AppDynamics 公司網站。

8. 在 hello hello 上方的工具列中按一下**設定**，然後按一下**管理**。
   
    ![管理](./media/active-directory-saas-appdynamics-tutorial/ic790216.png "管理")

9. 按一下 hello**驗證提供者** 索引標籤。
   
    ![驗證提供者](./media/active-directory-saas-appdynamics-tutorial/ic790224.png "驗證提供者")

10. 在 hello**驗證提供者**區段中，執行下列步驟的 hello:
   
    ![SAML 設定](./media/active-directory-saas-appdynamics-tutorial/ic790225.png "SAML 設定")   

    a. 針對 [驗證提供者]，選取 [SAML]。

    b. 在 hello**登入 URL**文字方塊中，貼上 hello 值**SAML 單一登入服務 URL**您從 Azure 入口網站複製的。

    c. 在 hello**登出 URL**文字方塊中，貼上 hello 值**登出 URL**您從 Azure 入口網站複製的。
       
    d. 在記事本中，複製到剪貼簿，其內容的 hello 中開啟 base-64 編碼的憑證，然後將它貼入 toohello**憑證**文字方塊
...
tooenable Azure AD 使用者 toolog 中 tooAppDynamics，它們必須佈建到 AppDynamics。 在 AppDynamics 的 hello 案例中，佈建須手動進行。

**tooconfigure 使用者佈建，執行下列步驟的 hello:**

1. 登入 tooyour AppDynamics 公司網站的系統管理員身分。

2. 跳過**使用者**，然後按一下  **+**  tooopen hello **Create User**對話方塊。
   
    ![使用者](./media/active-directory-saas-appdynamics-tutorial/ic790229.png "使用者")

3. 在 hello **Create User**區段中，執行下列步驟的 hello:
   
    ![建立使用者](./media/active-directory-saas-appdynamics-tutorial/ic790230.png "建立使用者")
   
    a. 型別 hello **Username**，**名稱**，**電子郵件**，**新密碼**，**重覆新密碼**的有效 AAD想成 hello tooprovision 帳戶相關的文字方塊。

    b. 按一下 [儲存] 。

    >[!NOTE]
    >您可以使用任何其他 AppDynamics 使用者帳戶建立工具或 Api 提供 AppDynamics tooprovision Azure AD 使用者帳戶。

### <a name="assigning-hello-azure-ad-test-user"></a>指派 hello Azure AD 的測試使用者

在本節中，您可以授與存取 tooAppDynamics 啟用許 Simon toouse Azure 單一登入。

![指派使用者][200] 

**tooassign 許 Simon tooAppDynamics，執行下列步驟的 hello:**

1. 在 hello Azure 入口網站，開啟 hello 應用程式檢視，然後導覽 toohello 目錄檢視，並跳過**企業應用程式**然後按一下 **所有應用程式**。

    ![指派使用者][201] 

2. 在 [hello] 應用程式清單中，選取**AppDynamics**。
...
當您按一下的 hello AppDynamics 磚 hello 存取面板中時，您應該取得自動登入 tooyour AppDynamics 的應用程式。
