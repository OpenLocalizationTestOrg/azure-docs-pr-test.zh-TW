...
---
# <a name="problems-signing-in-tooa-non-gallery-application-configured-for-federated-single-sign-on"></a>登入 tooa 非組件庫的應用程式設定同盟單一登入問題
...
8.  <span id="_Hlk477190042" class="anchor"></span>跳過**網域和 Url** > 一節。 請確認 hello 值在 hello 相符 hello 值顯示在 hello 錯誤中的 hello 識別碼值的文字方塊中的識別項。
...
1.  擷取 SAML 要求。 依照 hello 教學課程[如何 toodebug SAML 型單一登入 tooapplications 在 Azure AD 中](https://docs.microsoft.com/azure/active-directory/develop/active-directory-saml-debugging)toolearn toocapture hello SAML 要求的方式。
...
toosolve hello 問題，請從 hello 目錄移除 hello 應用程式。 接著，新增和重新設定 hello 應用程式，請遵循 hello 執行下列步驟：
...
8.  重新整理 Azure AD，並從 hello Azure AD 圖庫新增 hello 應用程式。 然後，設定一次 hello 應用程式。

之後重新設定 hello 應用程式，您應該能夠 toosign toohello 應用程式中。
