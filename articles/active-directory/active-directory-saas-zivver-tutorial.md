...
在此教學課程中，您學會如何 toointegrate ZIVVER 與 Azure Active Directory (Azure AD)。

與 Azure AD 整合 ZIVVER 可以提供下列優點 hello:

- 您可以控制存取 tooZIVVER Azure AD 中。
- 您可以啟用您的使用者 tooautomatically get 登入 tooZIVVER （單一登入） 具有其 Azure AD 帳戶。
...
tooconfigure ZIVVER 與 Azure AD 整合，您需要下列項目 hello:
...
1. 從 hello 圖庫加入 ZIVVER
2. 設定並測試 Azure AD 單一登入

## <a name="adding-zivver-from-hello-gallery"></a>從 hello 圖庫加入 ZIVVER
tooconfigure hello 整合 ZIVVER 到 Azure AD，您需要 tooadd ZIVVER hello 圖庫 tooyour 清單中的受管理的 SaaS 應用程式。

**tooadd ZIVVER 從 hello 組件庫中，執行下列步驟的 hello:**
...
4. 在 hello 搜尋方塊中，輸入**ZIVVER**，選取**ZIVVER**然後按一下 從結果面板**新增**按鈕 tooadd hello 應用程式。
...
單一登入 toowork，Azure AD 需要 tooknow hello 的對等項目的使用者中 ZIVVER 是 tooa 使用者在 Azure AD 中。 換句話說，Azure AD 使用者與 hello ZIVVER 中相關的使用者之間的連結關聯性需要 toobe 建立。

ZIVVER 中, 指派的 hello hello 值**使用者名**做為 hello hello 值的 Azure AD 中**Username** tooestablish hello 連結關聯性。

tooconfigure 及 ZIVVER 與 Azure AD 單一登入的測試，您必須遵循的建置組塊 toocomplete hello:
...
3. **[建立測試使用者 ZIVVER](#create-a-zivver-test-user)**  -toohave 許 Simon ZIVVER 所連結的 toohello Azure AD 使用者表示法中對應項目。
...
在本節中，您可以啟用 Azure AD 單一登入 hello Azure 入口網站中，並 ZIVVER 應用程式中設定單一登入。

**tooconfigure Azure AD 單一登入與 ZIVVER，執行下列步驟的 hello:**

1. 在 Azure 入口網站上 hello hello **ZIVVER**應用程式整合頁面上，按一下 **單一登入**。
...
3. 在 hello **ZIVVER 網域和 Url**區段中，執行下列步驟的 hello:

    ![ZIVVER 網域及 URL 單一登入資訊](./media/active-directory-saas-zivver-tutorial/tutorial_zivver_url.png)

    在 hello**識別碼**文字方塊中，型別 hello URL:`https://app.zivver.com/SAML/Zivver`
...
6. tooconfigure 單一登入上**ZIVVER**端，您需要下載 toosend hello**中繼資料 XML**太[ZIVVER 支援小組](https://support.zivver.com)。 設定此設定 toohave hello SAML SSO 連線兩端上正確設定這些欄位。
...
在本節中，您要在 ZIVVER 中建立名為 Britta Simon 的使用者。 使用[ZIVVER 支援小組](https://support.zivver.com)tooadd hello hello ZIVVER 平台的使用者。

### <a name="assign-hello-azure-ad-test-user"></a>指派給 Azure AD hello 測試使用者

在本節中，您可以授與存取 tooZIVVER 啟用許 Simon toouse Azure 單一登入。

![指派 hello 使用者角色][200] 

**tooassign 許 Simon tooZIVVER，執行下列步驟的 hello:**

1. 在 hello Azure 入口網站，開啟 hello 應用程式檢視，然後導覽 toohello 目錄檢視，並跳過**企業應用程式**然後按一下 **所有應用程式**。

    ![指派使用者][201] 

2. 在 [hello] 應用程式清單中，選取**ZIVVER**。
...
當您按一下的 hello ZIVVER 磚 hello 存取面板中時，您應該取得自動登入 tooyour ZIVVER 應用程式。
