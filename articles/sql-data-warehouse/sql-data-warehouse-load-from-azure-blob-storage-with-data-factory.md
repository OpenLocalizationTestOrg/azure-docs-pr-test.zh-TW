...
> 
 ...
 本教學課程示範如何 toocreate Azure Data Factory toomove 資料從 Azure 儲存體 Blob tooSQL 資料倉儲中的管線。 以下列的步驟將會看到 hello:
...
再開始本教學課程，您需要下列資源 toohave hello。
...
* **Azure Data Factory**: Azure Data Factory 會完成 hello 實際負載，因此您需要 toohave 其中一個，您可以使用 toobuild hello 資料移動的管線。如果您還沒有已經，了解如何 toocreate 其中一個步驟 1 中的[開始使用 Azure Data Factory （Data Factory 編輯器）][Get started with Azure Data Factory (Data Factory Editor)]。
...
一旦您擁有之所有 hello 片段準備好時，您就準備好 toocopy 範例資料 tooyour Azure 儲存體 Blob。

1. [下載範例資料][Download sample data]。 這項資料將會加入另一個的三年的銷售資料 tooyour AdventureWorksDW 範例資料。
...
1. 現在，我們也會對 SQL 資料倉儲定義我們的資料集。  我們一開始在 hello 同樣地，按一下 新的資料集' ' Azure SQL 資料倉儲 '。
...
最後，我們將會安裝並執行 Azure Data Factory 中的 hello 管線。  這是 hello 作業才會完成 hello 實際的資料移動。  您可以找到您可完成 SQL 資料倉儲與 Azure Data Factory 的 hello 作業的完整檢視[這裡][Move data tooand from Azure SQL Data Warehouse using Azure Data Factory]。

Hello '作者及部署' 區段中現在 '更命令' 按一下，然後按一下新管線 '。  建立 hello 管線之後，您可以使用下列程式碼 tootransfer hello 資料 tooyour 資料倉儲的 hello:
...
* [教學課程： 開始使用 Azure Data Factory] [ Tutorial: Get started with Azure Data Factory]這是 hello 核心教學課程來處理資料與 Azure Data Factory。 在本教學課程中，您將建置您的第一個管線使用 HDInsight tootransform 和分析每月為基礎的網站記錄檔。 請注意，本教學課程中沒有複製的活動。
* [教學課程： 將資料從 Azure 儲存體 Blob tooAzure SQL Database 中複製][Tutorial: Copy data from Azure Storage Blob tooAzure SQL Database]。 在本教學課程中，您將從 Azure 儲存體 Blob tooAzure SQL 資料庫建立 Azure Data Factory toocopy 資料中的管線。
