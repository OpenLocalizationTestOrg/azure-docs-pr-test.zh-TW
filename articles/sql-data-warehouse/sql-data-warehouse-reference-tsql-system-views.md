...
## <a name="sql-server-dmvs-applicable-toosql-data-warehouse"></a>SQL Server Dmv 適用 tooSQL 資料倉儲
下列 Dmv 適用 tooSQL 資料倉儲，但必須執行藉由連接 toohello hello**主要**資料庫。
...
SQL 資料倉儲公開 （expose) 許多 hello SQL Server 動態管理檢視 (Dmv)。 這些檢視下，當查詢在 SQL 資料倉儲中，會報告 hello hello 分佈上執行的 SQL 資料庫狀態。

由於 SQL 資料倉儲建置於 Microsoft MPP 技術上，SQL 資料倉儲和分析平台系統的 Parallel Data Warehouse (PDW) 使用 hello 相同的系統檢視表。

這就是為什麼每個 DMV 都有一個稱為 pdw_node_id 的特定資料行。 這是 hello hello 識別碼 hello 計算節點。 Pdw hello 運算節點會是 hello 架構更強大的概念。 在 SQL 資料倉儲中，hello 的架構有賴於加重 hello 分佈。

> [!NOTE]
> 這些檢視 toouse，'pdw_nodes_' 插入 hello 名稱 hello 下表所示。
> 
> 

| SQL 資料倉儲中的 DMV 名稱 | MSDN 上的連結 tooSQL 伺服器 TRANSACT-SQL 主題 |
|:--- |:--- |
