...
---
# <a name="create-an-azure-function-that-connects-tooan-azure-cosmos-db"></a>建立連接 tooan Azure Cosmos DB Azure 函式

這個範例指令碼會建立 Azure 函式應用程式，並連接 tooan Azure Cosmos DB 資料庫。
...
這個範例會建立 Azure 函式應用程式，並將 Cosmos DB 端點和存取金鑰 tooapp 設定。
...
Hello 指令碼範例執行後，hello 後續命令可以是使用的 tooremove hello 資源群組和所有相關聯的資源。
