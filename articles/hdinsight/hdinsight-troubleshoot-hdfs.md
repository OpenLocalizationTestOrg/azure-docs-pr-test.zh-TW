...
當手寫筆使用中 Apache Ambari Hadoop 分散式檔案系統 (HDFS) 裝載時，深入了解 hello 最常發生的問題和其解決方式。

## <a name="how-do-i-access-local-hdfs-from-inside-a-cluster"></a>如何存取 hello 從叢集內的本機 HDFS

### <a name="issue"></a>問題

從本機 HDFS hello 命令列和應用程式程式碼而不是使用 Azure Blob 儲存體或 Azure Data Lake Store 從內部存取 hello hello HDInsight 叢集。   

### <a name="resolution-steps"></a>解決步驟

1. 在 hello 命令提示字元中，使用`hdfs dfs -D "fs.default.name=hdfs://mycluster/" ...`字面意義，例如 hello 下列命令：
...
2. 從程式碼中，使用 hello URI`hdfs://mycluster/`字面意義，如下列範例應用程式的 hello 所示：
...
3. 執行的 hello 編譯檔案 (例如，名為的檔案`java-unit-tests-1.0.jar`) 上 hello HDInsight 叢集以 hello 下列命令：
...
hello 本機 HDFS 陷入 hello HDInsight 叢集上的安全模式。   

### <a name="detailed-description"></a>詳細描述

當您執行 hello 下列 HDFS 命令時，就會發生失敗：

```apache
hdfs dfs -D "fs.default.name=hdfs://mycluster/" -mkdir /temp
```

您會看到下列錯誤，當您執行 hello 命令 hello:
...
1. 取得 hello 狀態的 HDFS hello HDInsight 叢集上使用下列命令的 hello:
...
2. 檢查 hello 完整性的 HDFS hello HDInsight 叢集上使用下列命令的 hello:
