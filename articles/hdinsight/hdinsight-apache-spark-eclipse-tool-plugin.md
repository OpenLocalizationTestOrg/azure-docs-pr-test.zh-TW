...
當您開啟 Intellij 時，HDInsight Tools 自動偵測是否或不安裝 Scala 外掛程式。 按一下**確定**若要繼續，然後依照指示安裝 Eclipse Marketplace。
...
3. Scala 專案建立精靈會自動偵測您是否已安裝 Scala 外掛程式。 按一下**確定**繼續下載 Scala 外掛程式，然後依照指示來重新啟動 Eclipse。

    ![Scala 檢查](./media/hdinsight-apache-spark-eclipse-tool-plugin/auto-install-scala-2.png)
4. 在 [新增 HDInsight Scala 專案] 對話方塊中，提供下列值，然後按 [下一步]：
   * 輸入專案的名稱。
...
5.  在下一個對話方塊中，按一下 [程式庫] 索引標籤並保留預設值，然後按一下 [完成]。 
...
2. 按一下**作業**節點。 HDInsight 工具自動偵測是否或不安裝 E (fx) clipse 外掛程式。 按一下**確定**若要繼續，請依照指示安裝 Eclipse Marketplace 並重新啟動 Eclipse。

    ![安裝 E(fx)clipse](./media/hdinsight-apache-spark-eclipse-tool-plugin/auto-install-efxclipse.png)

3. 從 [作業] 節點開啟 [作業] 檢視。 在右窗格中，[Spark 作業檢視]  索引標籤會顯示已在叢集上執行的所有應用程式。 按一下您想要查看更多詳細資料的應用程式名稱。

    ![應用程式詳細資料](./media/hdinsight-apache-spark-intellij-tool-plugin/view-job-logs.png)
4. 如果您將滑鼠停留在作業圖形，它會顯示基本執行的作業資訊。 按一下即可在作業圖形顯示階段圖形和每項工作產生的資訊。

    ![作業階段詳細資料](./media/hdinsight-apache-spark-intellij-tool-plugin/Job-graph-stage-info.png)

5. 包括 Stderr 驅動程式、 驅動程式 Stdout 和目錄資訊的常用記錄檔中所列**記錄** 索引標籤。

    ![記錄詳細資料](./media/hdinsight-apache-spark-intellij-tool-plugin/Job-log-info.png)
6. 您也可以按一下視窗頂端的個別超連結，開啟 [Spark history UI] \(Spark 歷程記錄 UI\) 和 [YARN UI] \(應用程式層級)。
