...
3. 從 [選擇大小] 區段中，選取 VM 大小。 在本教學課程中，選取最小與最低的成本選項。 若要繼續，請使用 [選取] 按鈕。
...
5. 從 [購買] 區段中，選取 [購買] 按鈕以建立虛擬機器。

6. 一旦建立虛擬機器之後，會顯示其 [概觀] 區段。 從左側的清單中選取 [屬性]。 儲存 [公用 IP 位址] 和 [私人 IP 位址] 的值。 下一節將使用此資訊。
