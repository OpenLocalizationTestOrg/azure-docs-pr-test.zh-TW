...
本文將說明如何使用[Azure Resource Manager 範本](../azure-resource-manager/resource-group-authoring-templates.md)tooconfigure Azure 度量的警示。 這可讓您設定您的資源的警示時所建立的所有資源都監視正確的 tooensure tooautomatically。

hello 基本步驟如下所示：

1. 建立範本，為描述如何 toocreate hello 警示的 JSON 檔案。
2. [部署使用的任何部署方法的 hello 範本](../azure-resource-manager/resource-group-template-deploy.md)。

以下我們將描述如何 toocreate Resource Manager 範本使用先針對警示，然後 hello 另一項資源建立時警示。

## <a name="resource-manager-template-for-a-metric-alert"></a>度量警示的 Resource Manager 範本
toocreate 警示使用資源管理員範本，建立類型的資源`Microsoft.Insights/alertRules`和填寫所有相關的內容。 以下是建立警示規則的範本。
...
警示規則 hello 結構描述和屬性的說明[這裡會提供](https://msdn.microsoft.com/library/azure/dn933805.aspx)。

## <a name="resource-manager-template-for-a-resource-with-an-alert"></a>具有警示之資源的 Resource Manager 範本
在建立資源時建立警示，Resource Manager 範本上的警示通常很有用。 例如，您可能想 tooensure，"CPU %> 80"每次部署虛擬機器設定規則。 toodo，您將 hello 警示規則為 hello 資源陣列中的資源新增 VM 範本，並且新增相依性使用 hello`dependsOn`屬性 toohello VM 資源識別碼。 以下是完整的範例，建立 Windows VM，並將 hello CPU 使用率超過 80%時，通知訂閱管理員的警示。
...
* [新增診斷設定](monitoring-enable-diagnostic-logs-using-template.md)tooyour Resource Manager 範本
