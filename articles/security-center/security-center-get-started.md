...
資訊安全中心會收集虛擬機器 (VM) 的資料，以便評估其安全性狀態、提供安全性建議，並對您發出威脅警示。 當您第一次存取資訊安全中心時，訂用帳戶中的所有 VM 都會啟用資料收集。 資訊安全中心會在所有現有支援的 Azure 虛擬機器和任何新建立的 VM 上佈建 Microsoft Monitoring Agent。 若要深入了解資料收集的運作方式，請參閱[啟用資料收集](security-center-enable-data-collection.md)。

建議您啟用資料收集。 如果您是使用免費層的資訊安全中心，可以在安全性原則中關閉資料收集，以停用 VM 的資料收集。 資訊安全中心標準層上的訂用帳戶需要資料收集。 如需免費和標準定價層的詳細資訊，請參閱[資訊安全中心定價](security-center-pricing.md)。
...
2. 在 [安全性原則 - 定義每個訂用帳戶的原則] 刀鋒視窗上，選取訂用帳戶。
3. 在 [安全性原則] 刀鋒視窗上，[資料收集] 已啟用以自動收集記錄檔。 監視擴充功能會佈建在訂用帳戶的所有目前 VM 和新 VM 上。 (在資訊安全中心的免費層上，您可以將 [資料收集] 設定為 [關閉]，以退出資料收集。 將 [資料收集] 設定為 [關閉] 會讓資訊安全中心無法為您提供安全性警示與建議。)
4. 選取 [安全性原則] 刀鋒視窗上的 [預防原則]。 這會開啟 [預防原則] 刀鋒視窗。
5. 在 [預防原則] 刀鋒視窗上，開啟您想要作為安全性原則一部分的建議。 範例：
...
1. 返回 [資訊安全中心] 刀鋒視窗，然後選取 [安全性警示] 圖格。 [安全性警示] 刀鋒視窗會開啟並顯示警示清單。 資訊安全中心透過對安全性記錄檔和網路活動的分析，產生警示。 包含來自整合式合作夥伴解決方案的警示。
   ...
   >
   >
2. 選取警示以檢視其他資訊。 在此範例中，請選取 [探索到修改過的系統二進位檔]。 這會開啟刀鋒視窗，提供有關警示的其他詳細資料。
