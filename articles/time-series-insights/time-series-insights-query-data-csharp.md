...
---
# <a name="query-data-from-hello-azure-time-series-insights-environment-using-c"></a>查詢資料從使用 C# hello Azure 時間數列 Insights 環境

此 C# 範例會示範如何從 tooquery 資料 hello Azure 時間數列 Insights 環境。
hello 範例示範數個基本查詢 API 使用範例：
1. 在準備步驟中，取得 hello 存取權杖透過 hello Azure Active Directory API。 將此變數傳遞權杖 hello`Authorization`每個查詢 API 要求的標頭。 若要了解如何設定非互動式應用程式，請參閱[驗證與授權](time-series-insights-authentication-and-authorization.md)。 此外，請在 hello 開頭 hello 範例定義的所有 hello 常數均已正確都設定。
2. hello 使用者的環境的 hello 清單有存取 toois 取得。 Hello 環境的其中一個是做為重要，hello 環境，且此環境中進一步查詢資料。
3. 例如的 HTTPS 要求，要求可用性資料時感興趣的 hello 環境。
4. Web 通訊端要求的範例，為感興趣的 hello 環境要求事件彙總資料。 資料要求的 hello 整個可用性時間範圍。
...
如需完整查詢 API 參考 hello，請參閱 hello[查詢 API](/rest/api/time-series-insights/time-series-insights-reference-queryapi)文件。
