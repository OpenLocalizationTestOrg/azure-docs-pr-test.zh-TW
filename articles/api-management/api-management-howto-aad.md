...
在 [登入 URL] 中，輸入開發人員入口網站的登入 URL。 在此範例中，[登入 URL] 為 `https://aad03.portal.current.int-azure-api.net/signin`。 
...
從發佈者入口網站中 [外部身分識別] 索引標籤的 [Azure Active Directory] 區段，複製 [重新導向 URL]，並貼上至 [回覆 URL]。 
...
切換至 [可見度] 索引標籤，然後按一下 [從 Azure Active Directory 新增群組]。

![加入群組][api-management-add-groups]

從下拉式清單選取 [Azure Active Directory 租用戶]，然後在要新增的 [群組] 文字方塊中輸入所需群組的名稱。
...
## <a name="how-to-log-in-to-the-developer-portal-using-an-azure-active-directory-account"></a>如何使用 Azure Active Directory 帳戶登入開發人員入口網站
