...
Socket.IO 使用 WebSocket 提供 node.js 伺服器與用戶端之間的即時通訊。 此外，它還能支援遞補其他適用於舊版瀏覽器的傳輸 (例如長輪詢)。 本教學課程將逐步引導您裝載 Socket.IO 型聊天應用程式以做為 Azure Web 應用程式，並示範如何使用 [Azure Redis 快取]為應用程式調整規模。 如需 Socket.IO 的詳細資訊，請參閱 <http://socket.io/>。

> [!NOTE]
> 此工作中的程序適用於 [App Service Web Apps]。對於雲端服務，請參閱[在 Azure 雲端服務上使用 Socket.IO 建立 Node.js 聊天應用程式]。
> 
> 

## <a name="download-the-chat-example"></a>下載聊天範例
在此專案中，我們將使用 [Socket.IO GitHub 儲存機制]中的聊天範例。 請執行下列步驟來下載範例，並將它加入至您先前建立的專案。

1. 下載 Socket.IO 專案 (版本 1.3.5 已用於此文件) 的 [ZIP 或 GZ 封存版本]
...
2. 在文字編輯器中開啟 **app.js** 檔案。 變更包含 `var io = require('../..')(server);` 的字行，如下所示：
...
> [!NOTE]
> 若要完成此教學課程，您需要 Azure 帳戶。 如果您沒有帳戶，只需要幾分鐘的時間就可以建立免費試用帳戶。 如需詳細資料，請參閱 <a href="http://www.windowsazure.com/pricing/free-trial/?WT.mc_id=A7171371E" target="_blank">Azure 免費試用</a>。
> 
> 

1. 安裝 Azure 命令列介面 (Azure CLI)，並連線到您的 Azure 訂用帳戶。 請參閱 [安裝和設定 Azure CLI](../cli-install-nodejs.md)。
...
4. 使用下列命令，將現有的檔案認可到本機儲存機制：
   
        git add .
        git commit -m "Initial commit"
5. 使用下列命令，將檔案推送至 Azure Web Apps 儲存機制：
   ...
   > [!NOTE]
   > 在模組安裝期間，您可能會看到「找不到匯入的專案...」的錯誤。 請放心忽略這項錯誤訊息。
   > 
   > 
6. Socket.IO 使用 WebSockets (在 Azure 上依預設不會啟用)。 若要啟用 Web Sockets，請使用下列命令：
   ...
   > 
   > 若要使用 Azure 入口網站啟用 WebSocket，可從 [Web Apps] 刀鋒視窗中按一下 Web 應用程式，然後依序按一下 [所有設定] > [應用程式設定]。 在 [Web 通訊端] 下方，按一下 [開啟]。 然後按一下 [儲存] 。
   > 
   > 
7. 若要在 Azure 上檢視 Web 應用程式，請使用下列命令來啟動網頁瀏覽器，並瀏覽至裝載的 Web 應用程式：
...
> [!NOTE]
> 儲存快取的**主機名稱**和**主要金鑰**，因為後續步驟將會需要這些項目。
> 
> 

### <a name="add-the-redis-and-socketio-redis-modules"></a>新增 redis 和 socket.io-redis 模組
1. 從命令列中，切換至 **\\node\\chat** 目錄，然後執行下列命令。
   
        npm install socket.io-redis@0.1.4 redis@0.12.1 --save
   
   > [!NOTE]
   > 此命令中指定的版本是測試本文章所使用的版本。
   > 
   > 
2. 修改 **app.js** 檔案以在 `var io = require('socket.io')(server);` 之後緊接下列行
   ...
   > [!NOTE]
   > 雖然 **socket.io-redis** 配接器可以直接與 Redis 通訊，但是目前版本並不支援 Azure Redis 快取所需的驗證。 因此，系統會使用 **redis** 模組來建立初始連線，然後將該用戶端傳遞至 **socket.io-redis** 配接器。
   > 
...
Azure Web Apps 可用於多個 SKU，SKU 可以決定您網站適用的資源。 這包含允許 WebSocket 連線的數目。 如需詳細資訊，請參閱 [Web Apps 定價]頁面。
...
* **試圖將傳輸僅限制到 WebSockets**
  ...
  > [!NOTE]
  > 請注意，不支援 WebSockets 的舊版瀏覽器將無法在以上程式碼作用中時連線至該網站，因為此程式碼會將通訊僅限制到 WebSockets。
  > 
  > 
* **使用 SSL**
...
* **確認 web.config 設定**
  ...
  > [!NOTE]
  > 如果您的應用程式並未提供 **web.config** 檔案，那麼此一檔案只會由 Azure 網站來產生。 如果您在應用程式專案的根目錄中提供 **web.config** 檔案，Azure Web Apps 將會使用此檔案。
  > 
  > 
...
在本教學課程中，您學到如何建立裝載於 Azure Web 應用程式中的聊天應用程式。 您也可以將此應用程式交由 Azure 雲端服務託管。 如需相關作法的步驟，請參閱＜ [在 Azure 雲端服務上使用 Socket.IO 建立 Node.js 聊天應用程式]＞。

如需詳細資訊，也請參閱 [Node.js 開發人員中心]。

## <a name="whats-changed"></a>變更的項目
* 如需從網站變更為 App Service 的指南，請參閱： [Azure App Service 及其對現有 Azure 服務的影響]。
...
[在 Azure Redis 快取中建立快取]: ../redis-cache/cache-dotnet-how-to-use-azure-redis-cache.md

[socket.io-redis]: https://github.com/socketio/socket.io-redis
[Socket.IO GitHub 儲存機制]: https://github.com/socketio/socket.io
[ZIP 或 GZ 封存版本]: https://github.com/socketio/socket.io/releases
