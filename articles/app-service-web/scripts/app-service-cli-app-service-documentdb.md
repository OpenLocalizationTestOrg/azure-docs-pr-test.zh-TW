...
---
# <a name="connect-a-web-app-toocosmos-db"></a>連接 web 應用程式 tooCosmos DB

在此案例中，您將學習如何 toocreate Azure Cosmos DB 帳戶和 Azure web 應用程式。 然後，您將連結 hello Cosmos DB toohello web 應用程式使用應用程式設定。
...
此指令碼會使用下列命令 toocreate 資源群組、 web 應用程式、 Cosmos 資料庫和所有相關的資源的 hello。 Hello 資料表連結 toocommand 特定文件中的每個命令。
...
| [az cosmosdb create](https://docs.microsoft.com/en-us/cli/azure/cosmosdb#create) | 建立 Cosmos DB 帳戶。 這是 hello 資料儲存的位置。 |
| [az cosmosdb list-keys](https://docs.microsoft.com/en-us/cli/azure/cosmosdb#list-keys) | Hello 清單 hello 存取金鑰指定的 Cosmos DB 帳戶。 |
