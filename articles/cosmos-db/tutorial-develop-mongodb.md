...
---
# <a name="azure-cosmos-db-connect-tooa-mongodb-app-using-net"></a>Azure Cosmos DB： 連接 tooa MongoDB 應用程式使用.NET
...
本教學課程示範如何 toocreate Azure Cosmos DB 帳戶使用 hello Azure 入口網站和 toocreate 資料庫集合 toostore 使用和資料的 hello [MongoDB API](mongodb-introduction.md)。 
...
1. 在 Azure 入口網站中 hello hello **Azure Cosmos DB**頁面上，選取 hello API MongoDB 帳戶。 
2. Hello 左列中的 hello 帳戶 刀鋒視窗，按一下**快速入門**。 
3. 選擇您的平台 (*.NET 驅動程式*、*Node.js 驅動程式*、*MongoDB 殼層*、*Java 驅動程式*、*Python 驅動程式*)。 如果您沒有看到您的驅動程式或工具被列出，別擔心，我們會持續加入更多連線程式碼片段。 
4. 複製並貼入 MongoDB 應用程式，hello 程式碼片段，而且您準備好 toogo。

## <a name="set-up-your-mongodb-app"></a>設定您的 MongoDB 應用程式

您可以使用 hello[連接 tooMongoDB 虛擬機器上執行的 Azure 中建立 web 應用程式](../app-service-web/web-sites-dotnet-store-data-mongodb-vm.md)教學課程，以最少的修改，tooquickly 安裝 MongoDB 應用程式 (請在本機或已發行的 tooan Azure web 應用程式)，連接 MongoDB 帳戶 tooan API。  

1. 請遵循 hello 的教學課程中，有一項修改。  Hello Dal.cs 程式碼取代這：
...
2. 修改 hello hello hello Azure 入口網站中的索引鍵 頁面中的下列每個帳戶設定 hello Dal.cs 檔案中的變數：
...
您可以繼續進行下一個教學課程 toohello 並匯入您 MongoDB 資料 tooAzure Cosmos DB。  
