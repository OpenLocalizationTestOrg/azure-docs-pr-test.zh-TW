...
1. 登入 toohello [Azure 入口網站](https://portal.azure.com)，然後按一下 **+ 新增**hello 在左上方的囉 」 畫面。
...
3. 請確定您指定唯一的名稱在 hello**通知中樞**欄位。 選取您想要的 **[區域]**、**[訂用帳戶]** 和 **[資源群組]** \(如果您已經有的話)。 
   
    如果您已經有您希望 toocreate hello 中樞中的服務匯流排命名空間，請選取它透過 hello**選取現有**選項在 hello**命名空間**欄位。  否則，您可以使用 hello 預設名稱所建立，只要 hello 命名空間名稱可用，根據 hello 中樞名稱。 
   
    準備好時，請按一下 [建立] 。
   
      ![Azure 入口網站 - 設定通知中樞屬性](./media/notification-hubs-portal-create-new-hub/notification-hubs-azure-portal-settings.png)
4. 一旦建立 hello 命名空間與通知中樞，您將採用的 toohello 個別入口網站頁面。 
   
      ![Azure 入口網站 - 通知中樞入口網站頁面](./media/notification-hubs-portal-create-new-hub/notification-hubs-azure-portal-page.png)
5. 按一下**設定**然後**存取原則**-需要記下所使用的 tooyou hello 兩個連接字串，因為您稍後會需要這些 toohandle 推播通知。
